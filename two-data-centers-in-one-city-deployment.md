...
+ `wait-recover-timeout` 是当网络恢复后，切换回 `sync_recover` 模式的等待时间。默认时间为 0 秒。
+ `pause-region-split` 用于控制在 `async_wait` 和 `async` 状态是否暂停 Region 的 split 操作，暂停 Region split 可以防止在 `sync_recover` 状态同步数据时从属区域出现短暂的部分数据缺失。默认为 false。
