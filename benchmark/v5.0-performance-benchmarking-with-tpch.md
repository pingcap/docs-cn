---
title: TiDB TPC-H 性能对比测试报告 - v5.0 对比 v4.0
aliases: ['/docs-cn/dev/benchmark/v5.0-performance-benchmarking-with-tpch/']
---

# TiDB TPC-H 性能对比测试报告 - v5.0 对比 v4.0

## 测试目的

对比 TiDB v5.0 和 v4.0 OLAP 场景下的性能。

因为 TiDB v5.0 中新引入了 [TiFlash](/tiflash/tiflash-overview.md) 组件增强 TiDB HTAP 形态，本文的测试对象如下：

+ v4.0 仅从 TiKV 读取数据。
+ v5.0 仅从 TiKV 读取数据。
+ v5.0 通过智能选择混合读取 TiKV、TiFlash 的数据。
...
2. 通过 TiUP 的 bench 工具导入 TPC-H 10G 数据。
    ...
    * 执行以下命令将数据导入 v5.0：
        ...
        ```bash
        tiup bench tpch prepare \
          --host ${tidb_v5_host} --port ${tidb_v5_port} --db tpch_10 --password ${password} \
          --sf 10 \
          --tiflash \
          --analyze --tidb_build_stats_concurrency 8 --tidb_distsql_scan_concurrency 30
        ```
...
3. 运行 TPC-H 的查询。
    ...
    2. 查询并记录耗时。
        ...
        * 对于 TiDB v4.0，使用 MySQL 客户端连接到 TiDB，然后执行查询，记录 v4.0 查询耗时。
        * 对于 TiDB v5.0，使用 MySQL 客户端连接到 TiDB，再根据测试的形态，选择其中一种操作：
            * 设置 `set @@session.tidb_isolation_read_engines = 'tikv,tidb';` 后，再执行查询，记录 v5.0 仅从 TiKV 读取数据的查询耗时。
            * 设置 `set @@session.tidb_isolation_read_engines = 'tikv,tiflash,tidb';` 后，再执行查询，记录 v5.0 通过智能选择从 TiKV 和 TiFlash 混合读取数据的查询耗时。
...
| Query ID |  v4.0  |  v5.0 TiKV Only |  v5.0 TiKV / TiFlash Automatically |
| :-------- | :----------- | :------------ | :-------------- |
| 1       |    7.78s   |      7.45s  |      2.09s    |
| 2       |    3.15s   |      1.71s  |      1.71s    |
| 3       |    6.61s   |      4.10s  |      4.05s    |
...
以上性能图中蓝色为 v4.0，红色为 v5.0（仅从 TiKV 读），黄色为 v5.0（从 TiKV、TiFlash 智能选取），纵坐标是查询的处理时间。纵坐标越低，表示性能越好。

- v5.0（仅从 TiKV 读取数据），即 TiDB 仅会从 TiKV 中读取数据。将该结果与 v4.0 的结果对比可得知，TiDB、TiKV 升级至 4.0 版本后，TPC-H 性能得到的提升幅度。
- v5.0（从 TiKV、TiFlash 智能选取），即 TiDB 优化器会自动根据代价估算选择是否使用 TiFlash 副本。将该结果与 v4.0 的结果对比可得知，在 v5.0 完整的 HTAP 形态下，TPC-H 性能得到的提升幅度。
