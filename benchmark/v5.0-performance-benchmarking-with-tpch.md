...
对比 TiDB v5.0 MPP 模式下和主流分析引擎例如 Greenplum 和 Apache Spark 最新版在 TPC-H 100 下的性能表现。结果显示，TiDB 5.0 MPP 模式下相对这些方案有数倍的性能提升。

因为 TiDB v5.0 中新引入了 [TiFlash](/tiflash/tiflash-overview.md) 组件的 MPP 模式，这大幅增强 TiDB HTAP 形态，本文的测试对象如下：

+ v5.0 列式存储以及 MPP 执行模式
+ Greenplum 6.15.0
...
3 节点：
Intel(R) Xeon(R) CPU E5-2630 v4 @ 2.20GHz，40 核 189 GB 内存，NVMe 3TB
...
v5.0 的 TiDB 集群使用默认参数配置。所有 TPC-H 测试表均以 TiFlash 列存进行同步，无额外分区和索引。

##### Greenplum 配置

Greenplum 集群使用额外的一台 Master 节点部署，每台 Segment Server 部署 8 Segments。存储格式为 append-only / 列式存储，分区键为主键。
...
Apache Spark 测试使用 Apache Parquet 作为存储格式，以 Standalone 方式启动集群，使用 NVMe SSD 盘作为 `spark.local.dir` 以借助快速盘加速 Shuffle Spill 过程，无额外分区和索引。
...
以上性能图中蓝色为 TiDB v5.0，红色为 Greenplum 6.15.0，黄色为 Apache Spark 3.1.1，纵坐标是查询的处理时间。纵坐标越低，表示性能越好。
