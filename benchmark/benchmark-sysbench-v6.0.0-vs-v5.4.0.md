...
summary: TiDB v6.0.0 在 OLTP 场景下的 Sysbench 性能表现对比 v5.4.0。结果显示，Read Write 负载性能有大幅提升，提升了 16.17%，其他负载性能基本持平。测试环境为AWS EC2，硬件配置包括 PD、TiKV、TiDB和Sysbench实例。软件版本为 v5.4.0 和 v6.0.0，参数配置相同。测试方案包括部署 TiDB、导入数据、执行测试命令和备份数据。测试结果显示 Point Select 性能基本持平，Update Non-index 性能基本持平，Update Index 性能下降了 3.05%。Update Index 性能下降了 3.05%。
---
