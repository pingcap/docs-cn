...
默认情况下，TiDB 使用以 TTL 列开头且符合条件的索引扫描过期行，从而避免扫描尚未过期的索引项。TiDB 根据所选索引的 Region 拆分任务，完全位于过期时间边界之后的 Region 不会生成任务。
...
- 索引是可见的二级索引或非聚簇主键索引，且第一列是 TTL 列。
- 每个索引列都索引完整的列值，不使用前缀索引或表达式索引。
- 对于唯一复合索引，除 TTL 列之外的其他列都为 `NOT NULL`。
- 对于非唯一索引，TiDB 必须能将数据行标识追加到扫描顺序中。在索引中包含全部主键列即可满足该要求；不支持仅包含复合主键中的部分列。
- 用于标识数据行的列不是 `SET`、`FLOAT` 或 `DOUBLE` 类型。

TTL 任务不使用聚簇主键索引、部分索引、全局索引、多值索引、列式索引或不可见索引。
