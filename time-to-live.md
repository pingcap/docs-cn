...
* 对于使用[聚簇索引](/clustered-indexes.md)的表，如果主键的类型不是整数类型或二进制字符串类型，TTL 任务将无法被拆分成多个子任务，意味着扫描阶段使用的是 [TableFullScan 算子](/explain-overview.md#算子简介)（如果能够拆分子任务，意味着扫描阶段使用的是 TableRangeScan 算子）。这将导致 TTL 任务只能在一个 TiDB 节点上按顺序执行。如果表中的数据量较大，TTL 任务的执行可能会变得缓慢。其中，二进制字符串类型主要指下面几种：
