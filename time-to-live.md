...
默认情况下，TiDB 使用以 TTL 列开头且符合条件的索引扫描过期行。与按照表键顺序扫描相比，按照 TTL 列顺序扫描可以避免重复扫描尚未过期的索引项。TiDB 根据所选索引的 TiKV Region 拆分任务，完全位于过期时间边界之后的 Region 不参与任务。

该行为由全局变量 [`tidb_ttl_enable_index_scan`](/system-variables.md#tidb_ttl_enable_index_scan) 控制，默认开启。当关闭该变量或没有符合条件的索引时，TiDB 会回退到按照表键顺序扫描，并阻止优化器为该扫描选择二级索引。

TTL 任务使用的索引必须满足以下全部条件：

- 索引是 public、可见且局部的二级索引或非聚簇主键索引。聚簇主键索引、全局索引、多值索引、列式索引和条件索引不符合条件。
- 索引的第一列是 TTL 列，并且每个索引列都存储可见表列的完整值。前缀索引和包含隐藏表达式列的索引不符合条件。
- 索引能提供稳定且唯一的分页顺序。对于唯一复合索引，除 TTL 列之外的每个索引列都必须为 `NOT NULL`。对于非唯一索引，声明的索引列必须包含完整表键或完全不包含表键。如果完全不包含表键，且 TiDB 支持相应的物理顺序，TiDB 会将隐式表键追加到分页顺序中。
- 最终用于分页的列不包含 `SET`、`FLOAT` 或 `DOUBLE` 类型的列。

这里的表键是聚簇主键；对于没有聚簇主键的表，表键是 `_tidb_rowid`。仅包含复合表键中部分列的非唯一索引不符合条件。无符号整数聚簇主键以及包含前缀主键列的 common handle 不能用作隐式后缀，但如果索引显式包含完整表键，该索引仍然可能符合条件。

如果存在多个符合条件的索引，TiDB 依次优先选择仅包含 TTL 列的单列索引、显式包含完整表键的索引，以及分页元组最短的索引。如果分页元组长度相同，TiDB 优先选择需要读取列数更少的索引。

> **注意：**
>
> - 在滚动升级期间，如果某张表原本会使用索引扫描，但集群中 TiDB server 的构建版本不一致，TiDB 不会为该表创建新的 TTL 任务。TiDB server 的构建版本一致后，调度器会重试。如需在升级期间继续使用表键扫描创建 TTL 任务，请临时关闭 `tidb_ttl_enable_index_scan`。
> - 如果 TiDB 无法获取或比较 TiDB server 信息，则使用表键扫描创建任务。
> - 如果在创建 TTL 任务后删除所选索引，受影响的子任务会报错。后续 TTL 任务可以选择另一个符合条件的索引，或回退到表键扫描。
> - 每个扫描分页都是独立的 SQL 语句。如果在扫描期间更新索引列的值，当前任务可能跳过该行或再次读到该行。删除前，TiDB 会再次检查过期条件，因此不会删除不再过期的行。当前任务跳过的过期行仍可由后续 TTL 任务处理。
...
+ `mysql.tidb_ttl_task` 表中包含了正在执行的 TTL 子任务。单个 TTL 任务会被拆分为多个子任务，该表中记录了正在执行的这些子任务的信息。`scan_index_id` 列表示创建任务时选择的扫描路径。非 `NULL` 值是所选索引的 ID，`NULL` 表示该子任务按照表键顺序扫描。
