...
要使用索引扫描，TTL 列必须是索引的第一列。这样，TiDB 可以直接扫描可能包含过期行的索引范围，而不必扫描尚未过期的索引项。

默认情况下，TiDB 会自动选择一个符合条件的索引进行扫描。该行为由全局变量 [`tidb_ttl_enable_index_scan`](/system-variables.md#tidb_ttl_enable_index_scan) 控制，默认开启。当关闭该变量或没有符合条件的索引时，TiDB 会回退到按照表键顺序扫描，并阻止优化器为该扫描选择二级索引。对于使用聚簇主键的表，表键扫描会使用该聚簇主键。

除 TTL 列位于首位外，符合条件的索引还必须满足以下要求：

- 索引是可见的二级索引或非聚簇主键索引，且不是部分索引、全局索引、多值索引、向量搜索索引或全文索引。
- 每个索引列均索引完整的列值，不能使用前缀索引或表达式索引。
