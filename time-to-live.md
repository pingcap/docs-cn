...
默认情况下，TiDB 使用以 TTL 列开头且符合条件的索引扫描过期行，从而避免扫描尚未过期的索引项。
...
- 索引是二级索引或非聚簇主键。TiDB 不使用聚簇主键、部分索引、全局索引、多值索引、向量搜索索引、全文索引或不可见索引进行该扫描。
- 索引的第一列是 TTL 列，且每个索引列都索引完整的列值，不使用前缀索引或表达式索引。
- 对于包含多列的唯一索引，除 TTL 列之外的其他列都为 `NOT NULL`。
- 非唯一索引满足以下要求：
    - 索引包含全部主键列或完全不包含主键列，不能只包含复合主键中的部分列。
    - 如果表的聚簇主键由单个无符号整数列组成，索引必须包含该主键列。
    - 如果聚簇主键中的任意列指定了前缀长度，索引必须包含全部主键列。
    - 如果集群启用了[新排序规则框架](/character-set-and-collation.md#新框架下的排序规则支持)，对于由较早 TiDB 版本创建且聚簇主键包含非二进制字符串列的表，请在索引中包含全部主键列，以避免兼容性限制。
- 索引列不能是 `SET`、`FLOAT` 或 `DOUBLE` 类型。对于不包含主键列的非唯一索引，主键列也不能是这些类型。
