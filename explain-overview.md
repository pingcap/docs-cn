...
SQL 是一种声明性语言。一条 SQL 语句描述的是最终结果应该如何，而非查询过程中所用的方法。执行一条查询语句时，TiDB 会考虑所有可能的执行方法，包括表连接的顺序，以及是否可以使用索引。TiDB 考虑执行计划的过程称为 SQL 优化。

`EXPLAIN` 的返回结果显示了 TiDB 执行某条语句时选用的执行计划。也就是说，TiDB 在考虑过上数百或数千种可能的执行计划后，最终认定该执行计划消耗的资源最少，执行的速度最快。
...
`EXPLAIN` 实际不会执行该查询。[`EXPLAIN ANALYZE`](/sql-statements/sql-statement-explain-analyze.md) 可用于实际执行查询并显示执行计划。如果 TiDB 所选的执行计划非最优，可用 `EXPLAIN` 或 `EXPLAIN ANALYZE` 来进行诊断。有关 `EXPLAIN` 用法的详细内容，参阅以下文档：
...
+ `id` 描述算子名，或执行 SQL 语句所需的子任务。详见[算子简介](#算子简介)。
+ `estRows` 显示 TiDB 预计会处理的行数。该预估数可能基于字典信息（例如访问方法基于主键或唯一键），或基于 `CMSketch` 或直方图等统计信息。
+ `task` 显示算子执行语句时所在的位置。详见 [Task 简介](#task-简介)。
+ `access-object` 显示被访问的表、分区和索引。显示的索引为部分索引，如以上示例中 TiDB 使用了 `a` 列的索引。在有组合索引的情况下，该字段显示的内容可能有用。
+ `operator info` 显示该次访问的额外信息。详见 [`operator info` 结果](#operator-info-结果)。
...
算子的结构是树状的，但在查询执行过程中，并不严格要求子节点的任务在父节点之前完成。TiDB 支持查询内并行处理，即为子节点“流入”父节点。父节点、子节点和同级节点可能并行执行查询的一部分。
...
推荐使用同一类型的数据以及同一类型的[字符串和排序规则](/character-and-collation.md)进行比较，以避免引入额外的 `cast` 操作而导致不能利用索引。
...
+ `keep order:false` 表示该查询的语义不需要 TiKV 按顺序返回结果。如果修改查询指定排序（例如 `SELECT * FROM t WHERE a = 1 ORDER BY id`），该字段的返回结果为 `keep order:true`。
