...
基于 Raft 协议和合理的部署拓扑规划，TiDB 实现了集群的高可用，当集群少数节点挂掉依然能对外提供服务。 在此基础上，为了进一步保证用户数据的安全，TiDB 提供了集群的备份恢复功能，作为数据安全的最后一道防线，使得集群能够免于严重的自然灾害、提供业务误操作 “复原” 的能力。
...
- 审计业务的历史数据，满足司法审查的需求。
- 复制 (Clone) 生产环境，方便问题诊断、性能调优验证、测试等。
...
全量备份是对集群某个时间点的全量数据进行备份。TiDB 全量备份基于快照数据，即某一时刻最新的和满足事务一致性的数据。因而全量备份也可理解为[快照备份](/br/br-usage-backup.md#备份-tidb-集群快照)。

推荐定期备份集群快照到备份存储，例如在每天零点进行集群快照备份，这也是实现 PITR 的一个前提。

### 日志备份

全量备份一般会占用较大存储空间，且只包含某个时间点的集群数据。如果你需要**灵活的选择恢复的时间点**，可以执行[日志备份](/)，以小批量的形式定期将 TiDB 变更数据备份到指定存储中。这种备份方式是**备份最佳实践**，也是实现 PITR 的基础。

### 全量恢复或恢复部分库和表

你可以在一个空集群上执行快照数据备份的恢复，将该集群恢复到快照备份对应数据状态。使用请参考[恢复集群快照备份](/br/br-usage-restore.md#恢复快照备份数据)。

此外你可以只恢复备份数据中指定库/表的局部数据。该功能在恢复过程中过滤掉不需要的数据。使用请参考[恢复备份数据中指定库表的数据](/br/br-usage-restore.md#恢复备份数据中指定库表的数据)。

### 恢复到集群的历史任意时间点 (PITR)

BR 提供了一键恢复的命令，`br restore point`。你可以指定要恢复的时间点，恢复该时间点之前的快照数据或日志备份数据。BR 会自动判断和读取恢复需要的数据，然后将这些数据依次恢复到指定的集群。使用请参考 []()。

## 备份存储

TiDB 支持将数据备份到 Amazon S3、Google Cloud Storage (GCS)、Azure Blob Storage、NFS，或者实现 S3 协议的其他文件存储服务。
...
在上述功能的配置确保备份和恢复的集群一致的**前提**下，BR 和 TiKV、TiDB 及 PD 还可能因为版本之间实现不一致的问题导致功能运行失败，因此 BR 内置了版本检查。
