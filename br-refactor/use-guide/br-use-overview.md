...
**TiDB 支持两种类型的备份，应该使用哪种备份？** 全量备份包含集群某个时间点的全量数据，日志备份包含业务写入在 TiDB 产生的数据变更记录，日志备份的日志指的是 TiKV 中的 kv 变更数据的记录。我们推荐这两种备份方式配合一起使用：
...
* 每个全量备份保存到命名带有备份日期的目录下，例如 `backup-${cluster-id}/snapshot-202209081330`。
