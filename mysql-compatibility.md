...
* 分区表支持 `HASH`、`RANGE` 和 `LIST` 分区类型。分区表还支持 `ADD`、`DROP`、`TRUNCATE` 操作。其他分区操作会被忽略，TiDB 可能会报 `Warning: Unsupported partition type %s, treat as normal table` 错误，其中 `%s` 为不支持的具体分区类型。不支持以下分区表语法：
    + `PARTITION BY LIST`
    + `PARTITION BY KEY`
    + `SUBPARTITION`
