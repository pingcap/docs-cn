...
* 使用 `ALTER TABLE` 语句修改一个表的多个模式对象（如列、索引）时，不允许在多个更改中指定同一个模式对象。例如，执行 `ALTER TABLE t1 MODIFY COLUMN c1 INT, DROP COLUMN c1` 会输出 `Unsupported operate same column/index` 的错误。
