...
2. 终止正在后台运行的 `ANALYZE` 任务。
   ...
   - 如果 [`enable-global-kill`](/tidb-configuration-file.md#enable-global-kill-从-v610-版本开始引入) 的值为 `true` (默认为 `true`）, 你可以直接执行 `KILL TIDB ID;` 语句。`ID` 为上一步中查询得到的后台 `ANALYZE` 任务的 `ID`。
