...
TiDB 使用统计信息来决定[索引的选择](/choose-index.md)。变量 `tidb_analyze_version` 用于控制所收集到的统计信息。目前 TiDB 中支持两种统计信息：`tidb_analyze_version = 1` 以及 `tidb_analyze_version = 2`（默认）。两种版本中，TiDB 维护的统计信息如下：
...
> `tidb_enable_fast_analyze` 为实验性功能，目前并不与 `tidb_analyze_version=2` 的统计信息完全匹配，因此设置 `tidb_enable_fast_analyze` 请将 `tidb_analyze_version` 设置为 1。
