...
从 v7.1.0 开始，TiDB 引入了系统变量 [`lite-init-stats`](/tidb-configuration-file.md#lite-init-stats-从-v710-版本开始引入) 用于控制轻量级的统计信息初始化。

- 当 `lite-init-stats` 为 `true` 时，统计信息初始化时列和索引的直方图、TopN、CMSketch 均不会加载到内存中。
- 当 `lite-init-stats` 为 `false` 时，统计信息初始化时索引和主键的直方图、TopN、CMSketch 会被加载到内存中，非主键列的直方图、TopN、CMSketch 不会加载到内存中。当优化器需要某一索引或者列的直方图、TopN、CMSketch 时，这些统计信息会被会同步或者异步地加载到内存中。

将 `lite-init-stats` 设置为 `true`，可以加速统计信息初始化，并且避免不必要的统计信息加载，降低 TiDB 的内存使用。
