...
TiDB 从 v6.1.0 开始引入了统计信息收集的内存限制，当设置了 [`tidb_mem_quota_analyze`](/system-variables.md#tidb_mem_quota_analyze-从-v610-版本开始引入)，且统计信息收集任务（包括用户手动执行的 `ANALYZE` 和 TiDB 后台自动触发的）的总内存占用超过这个阈值时，用户执行的 `ANALYZE` 会被终止退出，并通过错误信息提示用户尝试更小的采样率或稍后重试。如果 TiDB 后台自动执行的统计信息更新任务因内存超限而退出，且使用的采样率高于默认值，则会使用默认采样率重试一次。当该变量值为负数或零时，TiDB 不对更新统计信息的前后台任务进行内存限制。

`tidb_mem_quota_analyze` 的合理值配置与集群的数据相关，在使用默认采样率的情况下，主要和列的数量以及列上的值的大小相关，也和 TiDB 的内存配置相关，具体的配置思路如下：

> **注意：**
>
> 配置思路仅供参考，实际配置需要在真实场景中测试决定。

- 最小值：需要大于集群上最宽的表采集统计信息时，TiDB 的最大内存使用。一个粗略的参考信息是，在测试集上，20 列的表在默认配置下，统计信息采集的最大内存使用约为 800 MiB；160 列的表在默认配置下，统计信息采集的最大内存使用约为 5 GiB。
- 最大值：需要小于集群在不进行统计信息采集时的内存空余量。
