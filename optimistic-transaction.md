...
> **注意：**
>
> 自 v3.0.8 开始，新创建的 TiDB 集群默认使用[悲观事务模型](/pessimistic-transaction.md)。但如果从 3.0.7 版本及之前的创建的集群升级到 >= 3.0.8 的版本，不会改变默认事务模型，即**只有新创建的集群才会默认使用悲观事务模型**。
...
5. TiDB 开始两阶段提交，保证事务的原子性的前提下，数据持久化。
