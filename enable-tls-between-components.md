...
- TiDB 与 TiKV、PD、TiFlash
- TiKV 与 PD，TiFlash 与 PD
- TiFlash 与 TiKV
...
- 如果 TiDB 集群部署在本地的数据中心，TiDB、PD、TiKV、TiFlash 和各种 client 在每次新建相互通讯的连接时都会重新读取当前的证书和密钥文件内容，实现证书和密钥的重新加载，无需重启 TiDB 集群。
- 如果 TiDB 集群部署在自己管理的 Cloud，TLS 证书的签发需要与云服务商的证书管理服务集成，TiDB、PD、TiKV、TiCDC、TiFlash 组件的 TLS 证书支持自动轮换，无需重启 TiDB 集群。
