...
> - 从 v7.1.3 开始，TiDB 会根据每个 Region 的冗余版本数量 [`gc.auto-compaction.redundant-rows-threshold`] 和比例 [`gc.auto-compaction.redundant-rows-percent-threshold`] 自动触发 compaction，从而提高 Compaction Filter 的 GC 速度。因此，在 v7.1.3 及之后的版本中，如果遇到上述情况，建议调整这两个参数，无需关闭 Compaction Filter。
