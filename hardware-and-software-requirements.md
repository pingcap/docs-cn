...
| TiKV | 数据盘和日志盘建议各最少预留 100 GB | 低于 80% |
| TiFlash | 数据盘建议最少预留 100 GB，日志盘建议最少预留 30 GB | 低于 80% |
| TiUP | 单个 TiUP cluster 占用空间不会超过 1 GB，在部署过程中会占用少于 1 MB /tmp 空间存放临时配置文件 | 不涉及|
| Ngmonitoring | Conprof：3 x 1 GB x 组件数量（3 天，每个组件每天占用约 1 GB） + 20 GB 预留空间 <br/>TopSQL：30 x 50 MB x 组件数量（30 天，每个组件每天占用约 50 MB） <br/> Top SQL 和 Conprof 共享预留空间 | 不涉及 |
