...
> - 从 v6.3.0 开始，在 Linux AMD64 架构的硬件平台下部署 TiFlash 时，CPU 必须支持 AVX2 指令集。确保命令 `cat /proc/cpuinfo | grep avx2` 有输出。通过使用 AVX2 指令集，TiFlash 的向量化引擎能提供更好的性能。而在 ARM 架构的硬件平台上部署 TiFlash 时，CPU 必须支持 ARMv8 指令集。确保命令 `cat /proc/cpuinfo | grep 'crc32' | grep 'asimd'` 有输出。
