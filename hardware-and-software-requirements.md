...
- 磁盘空间：
    - 存储节点：推荐 200 GB+，用作增加 TiFlash 副本、Region 副本迁移时向 Amazon S3 上传数据前的本地缓冲区。此外，还需要一个与 Amazon S3 兼容的对象存储。
    - 计算节点：推荐 100 GB+，主要用于缓存从存储节点读取的数据以提升性能。计算节点的缓存可能会被完全使用，这是正常现象。
- CPU 以及内存等要求参考下文。
...
| TiFlash 计算节点 | 16 核+ | 32 GB+ | SSD, 100 GB+ | 千兆网卡 | 0（参见下文“注意”说明） |
...
| TiFlash 计算节点 | 32 核+ | 64 GB+ | SSD, 100 GB+  | 万兆网卡（2 块最佳） | 0（参见下文“注意”说明） |
