...
+ **经过测试和验证的平台**：对于以下操作系统和 CPU 架构组合，每个 TiDB 版本均会通过 {{{ .company }}} 的系统性测试矩阵进行验证。这些组合推荐用于生产环境部署。
...
+ **兼容平台**：在以下组合中，TiDB 可以进行编译、构建和部署，并支持 OLTP、OLAP 和数据工具。这些组合未纳入每个版本的系统性测试矩阵；其兼容性通过编译构建、部署验证以及 TiDB 用户的实际使用得到确认。如需在这些平台上进行生产部署，请联系 {{{ .company }}}，以确认你的订阅服务是否涵盖相应支持。
