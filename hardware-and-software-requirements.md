...
| :-- | :-- |
| TiDB |  |
| PD | 数据盘和日志盘建议最少各预留 20 GB <br/> 健康水位使用率应低于 90%  |
| TiKV | 数据盘和日志盘建议各最少预留 100 GB <br/> 健康水位使用率应低于 80% |
| TiFlash | 日志盘：建议最少预留 30 GB<br/>数据盘：建议最少预留 100 GB，健康水位使用率应低于 80% |
| TiUP | TiUP cluster 会占用几百兆空间，在部署过程中会占用少于 1 MB /tmp 空间存放临时配置文件 |
| Ngmonitoring | Conprof: 3 x 1 GB x 组件数量 + 20 GB（3 天，每个组件每天估算 1 GB, 20 GB 是预留空间）<br/>TopSQL: 30 x 50 MB x 组件数量（30 天，每个组件每天估算 50 MB，和 Conprof 共享预留空间) |
