...
+ **经过测试和验证的平台。** 对于以下操作系统和 CPU 架构组合，TiDB 的每个版本都会根据 PingCAP 的系统化测试矩阵进行验证。推荐在生产环境中部署这些组合。
...
+ **兼容平台。** 在以下组合中，TiDB 可以编译、构建和部署，并支持 OLTP、OLAP 和数据工具。这些组合未纳入每个版本的系统化测试矩阵；其兼容性通过构建、部署验证以及 TiDB 用户的使用得到验证。对于在这些平台上的生产部署，请联系 {{{ .company }}} 以确认你的订阅所涵盖的支持范围。
