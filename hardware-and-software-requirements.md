...
> - TiKV 硬盘大小配置建议 PCIe SSD 不超过 4 TiB，普通 SSD 不超过 1.5 TiB。
> - 如果你在云服务商（如 AWS、Google Cloud 或 Azure）上部署 TiDB 集群，TiKV 节点必须使用云盘。在云环境中，实例崩溃时，本地磁盘上的数据会丢失。
...
- TiFlash 硬盘总容量大致为：`整个 TiKV 集群的需同步数据容量 / TiKV 副本数 * TiFlash 副本数`。例如整体 TiKV 的规划容量为 1 TiB、TiKV 副本数为 3、TiFlash 副本数为 2，则 TiFlash 的推荐总容量为 `1024 GiB / 3 * 2`。用户可以选择同步部分表数据而非全部，具体容量可以根据需要同步的表的数据量具体分析。


在部署 TiCDC 时，建议在大于 500 GiB 的 PCIe SSD 磁盘上部署。
