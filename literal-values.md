...
字符串分为两种：二进制字符串(binary string)和非二进制字符串。二进制字符串由字节序列构成，它的 charset 和 collation 都是 `binary`，在互相比较时利用**字节**作为单位；非二进制字符串由字符序列构成，有除 `binary` 以外的多种 charset 和 collation，在相互比较时用**字符**（一个字符可能包含多个字节，取决于 charset 的选择）作为单位。
...
一个 String Literal 可以拥有一个可选的 `character set introducer` 和 `COLLATE clause`，可以用来指派特定的字符集跟 collation。
