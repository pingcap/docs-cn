...
本文档提供了 Oracle 与 TiDB 的函数和语法差异对照，方便你根据 Oracle 函数查找对应的 TiDB 函数，了解语法差异。

本文档的内容是基于 Oracle 12.2.0.1.0 和 TiDB v5.4.0。其他版本可能存在差异。

## 函数对照表

下表列出了 Oracle 与 TiDB 函数的对照表。
...
Oracle：`DELETE FROM test t WHERE t.xxx = xxx;`

TiDB： `DELETE FROM test WHERE xxx = xxx;`

说明：TiDB 不支持删除语句中为表起别名。
...
说明：Oracle 只能识别单引号，TiDB 能识别单引号与双引号。
...
说明：Oracle 空串就是 NULL，TiDB 需要把空串转换为 NULL 数据。TiDB 中 NULL 和 '' 是有区别的。
...
说明：Oracle 中的 `ROWNUM` 可以使用 `LIMIT` 等价代替。Hibernate Query Language (HQL) 方式运行带 `LIMIT` 的 SQL 语句会出现错误，需要将 Hibernate 的运行方式改为 SQL 方式运行。

### UPDATE 语句

Oracle：`UPDATE test1 SET(test1.name,test1.age)= (SELECT test2.name,test2.age FROM test2 WHERE test2.id=test1.id)`

TiDB：`UPDATE test1,test2 SET test1.name=test2.name,test1.age=test2.age WHERE test1.id=test2.id`

说明：TiDB 在多表更新的时候，需要在 `SET` 的时候把具体的字段更新关系都列出来。

### 派生表别名

Oracle：`SELECT * FROM (SELECT * FROM test)`

TiDB：`SELECT * FROM (SELECT * FROM test)` t

说明：TiDB 多表查询的时候，每一个派生出来的表都必须有一个自己的别名。

### 差集运算

Oracle：`SELECT * FROM t1 MINUS SELECT * FROM t2;`

TiDB：`SELECT * FROM t1 EXCEPT SELECT * FROM t2;`

说明：TiDB 不支持 `MINUS`，需要改写为 `EXCEPT`。

### 空值取别名

Oracle： `SELECT null AS xx FROM dual`

TiDB：`SELECT '' AS xx FROM dual`

说明：NULL 与 '' 在 TiDB 中含义不同。Oracle 中 NULL 在 TiDB 中需要改写为 ''。
...
Oracle：`SELECT * FROM tables OFFSET 0 ROWS FETCH NEXT 2000 ROWS ONLY`

TiDB：`SELECT * FROM tables LIMIT 2000 OFFSET 0`

说明：Oracle 中 `OFFSET m` 表示跳过 `m` 行数据，`FETCH NEXT n ROWS ONLY` 表示取 `n` 条数据。TiDB 使用 `LIMIT n OFFSET m` 等价改写 `OFFSET m ROWS FETCH NEXT n ROWS ONLY`。

### Oracle 和 TiDB 排序 NULL 的顺序不同

Oracle：`ORDER BY COLUM ASC NULLS FIRST`

TiDB：`ORDER BY COLUM ASC`

说明：

- Oracle 实现方式：`ORDER BY COLUM ASC` 时，NULL 默认被放在最后；ORDER BY COLUM DESC 时，NULL 默认被放在最前。NULLS FIRST 时，强制 NULL 放在最前，非 NULL 的仍然按声明顺序 [ASC\|DESC] 进行排序。NULLS LAST 时，强制 NULL 放在最后，非 NULL 的仍然按声明顺序 [ASC\|DESC] 进行排序。
- MySQL 和 TiDB 的实现方式：ORDER BY COLUM ASC 时，NULL 默认被放在最前。`ORDER BY COLUM DESC` 时，NULL 默认被放在最后。
    - `O：SELECT * FROM t1 ORDER BY name NULLS FIRST;` 等价于 `T：SELECT * FROM t1 ORDER BY NAME ;`
    - `O：SELECT * FROM t1 ORDER BY name DESC NULLS LAST;` 等价于 `T：SELECT * FROM t1 ORDER BY NAME DESC;`
    - `O：SELECT * FROM t1 ORDER BY NAME DESC NULLS FIRST;` 等价于 `T：SELECT * FROM t1 ORDER BY ISNULL(name) DESC, name DESC;`
    - `O：SELECT * FROM t1 ORDER BY name ASC NULLS LAST;` 等价于 `T：SELECT * FROM t1 ORDER BY ISNULL(name), name;`
