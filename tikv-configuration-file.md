...
> **注意：**
>
> 引入该配置项是为了缓解备份期间遇到的 S3 限流导致备份失败的问题。在 v5.4.3、v6.1.1、v6.2.0 及以上版本，[BR 备份数据存储的目录结构进行了优化](/br/backup-and-restore-design.md#备份文件布局)，根本上解决了 S3 限流导致备份失败的问题 ，因此，该配置目前已废弃，不再推荐使用。 
