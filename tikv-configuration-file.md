...
> **注意：**
>
> 引入该配置项是为了缓解备份期间遇到的 S3 限流导致备份失败的问题。在 v5.4.3、v6.1.1、v6.2.0 及以上版本，[BR 备份数据存储的目录结构进行了优化](/br/backup-and-restore-design.md#备份文件布局)，根本上解决了 S3 限流导致备份失败的问题 ，因此，该配置不再推荐使用。 

+ 备份阶段 S3 分块上传的块大小。
+ TiKV 备份数据到 S3 时，当备份文件大于该配置设置的大小时，会自动进行[分块上传](https://docs.aws.amazon.com/zh_cn/AmazonS3/latest/API/API_UploadPart.html)。根据压缩率的不同，96 MB Region 产生的备份文件大约在 10 MB~30 MB 之间，因此该配置项也可以适当调整，来控制备份期间整体发往 S3 的请求数量。
+ 默认值：5MB
