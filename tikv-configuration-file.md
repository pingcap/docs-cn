...
TiKV 配置文件比命令行参数支持更多的选项。你可以在 [etc/config-template.toml](https://github.com/tikv/tikv/blob/release-8.1/etc/config-template.toml) 找到默认值的配置文件，重命名为 config.toml 即可。
...
+ gRPC 消息的压缩算法。它会影响 TiKV 节点之间的 gRPC 消息的压缩算法。从 v6.5.11、v7.1.6、v7.5.3、v8.1.1 起，它也会影响 TiKV 向 TiDB 发送的 gRPC（响应）消息的压缩算法。
...
+ 触发 TiKV 自动 compaction 需要的 RocksDB tombstone 所占比例。当 tombstone 所占比例达到此阈值，或满足 [`tombstone-num-threshold`](#tombstone-num-threshold-从-v757-和-v900-版本开始引入) 条件之一时，TiKV 将触发自动 compaction。
+ 仅在关闭 [Compaction Filter](/garbage-collection-configuration.md) 时生效。
+ 默认值：`30`
+ 最小值：`0`
+ 最大值：`100`
...
+ 触发 TiKV 自动 compaction 需要的冗余的 MVCC 数据行数，包含 RocksDB tombstone、TiKV stale versions 和 TiKV deletion tombstones。当冗余的 MVCC 数据行数达到此阈值，或满足 [`redundant-rows-percent-threshold`](#redundant-rows-percent-threshold-从-v757-和-v900-版本开始引入) 条件之一时，TiKV 将触发自动 compaction。
