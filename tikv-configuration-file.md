...
+ 默认值：100。如果当前为分区 Raft KV (storage.engine="partitioned-raft-kv")，默认值为 5。
+ 最小值：0
...
+ 触发 RocksDB compaction 需要的 redundant mvcc row 个数。该配置只对分区 Raft KV (storage.engine="partitioned-raft-kv") 生效。
+ 默认值: `50000`
+ 最小值: `0`

### `region-compact-redundant-rows-percent`
+ 触发 RocksDB compaction 需要的 redundant mvcc row 所占比例。该配置只对分区 Raft KV (storage.engine="partitioned-raft-kv") 生效。
+ 默认值: `20`
+ 最小值: `1`
+ 最大值: `100`
