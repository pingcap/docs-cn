...
+ TiKV 启动时会预留一块空间用作磁盘空间保护，当磁盘剩余空间不足该值时，TiKV 会对业务写操作进行限制。预留空间形式上包括两个部分，`80%` 用作空间不足时清理回收空间等运维操作所需要的额外磁盘空间，剩余 `20%` 为实际的物理文件，用于回收空间过程中额外空间占用过多而耗尽存储的最后一道防御。
+ 该物理文件名为 `space_placeholder_file`，位于 `storage.data-dir` 目录下。TiKV 磁盘空间耗尽无法正常启动需要紧急干预时，可以删除该文件，并且将 `reserve-space` 设置为 `0MB`。
+ 防御的有效性与取值大小有关联，实际预留大小为磁盘容量的 `5%` 与该配置项之间的最大值。
+ 默认值：5GB
+ 单位：MB|GB
