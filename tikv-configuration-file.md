...
> **注意：**
>
> 在小于 v6.5.0 的版本中，PITR 恢复仅支持将备份文件下载到本地的模式做恢复，从 v6.5.0 开始，PITR 功能支持直接读取备份日志文件到内存中做恢复，此配置项主要指定 TiKV 中用于 PITR 恢复的内存使用占比。

+ 用于 PITR 日志恢复功能，表示 PITR 恢复中可用内存与系统总内存的占比。
+ 可调整范围：[0.0, 0.5]
+ 默认值：0.3，表示系统 30% 的内存可用于 PITR 恢复；当为 0.0 时，表示使用下载日志文件到本地的模式做 PITR 恢复。
