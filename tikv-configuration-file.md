...
> **警告：**
>
> - 你**只能**在部署新的 TiKV 集群时将 `enable-ttl` 的值设置为 `true` 或 `false`，不可以在已经存在的 TiKV 集群中修改该配置项的值。由于该配置项为 `true` 的和 `false` 的 TiKV 集群所存储的数据的格式互不相同，如果你在已经存在的 TiKV 集群中修改该配置项的值，会造成不同格式的数据存储在同一个集群，导致重启对应的 TiKV 集群时 TiKV 报 "can't enable ttl on a non-ttl instance" 错误。
> - 你**只能**在 TiKV 集群中使用 `enable-ttl`，不可以在有 TiDB 节点的集群中使用该配置项（即在此类集群中把 `enable-ttl` 设置为 `true`），否则会导致数据损坏、TiDB 集群升级失败等严重后果。
...
+ v5.2.2 前，默认值为 9。自 v5.2.2 起，默认值为 20。
