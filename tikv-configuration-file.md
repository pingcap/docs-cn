...
+ 单次读写请求被强制等待的最大时间。
+ 默认值：500ms
+ 推荐设置：根据实验室测试结果，一般使用默认值即可；如果实例出现了 OOM 或者是剧烈的性能都抖动，可以设置为 1S，使得请求被延迟调节的时间不超过 1 秒。
...
+ 推荐设置：根据实验室测试结果，对 4 核以上的实例，使用默认值 0 即可；对 4 核实例，设置为 1000 到 1500 之间的值能取得比较均衡的效果；对 2 核实例，则不要超过 1200。

#### `foreground-write-bandwidth` <span class="version-mark">从 v6.0.0 版本开始引入</span>

+ 限制前台事务写入的带宽，这是一个软限制。
+ 默认值：0KB（即无限制）
+ 推荐设置：根据实验室测试结果，除非因为 `foreground-cpu-time` 设置不足以对写带宽做限制的时候，一般不用使用默认值 0 即可；否则，在 4 核及 4 核以下规格实例上，建议设置成 < 50M。
...
+ 默认值：0KB（即无限制）
+ 推荐设置：根据实验室测试结果，除非因为 `foreground-cpu-time` 设置不足以对读带宽做限制的时候，一般不用使用默认值 0 即可；否则，在 4 核及 4 核以下规格实例上，建议设置成 < 20M。
