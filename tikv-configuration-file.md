...
+ 存储慢日志的文件。
+ 如果未设置本项但设置了 `log.file.filename`，慢日志将输出至 `log.file.filename` 指定的日志文件中。如果本项和 `log.file.filename` 均未设置，所有日志默认输出到 `"stderr"`。如果同时设置了两项，慢日志则输出至各自指定的日志文件中。
+ 默认值：""
...
日志相关的配置项。 原 log 参数：log-level/log-format/log-file/log-rotation-size/log-rotation-timespan 废弃，变更为与 TiDB 保持一致，如果旧参数为非默认值，则保持兼容；如果旧参数和新参数同时配置，使用新参数。
...
+ 可保留的 log 文件的最大数量。如果未设置该参数，TiKV 默认保存所有的 log 文件；如果该参数值设置为 `7`，TiKV 最多会保留 7 个旧的日志文件。
+ 默认值：0
