...
+ 只有当时 [`format-version`](#format-version-从-v620-版本开始引入) >= 5 时，该配置项才生效。
+ 默认值：`false`
...
+ 可选值：
    - `0`：适用于所有 TiKV 版本。默认 checksum 类型为 CRC32，这个版本不支持修改 checksum 类型。
    ...
    - `5`：适用于 TiKV v6.1 及以上版本。全量和分区 filter 采用一种更快、更准确的 Bloom filter 实现，其结构不同。
+ 默认值：`2`
