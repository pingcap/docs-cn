...
+ 指定 PD 客户端在观察到错误时跳过报告错误的频率。例如，当配置项值为 `5` 时，PD 观察到错误后，将在每 4 次错误时跳过报告，直到每第 5 次错误时才报告。
+ 要禁用此功能，请将值设置为 `1`。
+ 默认值：`10`
...
+ 初始化 PD 连接的最大重试次数
+ 要禁用重试，将该值设置为 `0`。要解除重试次数的限制，将该值设置为 `-1`。
+ 默认值：`-1`
...
+ 默认值：按照每个日志 1MB 而计算出来的 3/4 Region 大小所能容纳的日志个数
+ 最小值：0
