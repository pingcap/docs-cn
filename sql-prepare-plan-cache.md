...
通过执行 `ADMIN FLUSH [SESSION | INSTANCE] PLAN_CACHE` 语句，你可以手动清空计划缓存。语句中的作用域用于指定需要清空的缓存级别，可以为 `SESSION` 或 `INSTANCE`，默认为 `SESSION`。
...
为了兼容这样的使用方式，从 v6.0 起，TiDB 支持 `tidb_ignore_prepared_cache_close_stmt` 变量。打开该变量后，TiDB 会忽略关闭 Preapre Statement 的信号，解决上述问题，如：
