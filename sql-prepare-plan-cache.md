...
在当前的 TiDB 开发版本中，默认会缓存预处理语句（Prepared Statements）的执行计划。其中包括两种预处理请求形式：

- 使用 `COM_STMT_PREPARE` 和 `COM_STMT_EXECUTE` 的协议功能；
- 执行 `Prepare` / `Execute` SQL 语句查询；

TiDB 优化器对这两种请求形式的处理是一致的：`Prepare` 时将参数化的 SQL 查询解析成 AST（抽象语法树），每次 `Execute` 时根据保存的 AST 和具体的参数值生成执行计划。
...
目前执行计划缓存功能默认的最大语句条数为 100，可以通过修改 TiDB 配置文件中的 [`prepare-plan-cache` 项](/tidb-configuration-file.md#prepared-plan-cache) 来配置此功能。
