...
- 升级或重启 TiDB 实例时，`Prepare` 信息会丢失，如果一开始执行了 `prepare stmt ...`，后面只执行 `execute stmt ...`；这样当不停机升级 TiDB 集群时，执行 `execute stmt ...` 可能遇到 `Prepared Statement not found` 的错误，需要再次执行一次 `prepare stmt ...`。
