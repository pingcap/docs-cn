...
在当前的 TiDB 开发版本中，默认预处理语句（Prepared Statements）的执行计划缓存。其中包括两种预处理请求形式：
...
目前执行计划缓存功能默认的最大语句条数为 100，可以通过修改配置文件中的 [`prepare-plan-cache` 项](/tidb-configuration-file.md#prepared-plan-cache) 来配置这项功能。
