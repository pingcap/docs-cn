...
- `?` 直接在 `Order By` 后的查询，如 `Order By ?`，此时 `?` 表示根据 `Order By` 后第几列排序，对查询影响较大，故不缓存；如果是普通表达式，如 `Order By a+?` 则可以缓存；
- `?` 直接在 `Group by` 后的查询，如 `Group By ?`，此时 `?` 表示根据 `Group By` 后第几列聚合，对查询影响较大，故不缓存；如果是普通表达式，如 `Group By a+?` 则可以缓存；
