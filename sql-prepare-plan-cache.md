...
- 不管计划是否已经被缓存，都会收到 SQL Binding 的影响。对于没有被缓存的计划，即在第一次执行 `Execute` 时，会受到已有 SQL Binding 的影响；而对于已经缓存的计划，如果有新的 SQL Binding 被创建产生，则原有已经被缓存的计划会失效。
- 对于已经被缓存的计划不会受到统计信息更新、优化规则和表达式下推黑名单更新的影响，仍然会使用已经保存在缓存中的计划。
...
在开启了执行计划缓存功能后，可以通过执行语句 `admin flush [session|instance] plan_cache` 来清空当前 Session/ Instance 上对应的全部缓存。其中需要注意的是：

1. 在不指定作用域的情况下，默认使用 Session 级别。

2. 如果错误的使用了 global 级别的语句，将会产生报错 `Do not support the 'admin flush global scope.'`.

3. 如果指定了在 Instance 级别，在当前执行语句的 Session 上的缓存是立即清空的，而对于其他 Session 则是延迟清空的，即只有在其他 Session 上执行了 `Execute` 命令才会真正清空该 Session 上的全部缓存。
...
-- 刷新当前 Session 上的所有缓存
mysql> admin flush session plan_cache;
Query OK, 0 rows affected (0.00 sec)
...
-- 刷新当前 instance 级别上的缓存
mysql> admin flush instance plan_cache;
Query OK, 0 rows affected (0.00 sec)
...
-- 不支持 Global 级别的缓存刷新
mysql> admin flush global plan_cache;
