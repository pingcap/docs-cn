...
- 如果需要直接备份 SST 文件（键值对），或者对延迟不敏感的增量备份，请使用备份工具 [BR](/br/br-overview.md)。
...
- `-o` 用于选择存储导出文件的目录，支持本地文件路径或[外部存储 URL](/br/misc/external-storage.md) 格式。
...
| -o 或 --output | 导出本地文件路径或[外部存储 URL](/br/misc/external-storage.md) | "./export-${time}" |
