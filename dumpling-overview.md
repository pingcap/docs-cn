...
| --csv-output-dialect | Dumpling 默认将 CSV 文件中的 Binary 数据类型以 UTF-8 格式表示。若要以其他格式（如 HEX）导出 Binary 数据，请将此参数设置为 'redshift'。这样，Binary 数据将以十六进制数的形式表示，但会丢失十六进制数的前缀 '0x'，如 '0x61' 将被表示成 '61'。 | '"' |
