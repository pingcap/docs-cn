...
- `-o` （或 `--output`）用于选择存储导出文件的目录，支持本地文件路径或[外部存储 - URL 格式](/br/backup-and-restore-storages.md#url-格式.md) 格式。
...
Dumpling 同时还支持从 `~/.aws/credentials` 读取凭证文件。Dumpling 导出到 Amazon S3 的配置参数与 BR 大致相同，更多参数描述，请参考[备份存储 - URL 格式](/br/backup-and-restore-storages.md)。
...
| -o 或 --output | 导出本地文件路径或[外部存储 - URL 格式](/br/backup-and-restore-storages.md#url-格式.md) | "./export-${time}" |
