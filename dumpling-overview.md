...
| --consistency | flush: dump 前用 FTWRL <br/> snapshot: 通过指定 TSO 来 dump 某个快照时候的 tidb 数据 <br/> lock: 对需要 dump 的所有表执行 `lock tables read` 命令 <br/> none: 不加锁 dump，无法保证一致性 <br/> auto: MySQL 默认用 flush, TiDB 默认用 snapshot | "auto" |
