...
| --csv-output-dialect | 表示可以将源数据导出成数据库所需的格式存储到 CSV，取值可为 `"snowflake"`、`"redshift"`、`"bigquery"`。如果设置为 `"snowflake"` 或 `"redshift"`，会把 Binary 数据类型转换成十六进制，但会丢失十六进制数的前缀 `0x`，例如 `0x61` 将被表示成 `61`。如果设置为 `"bigquery"`，会使用 base64 对 Binary 数据类型进行编码。| '"' |
