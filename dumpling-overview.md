...
- `flush`：使用 [`FLUSH TABLES WITH READ LOCK`](https://dev.mysql.com/doc/refman/8.0/en/flush.html#flush-tables-with-read-lock) 短暂地中断备份库的 DML 和 DDL 操作，保证并发备份连接的全局一致性和记录 pos 信息，所有的并发备份连接启动事务后释放该锁。推荐在业务低峰或者 MySQL 备库上进行全量备份。
