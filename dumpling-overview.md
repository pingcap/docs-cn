...
> **注意：**
>
> {{{ .company }}} 之前维护的 Mydumper 工具 fork 自 [mydumper project](https://github.com/maxbube/mydumper)，针对 TiDB 的特性进行了优化。从 v7.5.0 开始，[Mydumper](https://docs-archive.pingcap.com/zh/tidb/v4.0/mydumper-overview) 废弃，其绝大部分功能已经被 [Dumpling](/dumpling-overview.md) 取代，强烈建议切换到 Dumpling。
...
Dumpling 同时还支持从 `~/.aws/credentials` 读取凭证，并使用 AWS profile 进行认证。具体参数说明，请参考[外部存储服务的 URI 格式](/external-storage-uri.md)。

使用 `~/.aws/credentials` 中默认 AWS profile 的凭证：

```shell
tiup dumpling -u root -P 4000 -h 127.0.0.1 -r 200000 -o "s3://${Bucket}/${Folder}"
```

使用指定 AWS profile 的凭证：
