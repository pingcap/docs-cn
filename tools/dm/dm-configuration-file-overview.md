...
- `inventory.ini`：使用 DM-Ansible 部署 DM 集群的配置文件。需要根据所选用的集群拓扑来进行编辑。详见[编辑 `inventory.ini` 配置文件](/tools/dm/deployment.md#第-7-步-编辑-inventory-ini-配置文件)。
...
使用 DM-Ansible 部署 DM 集群时，`<path-to-dm-ansible>/conf` 中提供了任务配置文件模板：`task.yaml.exmaple` 文件。该文件是 DM 同步任务配置的标准文件，每一个具体的任务对应一个 `task.yaml` 文件。关于该配置文件的详细介绍，参见 [任务配置文件](/tools/dm/task-configuration-file-intro.md)。
...
2. 参考[任务配置文件](/tools/dm/task-configuration-file-intro.md)来修改 `your_task.yaml` 文件。
3. [使用 dmctl 创建数据同步任务](/tools/dm/manage-task.md#创建数据同步任务)。
