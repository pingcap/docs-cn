...
`br` v5.1.0 开始，快照备份时默认自动备份 **mysql schema 下的系统表数据**，但不默认恢复系统表数据。`br` v6.2.0 开始，增加恢复参数 `--with-sys-table` 支持恢复数据的同时恢复**部分系统表相关数据**。 `br` v7.6.0 开始，恢复参数 `--with-sys-table`  默认打开，默认支持恢复数据的同时恢复**部分系统表相关数据**。
...
- `br` v7.6.0 版本之前无法恢复 `user` 为 `cloud_admin` 并且 `host` 为 `'%'` 的用户数据，该用户是 TiDB Cloud 预留用户。`br` v7.6.0 版本开始，默认支持包括  `cloud_admin` 在内的所有用户恢复。
