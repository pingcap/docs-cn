...
> - 从 BR v8.3.0 开始，快照恢复增加了对 TiKV 和 TiFlash 的可用磁盘空间检查，包括在开始阶段会根据需要恢复的 SST 文件大小检查 TiKV 和 TiFlash 是否有足够的磁盘空间，在 TiKV（v8.3.0 或更高的版本）下载每个 SST 文件前会检查 TiKV 是否有足够的磁盘空间。如果检测到空间不足，恢复任务会报错失败。你可以通过设置 `--check-requirements=false` 来选择跳过恢复任务开始阶段的检查，但 TiKV 在下载 SST 文件前的磁盘空间检查无法跳过。
