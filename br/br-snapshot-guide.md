...
> **注意：**
>
> - 启用粗粒度打散 Region 的方式在 v7.6.0 版本是实验性功能，推荐在 1K 数量的表的集群中恢复。
> - 在 TiDB v7.6.0 版本中，通过设置 `--granularity="coarse-grained"` 参数启用粗粒度打散 Region 的方式加速恢复是实验性功能，还未支持断点恢复，同时建议在有低于 1,000 张表的集群中启用该参数进行数据恢复加速。
