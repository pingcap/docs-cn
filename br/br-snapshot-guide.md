...
`br` v5.1.0 开始，快照备份时默认自动备份 **mysql schema 下的系统表数据**，但不默认恢复系统表数据。`br` v6.2.0 开始，增加恢复参数 `--with-sys-table` 支持恢复数据的同时恢复**部分系统表相关数据**。`br` v7.6.0 开始，恢复参数 `--with-sys-table` 默认开启，即默认支持恢复数据的同时恢复**部分系统表相关数据**。
