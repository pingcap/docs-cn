...
### 关于该参数 `--granularity="coarse-grained" 的最佳实践

* 只有当恢复到全新的空集群的时候，开启该参数，才能最大限度的提升恢复性能。否则，在恢复数据的 Table ID 被集群中其他表占用的情况下，会导致恢复过程中触发重写操作，整体恢复过程便从 IO 密集应用变成了 CPU 密集应用，整体性能便会收到 CPU 资源使用率的限制。

* 建议将 TiKV 的配置参数 [import] num-threads 设置为该 TiKV 节点的 CPU 核心数 - 1，避免恢复模块消耗过多 CPU 资源，从而抢占了其他模块的资源使用率，从而提升整体性能。

* 当遇到集群中 TiKV 下载 SST 文件带宽长时间打满的情况后（通过监控 Backup & Import -> Import Download SST Throughput 确认），可能会造成任务堆积，继而会导致 BR 节点消耗更多的内存。为了避免该问题，建议调整 `--ratelimit` 到 2/3 的磁盘带宽大小。
