...
为进一步提升大集群的恢复速度，BR 从 v7.6.0 开始支持通过设置 `--granularity="coarse-grained"` 参数启用粗粒度打散 Region 算法（实验特性）进行更快的并行恢复。通过优化 Region 打散算法，BR 可以迅速将恢复任务拆分为大量小任务，并批量分散到所有 TiKV 节点上。粗粒度打散 Region 算法充分利用每个 TiKV 节点的所有资源，实现了并行快速恢复。
