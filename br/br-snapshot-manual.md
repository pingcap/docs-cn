...
2. **表级别校验和**：对表数据内容进行完整性校验，验证数据的业务逻辑一致性。这一校验默认关闭，可以通过参数开启。
...
从 v9.5.0 版本开始，BR 工具在执行恢复操作时默认不进行表级别校验和检查（`--checksum=false`），以提升恢复性能。如果需要进行表级别校验和检查，可以显式指定 `--checksum=true`。文件级别的校验和检查始终进行，确保恢复数据的基本完整性。
