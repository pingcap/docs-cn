...
- [TiDB 快照备份与恢复命令行手册](#tidb-快照备份与恢复命令行手册)
  - [备份集群快照](#备份集群快照)
...
- `--with-sys-table`: 恢复集群数据的同时恢复 **部分系统表** 的数据，包括恢复账号权限数据和 SQL Binding 信息，但暂不支持恢复统计信息 (`mysql.stat_*`) 和系统参数 (`mysql.tidb, mysql.global_variables`) 等信息，更多详细信息见[恢复 mysql 数据库下的表](/br/br-snapshot-guide.md#恢复-mysql-数据库下的表)。
