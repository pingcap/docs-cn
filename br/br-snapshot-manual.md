...
从 TiDB v7.5.0 开始，br 命令行工具支持备份和恢复数据库的统计信息，包括列、索引和表级别的统计信息。通过支持该功能，从备份中恢复的 TiDB 数据库不再需要手动运行统计信息收集任务或等待自动收集任务的完成，简化了数据库维护并提升了查询性能。

br 命令行工具中新引入参数 `--ignore-stats` 用于控制备份数据时是否备份表统计信息。该参数默认开启，配置项为 `--ignore-stats=true`，也就是说，在备份数据时不备份表统计信息。
...
在恢复数据时，如果备份集中包含了表统计信息，br 命令行工具默认在恢复数据时会自动恢复表统计信息：
