...
从 TiDB v7.5.0 开始，br 命令行工具引入参数 `--ignore-stats`，可以支持备份和恢复数据库的列、索引、和表级别的统计信息。启用该功能后，从备份中恢复的 TiDB 数据库不再需要手动运行统计信息收集任务，也无需等待自动收集任务的完成，从而简化了数据库维护工作，并提升了查询性能。

该参数默认开启，配置项为 `--ignore-stats=true`，即在备份数据时不备份表统计信息。
...
完成上述配置后，在恢复数据时，如果备份集中包含了表统计信息，br 命令行工具在恢复数据时会自动恢复表统计信息：
