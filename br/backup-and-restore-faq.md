...
## 在 TiDB v5.4.0 及后续版本中，当在有负载的集群进行备份时候，备份速度为什么会变得很慢？

从 TiDB v5.4.0 起，TiKV 的备份新增了一项“自动调节”功能，此功能会默认开启。当集群负载较高时，该功能会自动限制备份使用的资源，从而减少备份对在线集群的性能造成的影响。如需了解关于“自动调节”功能的更多信息，请参见[自动调节](/br/br-features.md#自动调节-从-v54-版本开始引入)。

TiKV 支持[动态配置](/tikv-control.md#动态修改-tikv-的配置)自动调节功能，无需重启集群。以下为该功能的使用方法：
...
如需了解通过 `tikv-ctl` 在线修改自动调节功能的命令行，请参阅[自动调节功能的使用方法](/br/br-features.md#使用方法)。

另外，该功能也降低了处理备份的默认的工作线程数量（详见 [`backup.num-threads`](/tikv-configuration-file.md#num-threads-1)）。因此，你通过 Grafana 监控面板看到的备份速度、备份时的 CPU 使用率、备份时的 I/O 资源利用率都会小于 v5.4 之前的版本。在 v5.4 之前的版本中，`backup.num-threads` 的默认值为 CPU * 0.75，即处理备份的工作线程数量占 75% 的逻辑 CPU，最大值为 `32`；在 v5.4 及之后的版本中，该配置项的默认值为 CPU * 0.5，最大值为 `8`。

在离线备份场景中，你可以通过将 `backup.num-threads` 修改为更大的数字的方式来加速备份。这项配置同样可以通过  `tikv-ctl` 进行在线修改。
...
- 对于 TiDB v5.4.0 及以后的版本：
    ...
    BR 调低了默认的备份 CPU 使用率，并且引入了在集群高负载的场景下能自动自动限制备份使用的资源的 “自动调节”功能。因此，使用默认配置进行备份时，备份对有负载集群的影响会显著小于 v5.4.0 之前的版本。

    在内部一个单节点的测试中，5.3.0 的默认配置下全速备份，会使纯写 workload 的 QPS 降低 75%；而使用 5.4.0 的默认配置，只会使同样 workload 的 QPS 降低 25%。
不过相应地，5.4.0 的默认配置的耗时是 5.3.0 的 1.7 倍。

对于 5.3.0 及以前的版本：

**BR 默认的配置参数是为离线备份服务的。**

在默认配置下，BR 可能会占用集群大量的 CPU 和 I/O 资源，导致集群的延迟上升。

内部测试中，使用一个 6 TiKV 的集群、一个单表的数据集，进行以下混合操作，读写比例约为 `10:1`：

1. 单点插入；
2. 单点更新；
3. 小范围的范围查询。

基准的总 QPS 在 20K 左右，基准的 P95 延迟在 28ms 左右，P99 在 50ms 左右。

该测试可以确定的影响有以下结论：

- 通过限流将集群的总 CPU 利用率控制在 91% 时，**写 workload** 的 QPS 下降了 50%，P99 延迟上升数倍；
...
2. 调节 TiKV 的 `backup.num-threads` 参数限制资源使用。这个参数决定了执行备份任务的线程池大小。过往的测试数据表明，在 BR 的线程数大小不大于 `8`，且集群总 CPU 利用率不超过 60% 的时候，BR 对集群（无论读写负载）几乎没有什么影响。

> **提示：**
>
> *为什么限制备份线程池大小可以限制备份的影响？*
>
> 现在的备份过程中，涉及到大量的 SST 解码、编码及压缩、解压，这个过程需要消耗大量 CPU 资源。
> 以往的测试证明，在使用阻塞的 I/O 模型的前提下，单线程扫描和在内存中生成备份文件的过程可以使用 100% CPU。
> 这暗示不计上传过程的话，备份事实上是 CPU 密集的。

两种方案都可以减少 BR 对集群的影响，同时也会降低备份的速度。
