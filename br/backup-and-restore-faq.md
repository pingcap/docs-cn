...
## 5.4.0 之后在有负载的集群备份的时候，速度变得很慢，为什么？

在 5.4.0 以后，TiKV 的备份新增了一项“自动调节”功能，默认会开启。该功能会在集群负载较高的时候自动限制 Backup 使用的资源量，来减少对在线集群业务的影响。

如果想要停用掉这项功能，可以修改配置项 `backup.enable-auto-tune` 为 `false`；这项配置可以使用 `tikv-ctl` 在线修改，无需重启集群。

另外，5.4.0 之后也降低了备份默认使用的线程的数量，这可能导致您看到的备份速度和备份时候的 CPU 及 I/O 利用率都小于之前版本。
在离线备份场景，可以通过修改 `backup.num-threads` 到更大的数字来加速备份，这项配置同样可以在线修改。

> **提示：**
> 
> 作为参考，这项配置在 5.3.0 及之前默认是 75% 的逻辑 CPU 数量，最大为 32；在 5.4.0 及之后默认是 50% 的逻辑 CPU 数量，最大为 8。
...
BR 调低了默认的备份 CPU 使用率，并且引入了在高负载的时候自动限流 BR 的功能，因此使用默认配置进行备份的影响会显著小于之前版本。
