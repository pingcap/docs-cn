...
恢复数据时，系统表不会冲突。由于恢复 `mysql.*` 中系统表数据的技术实现尚不完善，因此 BR 默认**不恢复**系统库 `mysql` 中的表数据，也就不会产生任何冲突。详情参阅[备份和恢复 `mysql` 系统库下的表数据（实验特性）](/br/br-usage-restore.md#恢复创建在-mysql-数据库下的表实验性功能)。
