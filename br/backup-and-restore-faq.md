...
在 5.4.0 以后，TiKV 的备份新增了一项“自动调节”功能：该功能会在集群负载较高的时候自动限制 Backup 使用的资源量，以求减少对在线集群的影响。
...
内部测试中，使用一个 6 TiKV 的集群，使用一个单表的数据集，进行以下混合操作，读写比例约为 `10:1`：
...
通过限流将集群的总 CPU 利用率控制在 91% 的时候，**写 workload** 的 QPS 下降了 50%，P99 延迟上升数倍。
