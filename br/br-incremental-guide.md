...
由于增量备份的恢复过程中，依赖于备份时间点的库表快照进行增量 DDL 的过滤，因此对于增量备份过程中删除的表，在恢复后可能仍然存在，需要手动删除。
增量备份尚不支持批量重命名表这一类型的 DDL，如果选定的增量备份范围内发生了批量重命名表的操作，那么有几率造成恢复失败。建议在批量重命名后进行一次全量备份，并在恢复中用最新的全量备份替代增量数据。
