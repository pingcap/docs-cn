...
在 8.3 版本后，新增配置参数 `--allow-pitr-from-incremental`（默认为 true) 来控制增量备份和后续的日志备份是否兼容。兼容的情况下，增量恢复开始前会对需要回放的 DDL 进行严格检查，该模式目前尚不支持 3 种 DDL，分别是 ADD INDEX/MODIFY COLUMN/REORG PARTITION，因此要想搭配使用增量备份和日志备份，需确保增量备份的过程中没有上述类型的 DDL。如果整个恢复流程中只需要增量恢复而不考虑日志备份，则可以通过设置 `--allow-pitr-from-incremental=false` 来跳过增量恢复阶段的检查。
