...
> **注意：**
>
> 已知问题：从 v7.2.0 版本开始，新建集群的部分系统表字段改为大小写敏感，但 v7.2.0 之前版本，或者是 v7.2.0 版本以下的集群**在线升级**到 v7.2.0 及以上版本，对应的系统表字段大小写不敏感。 带系统表的备份和恢复如果发生在2类集群之间，就会失败。详见 [Issue #43717](https://github.com/pingcap/tidb/issues/43717)。

下表列出了全量备份的兼容性矩阵，表格中所有数据均来自新建集群：
...
下表列出了日志备份的兼容性矩阵，表格中所有数据均来自新建集群：
