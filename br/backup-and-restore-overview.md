...
> **注意：**
>
> 在执行备份和恢复操作时，建议使用相同版本的 BR 和 TiDB 集群。

在执行备份和恢复操作前，BR 会比较自身版本与 TiDB 集群版本。如果版本之间不兼容，BR 会提示报错并退出。如要忽略版本检查，可以通过设置 `--check-requirements=false` 强行跳过版本检查。需要注意的是，跳过检查可能会遇到数据不兼容的问题。

| 备份版本（纵向）\ 恢复版本（横向）  | 恢复到 TiDB v6.0 | 恢复到 TiDB v6.1| 恢复到 TiDB v6.2 | 恢复到 TiDB v6.3,v6.4,v6.5 | 恢复到 TiDB v6.6 |
|  ----  |  ----  | ---- | ---- | ---- | ---- |
| TiDB v6.0、v6.1、v6.2、v6.3 或 v6.4 快照备份 | 兼容（已知问题，如果备份数据中包含空库可能导致报错，参考 [#36379](https://github.com/pingcap/tidb/issues/36379)） | 兼容 | 兼容 | 兼容 | 兼容（需使用 v6.6 及以上版本的 BR） |
| TiDB v6.3、v6.4 或 v6.5 日志备份| 不兼容 | 不兼容 | 不兼容 | 兼容 | 兼容 |
| TiDB v6.6 日志备份| 不兼容 | 不兼容 | 不兼容 | 兼容 | 兼容 |
