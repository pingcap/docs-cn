...
>
> 从 v8.5.5 版本开始，如果就集群版本的系统表数据并没有大小写冲突（例如同时存在 test.t1 和 test.T1 的记录），在恢复时可以指定 BR 参数 --sys-check-collation，BR 会尝试检查数据在不同 collation 下的兼容性，如果兼容，则能成功恢复来自旧版本的备份，否则，BR 将报错。
