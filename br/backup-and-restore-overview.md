...
> 
在执行备份和恢复操作之前，BR（Backup & Restore）工具会检查自己的版本与 TiDB 集群的版本是否兼容。如果它们之间不兼容，BR 工具将会报错并停止执行。如果你想跳过这个版本检查，可以设置 `--check-requirements=false`。但是需要注意，跳过检查可能会导致恢复的数据不兼容的问题。

从 TiDB v6.7.0 版本开始，我们逐步支持通过 SQL 语句来执行备份和恢复操作。因此，我们强烈建议您在备份和恢复集群时使用与 TiDB 集群相同大版本的 BR 工具，并避免跨大版本进行数据备份和恢复操作。这将有助于确保恢复操作的顺利执行和数据的一致性。


TiDB v6.6.0 版本之前的 BR 版本兼容性矩阵将保持不变：
