...
> - 自 v6.2.0 起，BR 恢复数据前会检查集群是否为空，以及目标集群的系统表是否跟备份数据中的系统表兼容。如不符合条件，BR 会提示类似如下信息，此时可参考提示信息，通过显式指定 `filter` 的方式跳过恢复系统表:
```
#######################################################################
...
从 v6.2.0 开始，BR 在恢复全量数据到新集群时会自动恢复权限相关系统表，推荐通过该方式恢复权限系统表，详情请查阅[恢复快照备份数据](#恢复快照备份数据)。

如需恢复权限表以外的其他系统表，可参考以下方式。
