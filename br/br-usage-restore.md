...
> **注意：**
>
> 自 BR v5.1.0 开始，全量备份会备份**系统表数据**。BR v6.2.0 以前的版本，在默认设置下恢复只会恢复用户数据，而不会恢复系统表数据。自 BR v6.2.0 开始，在默认设置下（即没有 `filter` 等参数影响数据恢复范围），恢复数据时 BR 将同时恢复**系统权限相关数据**。该功能也带来了一些兼容性改变，同样在默认配置下，`br restore full` 不再支持恢复数据到已有数据的集群，否则需要过滤掉相关的系统表才能恢复，参考命令 `br restore full --filter '.' --filter '!mysql.*'`
