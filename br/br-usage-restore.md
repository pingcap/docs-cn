...
BR 支持在一个空集群上执行快照备份恢复，将该集群恢复到快照备份时刻点的集群最新状态。在默认设置下（无 `filter` 等影响数据恢复范围的参数），如果备份数据中包含系统表数据（自 BR 5.1.0 版本开始，全量备份会备份系统表数据），恢复时同时恢复系统权限相关数据，包括如下表:
...
> **注意：**
>
> 之前版本在默认设置下只会恢复用户数据，而不会恢复系统表数据。另外新版本会检查集群是否为空，以及新集群系统表是否跟备份数据中的系统表兼容，如不符合条件，BR 会提示类似如下信息，此时可参考提示信息，通过显示指定 `filter` 的方式恢复旧版行为:
```
#######################################################################
...
## 恢复创建在 `mysql` 数据库下的表
从当前版本开始，在进行全量数据恢复到新集群时会自动恢复权限相关系统表，建议通过该方式恢复权限系统表，参考[恢复快照备份数据](#恢复快照备份数据)，恢复除权限表外的系统表参考如下方式：
