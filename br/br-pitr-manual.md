...
- `--pitr-batch-count`：日志恢复单批次包含的最大文件数。一旦达到该数量阈值，就立即结束当前批次发起恢复。
- `--pitr-batch-size`：日志恢复单批次包含的最大文件大小。一旦达到该数量阈值，就立即结束当前批次发起恢复。
- `--pitr-concurrency`：日志恢复过程中的并发数量，一个并发对应一个批次的数据恢复。
