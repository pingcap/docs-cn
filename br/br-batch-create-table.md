...
- v6.0.0 前的串行建表方案：
    ...
    在使用 BR 执行数据恢复任务时，BR 会先在目标 TiDB 集群创建库和表后，再开始进行数据恢复。建表时，BR 会调用 TiDB 内部 API 后创建表，其运作方式类似 BR 在执行 SQL `Create Table` 语句。建表任务由 TiDB DDL owner 依次串行执行。在每张表被创建时，各会引起一次 DDL schema 版本的变更，而每次的 schema 版本的变更都需要同步到其他 TiDB DDL worker（含 BR）。因此，当需要创建的表的数量比较多时，串行建表方案会导致建表时间过长。
