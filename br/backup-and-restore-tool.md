...
BR 除了可以用来进行常规的备份恢复外，也可以在保证兼容性前提下用来做大规模的数据迁移。
...
备份恢复的详细设计可以参考[备份与恢复工具 BR 的设计](/br/backup-and-restore-design.md)。
...
- 集群快照备份：TiDB 集群快照数据只包含某个物理时间点上集群的最新的，满足事务一致性的所有数据。TiDB 支持备份集群快照数据，使用请参考[备份 TiDB 集群快照](/br/br-backup-usage.md)；
- 集群增量备份：TiDB 集群增量数据包含在某个时间段的起始和结束两个物理时间点对应的快照的变化差异的数据。 增量数据相对比全量数据而言数据量更小，适合配合快照备份一起使用，减少备份的数据量。使用请参考[备份 TiDB 集群增量数据](/br/br-backup-usage.md)；
- 只备份指定库表：TiDB 支持在快照备份和增量数据备份的基础上，过滤掉不需要的备份数据，帮助用户实现只备份关键业务的数据。使用请参考[备份 TiDB 集群的指定库表的数据](/br/br-backup-usage.md)；
- 备份数据加密： TiDB 备份功能支持在备份端，或备份到 Amazon S3 的时候在存储服务端，进行备份数据加密，用户可以根据自己情况选择其中一种使用。使用请参考[备份数据加密](/br/br-backup-usage.md)；
- 备份性能和影响： TiDB 备份期间对集群的影响可以保持在 20% 以下，通过合理的配置 TiDB 集群用于备份资源，影响可以降低到 10% 及更低；单 TiKV 存储节点的备份速度可以达到 50 MB/s ～ 100 MB/s，备份速度具有可扩展性；
- 备份存储类型支持：TiDB 备份功能支持将数据备份到 Amazon S3/Google Cloud Storage/Azure Blob Storage/NFS，或者实现 s3 协议的其他文件存储服务。使用请参考 [备份数据到远端存储](/br/br-backup-usage.md)。

### 从备份数据恢复 TiDB 集群

- 恢复集群快照备份：TiDB 恢复功能支持在一个空集群上执行快照备份的恢复，将该集群恢复到快照备份时刻点的集群最新状态。使用请参考[恢复集群快照备份](/br/br-usage-restore.md)；
- 恢复集群增量备份：TiDB 恢复功能支持恢复某个时间的增量备份数据。使用请参考[备份 TiDB 集群增量数据](/br/br-usage-restore.md)；
- 恢复备份中的指定库表：BR 支持只恢复备份数据中指定库/表的局部数据。该功能在恢复过程中过滤掉不需要的数据，可以用于往已经存在的 TiDB 集群上恢复指定库/表的数据。使用请参考[恢复备份数据中指定库表的数据](/br/br-usage-restore.md)；
- 恢复性能和影响： TiDB恢复功能只支持恢复数据到新集群，会尽可能多的使用恢复集群的资源；恢复速度约为 单 TiKV 存储节点 100 MB/s，恢复速度具有可扩展性。
...
> **注意：**
>
> - 如果没有挂载 NFS 到 BR/TiKV 节点或者使用支持 S3/GCS/Azure Blob Storage 协议的远端存储存储，那么 BR 备份的数据会生成在各个 TiKV 节点上。 由于 BR 只备份 leader 副本，所以各个节点预留的空间需要根据 leader size 来预估；
...
在使用 BR 之前先说明 BR 使用上存在限制。

#### 不支持的场景

以下是 BR 不支持的场景：

- BR 恢复数据到运行 TiCDC / TiDB Binlog 的上游集群时，恢复数据无法由 TiCDC / TiDB Binlog 同步到下游。
...
- 不推荐向正在提供服务的在线集群执行恢复，恢复期间对在线业务影响很大，建议低峰期或者限速 (`rate-limit`) 执行恢复；
- 不推荐 BR 备份和恢复任务并行进行。不同的任务并行不仅会导致备份或恢复的性能降低，影响在线业务；也会因为缺少不同任务协调机制对集群的状态产生影响，甚至造成任务失败；
