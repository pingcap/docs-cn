...
| **BR v5.0 / TiDB v5.0** | ✅ | ✅ | ✅
| **BR v4.0 / TiDB v4.0** | ❌（同右边） | ❌（如果恢复使用了非 int 类型聚簇主键的表到 4.0 TiDB 集群，BR 会在无任何警告的前提下导致数据错误） | ✅（如果 TiKV >= v4.0.0-rc.1，BR 包含 [#233](https://github.com/pingcap/br/pull/233) Bug 修复，且 TiKV 不包含 [#7241](https://github.com/tikv/tikv/pull/7241) Bug 修复，那么 BR 会导致 TiKV 节点重启) |
