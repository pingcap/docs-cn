...
自 v5.1.0 起，BR 默认会**备份**全部数据，包括系统库 (`mysql.*`)，但为了兼容之前 BR 的版本，**恢复**的时候默认**不**恢复系统表，只有设置了 [`filter` 参数](https://docs.pingcap.com/zh/tidb/stable/use-br-command-line-tool#使用表库过滤功能备份多张表的数据)才会把系统表恢复到临时库中，然后通过 rename 的方式恢复到系统库。
