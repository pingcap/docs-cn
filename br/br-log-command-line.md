...
当使用 `br log stop` 命令停止日志备份任务后，可在另一个 `--storage` 路径下重新创建一个新的日志备份任务，也可以在原来的 `--storage` 路径下重新启动日志备份任务。如果是续接上一次的任务，此时需要注意：

- 使用 `br log start` 命令来重新启动备份任务，配置的 `--storage` 参数需与停止任务之前的参数相同；
