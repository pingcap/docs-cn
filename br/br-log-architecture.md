...
- `{resolved_ts}-{uuid}.meta` 文件：每个 TiKV 节点每次上传日志备份数据时会生成一个该文件，保存本次上传的所有日志备份数据文件。其中 `{resolved_ts}` 是本节点的日志备份的 Resolved Timestamp，所有 TiKV 节点的最小的 Resolved Timestamp 就是日志备份任务最新的 `resolved_ts`；`{uuid}` 是生成该文件的时候随机生成的。
