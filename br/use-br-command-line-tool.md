...
* `--concurrency`：控制备份阶段如何将任务拆分为多个请求，并以指定的并发数（concurrency）发送到同一个 TiKV 节点。该参数主要影响任务的拆分粒度，而不再直接决定备份的吞吐性能。通常无需修改默认值，如需提升备份性能，建议通过调整 tikv.backup.num-threads 参数来优化。
