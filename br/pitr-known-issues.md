...
该场景发生在通过日志数据来恢复全量导入的集群初始化数据，经过测试发现，当存在长时间(24h)大量热点写入，且平均单台 TiKV 节点写入 ops > 50k/s(可以通过 grafana 中 `TiKV-Details` -> `Backup Log` -> `Handle Event Rate` 确认该数值)，那么有几率会遇到这个情况。
