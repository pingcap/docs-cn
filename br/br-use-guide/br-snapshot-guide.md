...
本教程介绍快照备份和恢复功能的使用。快照备份是集群全量备份的一种实现。它基于 TiDB 的 [MVCC](/tidb-storage#mvcc) 实现，将指定快照包含的所有数据备份到目标存储中。备份下来的数据大小约等于集群（压缩后的）单副本数据大小。 备份完成之后，你可以在一个空集群上恢复备份的数据，将集群恢复到与快照备份对应的数据状态，同时恢复功能会依据集群副本设置恢复出多副本。
