...
aliases: ['/docs-cn/dev/br/pitr-faq/']
---
...
## 在使用 `br restore point` 命令恢复下游集群后，无法从 `TiFlash` engine 查询中到数据？

在当前 v6.2.0 版本中，使用 PiTR 功能恢复下游集群数据时，并不支持恢复下游的 TiFlash replica，用户在恢复数据之后，需要手动设置 schema/table 的 TiFlash replica, 如：
...
此时，用户需要根据错误提示来确认故障的原因并恢复故障，确认故障恢复之后，可执行下面的命令来恢复备份任务：
```shell
br log resume --task-name=task1 --pd x.x.x.x:2379
```

然后再次查询 `br log status`,任务状态变为正常，备份任务继续执行。
```shell
● Total 1 Tasks.
> #1 <
              name: task1
