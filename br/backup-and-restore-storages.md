...
- 恢复时 TiKV 和 br 命令行工具需要的访问备份数据目录的最小权限：`s3:ListBucket`、`s3:GetObject` 和 `s3:PutObject`（br 命令行工具会将断点信息写到备份数据目录下的子目录 `/checkpoints` 中；在恢复日志备份数据时，br 命令行工具会将备份恢复集群的表 id 映射关系写到备份数据目录下的子目录 `/pitr_id_maps` 中）。
