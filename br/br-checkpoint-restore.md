...
在日志恢复过程中，日志数据恢复的顺序是无序的，因此可能会出现 delete-kv 优先于 write-kv 恢复的情况。如果此时触发 GC，delete-kv 会被删除，导致后续恢复的 write-kv 无法被 GC 处理。为了避免这种情况，br 命令行工具会在日志恢复过程中暂停 GC。当 br 工具中途退出后，GC 将仍然保持暂停状态。
