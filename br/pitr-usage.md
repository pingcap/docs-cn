...
1. 在 S3 创建用于保存备份数据的目录 `s3://tidb-pitr-bucket/backup-data`。
    ...
    2. 创建备份数据总目录。在上一部创建的 bucket（例如 `tidb-pitr-bucket`）下创建目录 `backup-data`，参考 [AWS 官方文档](https://docs.aws.amazon.com/zh_cn/AmazonS3/latest/user-guide/create-folder.html)。
...
当任务状态变为 `ERROR` 后，你需要根据提示确认故障原因，并主动恢复故障环境。然后，通过 `br log resume --task-name {TASK-NAME} --PD {IP}:{PORT}` 来恢复任务，备份任务将继续从上一次的备份时间点备份数据。

> **注意：**
>
> 由于此功能会备份集群的多版本数据，当任务发生错误且状态变更为 `ERROR` 后，同时会为该任务在当前备份进度点的数据设 `safe point`，`safe point` 的数据将保证 24 小时不被 GC。所以，当任务恢复之后，会继续从上一个备份点备份日志。如果任务失败时间超过 24 小时，前一次备份进度点的数据已经被 GC，此时恢复任务操作会提示失败。这种场景下只能终止本次任务，重新开启新的备份任务。
