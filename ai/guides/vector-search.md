---
title: 向量检索
summary: 了解如何在你的应用中使用向量检索。
---

# 向量检索

向量检索利用语义相似性，帮助你找到最相关的记录，即使你的查询没有明确包含所有关键字。

> **注意：**
>
> 有关向量检索的完整示例，请参见 [向量检索示例](/ai/examples/vector-search-with-pytidb.md)。

## 基本用法

本节展示了如何通过几个简单步骤在你的应用中使用向量检索。在开始之前，你需要先[连接到数据库](/ai/guides/connect.md)。
...
- 使用 `VEC_COSINE_DISTANCE` 函数创建了向量索引，以优化向量检索性能。
...
向量检索使用向量距离度量来衡量向量之间的相似性和相关性。距离越近，记录越相关。要在表中查找最相关的文档，你需要指定一个查询向量。
...
使用 `table.search()` 方法执行向量检索。默认使用 `search_mode="vector"`。
...
| 度量名称                  | 描述                                                                 | 最佳应用场景 |
|--------------------------|---------------------------------------------------------------------|--------------|
| `DistanceMetric.COSINE`  | 计算两个向量的余弦距离（默认）。衡量向量之间的夹角。                | 文本嵌入、语义检索 |
| `DistanceMetric.L2`      | 计算两个向量的 L2 距离（欧氏距离）。衡量直线距离。                  | 图像特征     |

要更改向量检索使用的距离度量，可使用 `.distance_metric()` 方法。
...
使用 `.distance_threshold()` 方法为检索结果设置最大距离。只有距离小于该阈值的记录才会被返回。
...
在 TiDB 中进行向量检索时，你可以对标量字段（如整数型和字符串）或 JSON 字段进行元信息过滤。

通常，向量检索结合元信息过滤有两种模式：

- **后过滤**：TiDB 首先在整个向量空间中执行向量检索，获取 top-k 候选，然后对候选集应用过滤条件。向量检索阶段通常会使用向量索引以提升性能。
- **预过滤**：TiDB 在向量检索前先应用过滤条件。如果过滤条件选择性高，且被过滤字段有标量索引，该模式可以减少检索空间并提升性能。
...
使用 `.filter()` 方法和过滤字典为向量检索添加过滤条件。

默认情况下，`table.search()` API 使用后过滤模式，以最大化向量索引的检索性能。
...
> **注意：**
>
> 使用向量索引时，如果最终的 `limit` 很小，结果的准确性可能会降低。你可以通过 `.num_candidate()` 方法控制向量检索阶段从向量索引中获取多少候选，而不改变 `limit` 参数。
...
如果你需要将向量检索与其他过滤条件结合，可以采用后过滤模式。在该模式下，ANN 查询会被拆分为两部分：
...
> **注意：**
>
> 后过滤模式可能导致结果为空。例如，内层查询检索到 top 50 最相似的记录，但这些记录都不满足 `WHERE` 条件。
>
...
TiDB 支持在单个表中定义多个向量列，便于存储和检索不同类型的向量嵌入。
...
你可以在 schema 中定义多个向量字段，并通过 `.vector_column()` 方法指定要检索的向量字段。
...
`table.search()` API 支持将检索结果转换为多种常见数据处理格式：
