...
summary: 修改 TiDB Cloud Filesystem 的媒体内容提取配置。
---

# ti fs update-file-system-extract-configuration

修改单个 Filesystem 的可选镜像、音频或视频媒体内容提取配置。启用或替换 provider 时，会执行一次真实的 provider 验证请求，这可能会产生少量 provider 费用。启用后，Filesystem 中的媒体文件会被发送到所选 provider 进行媒体内容提取。
...
- `--file-system-id <string>`：设置不可变的 Filesystem ID。\[required]
- `--media-type <string>`：选择 `image`、`audio` 或 `video`。\[required]
- `--dry-run`：在不联系 Filesystem 后端或 AI provider 的情况下验证该请求。
...
provider API key 仅接受通过 `TI_FS_AI_PROVIDER_API_KEY` 提供。它会被发送到 Filesystem 后端进行验证和加密存储，`ti` 不会在本地存储它，并且返回时只会以掩码形式显示。`openai` 支持镜像、音频和视频。Alibaba Cloud Model Studio Qwen ASR 可通过 `qwen-asr` 用于音频。其他服务仅在其实现了完全符合要求的 OpenAI-compatible contract 时才可用。不支持原生 Anthropic、Gemini、Vertex AI、Bedrock 和 Azure OpenAI 接口。
