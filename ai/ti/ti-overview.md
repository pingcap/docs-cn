...
summary: 了解何时使用 TiDB Cloud CLI (`ti`) 来管理 TiDB Cloud Starter 实例和 TiDB Cloud Filesystem 中的文件系统。
---

# TiDB Cloud CLI (`ti`) 概述

[TiDB Cloud CLI (`ti`)](https://github.com/tidbcloud/ti-cli) 是一个用于管理 [TiDB Cloud Starter](https://docs.pingcap.com/tidbcloud/select-cluster-tier/?plan=starter#starter) 实例和 [TiDB Cloud Filesystem 中的文件系统](#tidb-cloud-filesystem) 的 CLI。它同时适用于交互式使用和自动化场景，并且默认提供结构化的 JSON 输出。
...
| 在临时环境中使用文件系统 | 在受信任的机器上预配一个文件系统，然后将其文件系统访问令牌和 region code 提供给沙箱，而无需复制 CLI 配置（Profile）或提供 TiDB Cloud API key。 |
...
`ti` 专为 TiDB Cloud Starter 的自动化以及 TiDB Cloud Filesystem 管理而设计，而 `ticloud` 继续支持 TiDB Cloud Essential 以及 `ti` 中尚不可用的其他 TiDB Cloud 操作。

| CLI | 最适合 | 关键特性 |
| --- | --- | --- |
| `ti` | 受支持的 TiDB Cloud Starter 自动化工作流和 TiDB Cloud Filesystem | 为自动化而设计；默认输出 JSON；命令支持非交互式工作流，同时 `ti configure` 也可以进行交互式提示 |
...
- 对于管理 TiDB Cloud Filesystem，请使用 `ti`。
...
- [使用 TiDB Cloud CLI 管理 TiDB Cloud Filesystem](/ai/ti/guides/manage-filesystems-via-cli.md)
