...
summary: 了解 TiDB 中的向量搜索功能。该功能为文档、图片、音频和视频等多种数据类型提供了先进的语义相似性检索解决方案。
aliases: ['/tidb/stable/vector-search-overview/','/tidb/dev/vector-search-overview/','/tidbcloud/vector-search-overview/']
---

# 向量搜索概述

向量搜索为文档、图片、音频和视频等多种数据类型提供了强大的语义相似性检索解决方案。它允许开发者利用 MySQL 的专业知识，构建具备生成式 AI 能力的可扩展应用，简化高级检索功能的集成。
...
向量搜索是一种以数据语义为核心、提供相关性结果的检索方法。

与传统的全文检索依赖于精确关键字匹配和词频不同，向量搜索会将多种数据类型（如文本、图片或音频）转换为高维向量，并基于这些向量之间的相似性进行查询。这种检索方法能够捕捉数据的语义含义和上下文信息，从而更准确地理解用户意图。

即使检索词与数据库中的内容并不完全匹配，向量搜索也可以通过分析数据的语义，返回符合用户意图的结果。

例如，使用全文检索查询 “a swimming animal” 只会返回包含这些精确关键字的结果。而向量搜索则可以返回其他游泳动物（如鱼或鸭子）的结果，即使这些结果中并不包含完全相同的关键字。
...
TiDB 引入了专为优化向量嵌入存储与检索设计的 [向量数据类型](/ai/reference/vector-search-data-types.md) 和 [向量搜索索引](/ai/reference/vector-search-index.md)，提升其在 AI 应用中的使用效率。你可以将向量嵌入存储在 TiDB 中，并通过这些数据类型执行向量搜索查询，查找最相关的数据。
...
检索增强生成（Retrieval-Augmented Generation，RAG）是一种旨在优化大语言模型（LLM）输出的架构。通过向量搜索，RAG 应用可以将向量嵌入存储在数据库中，并在 LLM 生成响应时检索相关文档作为额外上下文，从而提升答案的质量和相关性。

### 语义检索

语义检索是一种基于查询语义返回结果的检索技术，而不仅仅是关键字匹配。它通过嵌入理解不同语言和多种类型数据（如文本、图片和音频）的含义。向量搜索算法随后利用这些嵌入，查找最能满足用户查询需求的相关数据。
