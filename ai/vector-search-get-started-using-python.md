...
### 步骤 3. 配置 TiDB 连接字符串 {#step-3-configure-the-tidb-connection-string}

根据你选择的 TiDB 部署方式，配置连接字符串。
...
1. 进入 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，然后点击目标 {{{ .starter }}} 实例的名称，进入其实例概览页。
...
### 步骤 5. 连接 TiDB {#step-5-connect-to-tidb}

使用 `TiDBVectorClient` 类连接 TiDB，并创建包含向量列的 `embedded_documents` 表。
