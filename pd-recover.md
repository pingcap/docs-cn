...
使用启动参数 `--force-new-cluster` 拉起该存活的 PD 节点，并确保该节点使用的是存活节点原始的数据目录。你可以在命令行中通过 `--data-dir` 显式指定，也可以在 `conf/pd.toml` 中提前配置 `data-dir`。例如：
...
> - 如果同时在 `conf/pd.toml` 和 `data-dir` flag 中指定存活节点原始的数据目录，则 `conf/pd.toml` 优先生效。
