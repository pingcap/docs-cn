...
硬盘选择标准同 [TiFlash 单独部署模式](#tiflash-单独部署模式)。硬盘总容量大致为：`整个 TiKV 集群的需同步数据容量/ TiKV 副本数 * TiFlash 副本数`。例如整体 TiKV 的规划容量为 1TB, 3 副本，TiFlash 2 副本，则 TiFlash 的推荐总容量为 (1024GB / 3 * 2)。用户可以选择同步部分表数据而非全部。
