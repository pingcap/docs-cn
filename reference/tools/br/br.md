...
在进行 BR 备份时，显示指定参数 `--s3.region` 和 `--send-credentials-to-tikv`, 第一个参数表示 S3 存储所在的区域，第二个参数表示将 S3 的访问权限传递给 TiKV 节点。
...
在进行 BR 恢复时，显示指定参数 `--s3.region` 和 `--send-credentials-to-tikv`, 第一个参数表示 S3 存储所在的区域，第二个参数表示将 S3 的访问权限传递给 TiKV 节点。`--storage`参数中的 `Bucket` 和 `Folder` 分别代表需要恢复的数据所在的 S3 存储桶和文件夹。
