...
| `server-id` | Identifies the server ID of MySQL or MariaDB in the upstream as a slave. In a replication group, each instance (master and slave included) must have a unique server ID. In v1.0.2 and later versions, the `server_id` is automatically generated by DM. | |
