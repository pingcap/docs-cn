...
你需要使用 dmctl 手动处理 TiDB 不兼容的 DDL 语句（包括手动跳过该 DDL 语句或使用用户指定的 DDL 语句替换原 DDL 语句，详见[跳过 (skip) 或替代执行 (replace) 异常的 SQL 语句](/v3.1/reference/tools/data-migration/skip-replace-sqls.md)）。
...
2. 使用 Ansible [停止整个 DM 集群](/v3.1/how-to/deploy/data-migration-with-ansible.md#第-10-步关闭-dm-集群)。
...
5. 使用 Ansible [启动整个 DM 集群](/v3.1/how-to/deploy/data-migration-with-ansible.md#第-9-步部署-dm-集群)。
