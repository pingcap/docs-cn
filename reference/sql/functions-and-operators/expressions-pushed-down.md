...
其中，`name` 为禁止下推的函数名，`store_type` 指明希望禁止该函数下推到哪个 store，目前 TiDB 支持三种 store，分别为 `tikv`， `tidb` 和 `tiflash`，`store_type` 不区分大小写，如果需要禁止向多个 store 下推的话各个 store 之间以逗号隔开，`reason` 列可以记录该函数被加入黑名单的原因。
