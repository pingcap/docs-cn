...
* `TYPE`：实例类型，目前实例的可取值为 `tidb`，`pd` 或 `tikv`。
...
* `VERSION`：对应实例的语义版本号。TiDB 版本为了兼容 MySQL 的版本号，以 `${mysql-version}-${tidb-version}` 的格式展示版本号。
* `GIT_HASH`：编译实例版本时的 Git Commit Hash，用于识别两个实例是否是绝对一致的版本。
...
+------+-----------------+-----------------+-------------+------------------------------------------+---------------------------+--------------+
| tidb | 127.0.0.1:4000  | 127.0.0.1:10080 | 4.0.0-beta  | b5ea3232afa970f00db7a0fb13ed10857db1912e | 2020-03-02T16:27:28+08:00 | 4m18.845924s |
