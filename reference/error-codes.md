...
* Error Number: 8028
    ...
    TiDB 没有表锁（在 MySQL 中称为元数据锁，在某些其他数据库中称为意向锁等等）。当事务执行时，TiDB 表结构发生了变化是无法被事务感知到的。因此，TiDB 在事务提交时，会对事务涉及表的结构进行检查。如果事务执行中表结构发生了变化，则事务将提交失败，并返回该错误。遇到该错误，应用程序可以安全地重新执行整个事务。
...
* Error Number: 8031
    ...
    将负数转变为无符号数时，被负数转变为正数。多在告警信息里出现。
