...
category: FAQ
aliases: ['/docs-cn/v3.1/faq/tidb-in-kubernetes/']
---
...
* 集群已经在运行
    ...
    * 参考[时区支持](/v3.1/how-to/configure/time-zone.md)，修改 TiDB 服务时区配置。
...
* TiKV 自动故障转移之后的集群调整，参考[自动故障转移](/v3.1/tidb-in-kubernetes/maintain/auto-failover.md)；
* 维护或下线指定的 Kubernetes 节点，参考[维护节点](/v3.1/tidb-in-kubernetes/maintain/kubernetes-node.md)。
...
假如要为 TiDB in Kubernetes 添加 TiSpark 组件，你需要在**同一个** Kubernetes 集群中自行维护 Spark，确保 Spark 能够访问到 PD 和 TiKV 实例的 IP 与端口，并为 Spark 安装 TiSpark 插件，TiSpark 插件的安装方式可以参考 [TiSpark](/v3.1/reference/tispark.md#已有-Spark-集群的部署方式)。
...
* 资源不足，导致 Pod 一直阻塞在 `Pending` 状态。详细说明参见[集群故障诊断](/v3.1/tidb-in-kubernetes/troubleshoot.md)。
