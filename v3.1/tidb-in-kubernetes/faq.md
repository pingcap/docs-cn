...
* If the cluster is running:
    ...
    * Refer to [Time Zone Support](/v3.1/how-to/configure/time-zone.md) to modify TiDB service time zone settings.
...
* Adjusting the cluster after the auto-failover of TiKV. See [Auto-Failover](/v3.1/tidb-in-kubernetes/maintain/auto-failover.md) for details;
* Maintaining or dropping the specified Kubernetes nodes. See [Maintaining Nodes](/v3.1/tidb-in-kubernetes/maintain/kubernetes-node.md) for details.
...
* Insufficient resource or HA Policy causes the Pod stuck in the `Pending` state. Refer to [Troubleshoot TiDB in Kubernetes](/v3.1/tidb-in-kubernetes/troubleshoot.md#the-pod-is-in-the-pending-state) for more details.
