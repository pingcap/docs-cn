...
- If a `StorageClass` exists in the cluster but the available PV is insufficient, you need to add PV resources correspondingly. For Local PV, you can expand it by referring to [Local PV Configuration](/v3.1/tidb-in-kubernetes/reference/configuration/storage-class.md#local-pv-configuration).
...
After checking the error messages in the log, you can refer to [Cannot start `tidb-server`](/v3.1/how-to/troubleshoot/cluster-setup.md#cannot-start-tidb-server), [Cannot start `tikv-server`](/v3.1/how-to/troubleshoot/cluster-setup.md#cannot-start-tikv-server), and [Cannot start `pd-server`](/v3.1/how-to/troubleshoot/cluster-setup.md#cannot-start-pd-server) for further troubleshooting.
