...
- 如果集群内 [kubelet](https://kubernetes.io/docs/reference/command-line-tools-reference/kubelet/) 允许配置 `--allowed-unsafe-sysctls=net.*`，请为 `kubelet` 配置这个参数，并按如下方式配置 TiDB：
...
- 如果集群内 [kubelet](https://kubernetes.io/docs/reference/command-line-tools-reference/kubelet/) 不允许配置 `--allowed-unsafe-sysctls=net.*`，请按如下方式配置 TiDB：
...
> **注意：**
>
> 以上配置的环境要求：TiDB Operator 1.1 及以上版本。
