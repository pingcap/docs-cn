...
本文档只阐述未包含在命令行参数中的参数，命令行参数参见 [TiKV 配置参数](/v3.1/reference/configuration/tikv-server/configuration.md)。
...
+ 悲观事务在 TiKV 中等待其他事务释放锁的最长时间，单位为毫秒。若超时则会返回错误给 TiDB 并由 TiDB 重试加锁，语句最长等锁时间由 `lock_wait_timeout` 控制。
+ 默认值：1000
+ 最小值：1
