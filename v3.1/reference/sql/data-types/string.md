...
定长字符串。`CHAR` 列的长度固定为创建表时声明的长度。在 ASCII 字符集下，长度可以为从 0 到 255 的任何值。当保存 CHAR 值时，在它们的右边填充空格以达到指定的长度。
...
变长字符串。M 表示最大列长度（字符的最大个数）。在 ASCII 字符集下，范围是 0 到 65535。在选择 `VARCHAR` 长度时，应当根据最长的行的大小和使用的字符集确定。详情请参阅 [不同字符集所占用的空间长度](#不同字符集所占用的空间长度)。
...
文本串。M 表示最大列长度（字符的最大个数），在 ASCII 字符集下，范围是 0 到 65535。在选择 `TEXT` 长度时，应当根据最长的行的大小和使用的字符集确定。详情请参阅 [不同字符集所占用的空间长度](#不同字符集所占用的空间长度)。
...
每种列类型都有对应的最大占用空间字节数，例如 `VARCHAR`，其空间大小不得超过 65535 字节。

而对于不同的字符集，单个字符所占用的空间可能有所不同。例如 ASCII 字符集，单个字符占用 1 个字节；UTF8 字符集，单个字符占用 3 个字节。因此，在有限的空间大小下，最大列长度也因字符集而异。

以下是字符集与单个字符占用的字节数之间的关系：
