...
`CHARACTER_SETS` 表提供[字符集](/v3.1/reference/sql/character-set.md)相关的信息。TiDB 目前仅支持部分字符集。
...
`SLOW_QUERY` 表中提供了慢查询相关的信息，其内容通过解析 TiDB 慢查询日志而来，列名和慢日志中的字段名是一一对应。关于如何使用该表调查和改善慢查询请参考[慢查询日志文档](/v3.1/how-to/maintain/identify-abnormal-queries/identify-slow-queries.md)。
