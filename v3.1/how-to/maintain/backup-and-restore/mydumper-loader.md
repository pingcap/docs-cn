...
本文档将详细介绍如何使用 Mydumper/Loader 对 TiDB 进行全量备份与恢复。增量备份与恢复可使用 [TiDB Binlog](/v3.1/reference/tidb-binlog/overview.md)。
...
可使用 [`mydumper`](/v3.1/reference/tools/mydumper.md) 从 TiDB 导出数据进行备份，然后用 [`loader`](/v3.1/reference/tools/loader.md) 将其导入到 TiDB 里面进行恢复。
...
我们使用 `loader` 将之前导出的数据导入到 TiDB，完成恢复操作。Loader 的下载和具体的使用方法见 [Loader 使用文档](/v3.1/reference/tools/loader.md)
