...
3. If TiDB Lightning was improperly restarted, see also the "[How to properly restart TiDB Lightning](/v3.1/faq/tidb-lightning.md#how-to-properly-restart-tidb-lightning)" section in the FAQ.

## Checkpoint for … has invalid status: (error code)

**Cause**: [Checkpoint](/v3.1/reference/tools/tidb-lightning/checkpoints.md) is enabled, and Lightning or Importer has previously abnormally exited. To prevent accidental data corruption, Lightning will not start until the error is addressed.
...
See the [Checkpoints control](/v3.1/reference/tools/tidb-lightning/checkpoints.md#checkpoints-control) section for other options.
