...
从 static 静态裁剪模式切到 dynamic 动态裁剪模式时，需要手动检查和收集汇总统计信息。如示例三中，在刚切到 dynamic 的时候，分区表上仍然只有分区的统计信息，需要等到下一次 auto-analyze 周期才会更新生成汇总统计信息。

**示例三**：以下示例在 `static` 切到 `dynamic` 模式后检查表级别的汇总统计信息。
