...
### `tso-update-physical-interval` <span class="version-mark">从 v6.4.0 版本开始引入</span>

+ TSO 物理时钟更新周期。
+ 在一个 TSO 物理时钟更新周期内，PD 最多提供 262144 个 TSO。如果需要更多的 TSO，可以将这个参数调小。最小值为 `1ms`。
+ 缩短这个参数会增加 PD 的 CPU 消耗。根据测试，`1ms` 相比 `50ms` 额外消耗约 0.1 个 CPU core。
+ 默认值：50ms
+ 最小值：1ms
