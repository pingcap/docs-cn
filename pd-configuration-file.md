...
> **警告：**
>
> - 对于 v8.1.0 到 v8.5.2 版本，TiDB 移除了遥测功能，该配置项已不再生效。保留该配置项仅用于与之前版本兼容。
> - 对于 v8.5.3 到 v8.5.6 版本，TiDB 重新引入了遥测功能。但是，它仅在本地记录与遥测相关的信息，不再通过网络向 PingCAP 发送数据。
> - 从 v8.5.7 开始，TiDB 废弃了该配置项和遥测功能。

+ 用于控制是否启用 TiDB Dashboard 遥测收集功能。
+ 默认值：`false`
