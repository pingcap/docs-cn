...
PD 配置文件比命令行参数支持更多的选项。你可以在 [conf/config.toml](https://github.com/pingcap/pd/blob/release-8.1/conf/config.toml) 找到默认的配置文件。
...
> **注意：**
> 
> 要修改调度相关的 PD 配置项，请根据情况使用以下其中一种方法：
>
> - 对于新部署集群，在配置文件中修改该配置项。此方法对已有集群不生效，无法像修改配置文件中其他参数一样通过此方法修改已有集群配置文件中的 `schedule` 参数。
> - 对于已有集群，使用命令行工具 [PD Control](/pd-control.md) 进行修改。
