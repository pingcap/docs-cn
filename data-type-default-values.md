...
所有数据类型都可以设置默认值。从 v8.0.0 开始，新增支持 [`BLOB`](/[data-type-string.md](http://data-type-string.md/)#blob-类型)、[`TEXT`](/[data-type-string.md](http://data-type-string.md/)#text-类型) 以及 [`JSON`](/[data-type-json.md](http://data-type-json.md/)#json-类型) 这三种数据类型设置默认值，但仅支持使用表达式设置[默认值](#表达式默认值)。
