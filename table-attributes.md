...
> - TiDB v5.3.0 和 v5.3.1 **不支持** Index Region。有关 Index Region 的具体信息可以参考 [Split Index Region](/sql-statements/sql-statement-split-region.md#split-index-region)。
> - 在 TiDB v5.3.0 和 v5.3.1 中，添加表属性**仅对**已存在的分区生效。如需在添加表属性后新增分区或删除分区，你需要重新设置表属性。有关重新设置表属性的方法可以参考[表属性的使用方法](#使用方法)。
