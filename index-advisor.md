...
索引推荐功能分别评估 `a` 和 `b` 上的单列索引，并最终将它们合并为一个组合索引 `(a, b)` 推荐出来。

以下 `EXPLAIN` 结果比较了无索引和使用推荐的两列索引的查询执行计划。索引推荐功能在内部评估这两种方案，并选择执行计划成本最低的选项。索引推荐功能还会考虑 `a` 和 `b` 上的单列索引，但在这种情况下，这些索引的性能不如组合的两列索引（为简洁起见，省略了这些执行计划）。
