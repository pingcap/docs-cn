...
在 `AS OF TIMESTAMP` 中，你可以指定历史数据的时间点或时间范围：

- 如果你想要指定一个精确的时间点，可在 `AS OF TIMESTAMP` 中使用日期时间和时间函数，日期时间的格式为："2016-10-08 16:45:26.999"，最小时间精度范围为毫秒，通常可只写到秒，例如 "2016-10-08 16:45:26"。你也可以通过 `NOW(3)` 函数获得精确到毫秒的当前时间。从 v7.5.0 开始，你还可以在 `AS OF TIMESTAMP` 中使用 TSO 时间点。
- 如果想读取几秒前的数据，推荐使用例如 `NOW() - INTERVAL 10 SECOND` 的表达式。
...
- `AS OF TIMESTAMP '445494955052105728'` 表示读取 445494955052105728 这个 TSO 时间点最新的数据
