...
>
> 你可以通过为使用 Stale Read 特性的查询禁用 TiFlash 副本来解决这个问题，推荐的方式有两种：一种是通过设置变量来禁用 TiFlash 副本 `set session tidb_isolation_read_engines='tidb,tikv'`；另一种是使用 [hint](https://docs.pingcap.com/zh/tidb/v5.1/optimizer-hints#read_from_storagetiflasht1_name--tl_name--tikvt2_name--tl_name-) 强制 TiDB 从 TiKV 读取数据。
