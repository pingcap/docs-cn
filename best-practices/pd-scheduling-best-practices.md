...
你可以通过观察 PD 相关的 Metrics 或使用 pd-ctl 工具等方式查看调度系统状态。更具体的信息可以参考 [PD 监控](/reference/key-monitoring-metrics/pd-dashboard.md)和 [PD Control](/reference/tools/pd-control.md)。
...
使用 pd-ctl 可以从以下三个方面来调整 PD 的调度策略。更具体的信息可以参考 [PD Control](/reference/tools/pd-control.md)。
