...
TiDB 的分区表提供了一种灵活的方式来管理大规模数据集，提升查询效率、便于批量删除历史数据并缓解写入热点问题。通过将数据划分为逻辑分区，TiDB 利用分区裁剪 (Partition Pruning) 在查询执行时跳过不相关的分区，从而减少资源消耗并提升性能，尤其适用于包含大量数据的联机分析处理 (Online Analytical Processing, OLAP) 工作负载。

一个常见用例是将 [Range 分区](/partitioned-table.md#range-分区)与本地索引 (Local Index) 结合，以通过例如 [`ALTER TABLE ... DROP PARTITION`](/sql-statements/sql-statement-alter-table.md) 等操作高效清理历史数据。此方法能几乎瞬间删除过期数据，并在按分区键过滤时保持高查询效率。然而，从非分区表迁移到分区表后，那些无法利用分区裁剪的查询（例如未包含分区键的查询）可能出现性能下降。在这种情况下，可以使用[全局索引 (Global Index)](/global-indexes.md) 通过在所有分区间提供统一的索引结构来缓解性能影响。

另一种场景是使用哈希或 Key 分区来解决写入热点问题，尤其是当工作负载使用 [`AUTO_INCREMENT`](/auto-increment.md) ID 时，顺序插入会使得特定 TiKV Region 过载。将写入分散到不同分区可以平衡负载，但与 Range 分区类似，那些未能利用分区裁剪的查询可能再次出现性能下降，此时可以利用全局索引解决该问题。

尽管分区化带来明显好处，但也会引入挑战。例如，新创建的 Range 分区可能会产生临时热点。为了解决此问题，TiDB 支持自动或手动的 Region 预分裂 (Pre-splitting) 来平衡数据分布，避免出现瓶颈。
