...
- > 注意  
  > 在使用流式读取数据时，需要将 resultset 读取完成或 close 后，才能继续使用该 stmt 进行下次查询，否则会报错 `No statements may be issued when any streaming result sets are open and in use on a given connection. Ensure that you have called .close() on any active streaming result sets before attempting more queries.`
  > 如果需要在 resultset 读取完成或 close 前进行查询避免上述报错，可在 url 添加配置参数 `clobberStreamingResults=true`，这样会自动 close resultset，但之前流式查询未被读取的结果集会丢失。
