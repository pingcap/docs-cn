...
- 仅适用于 TiDB X：TiDB X 只支持将该变量设置为 `"standard"`，不支持设置为 `"bulk"`。如果设置为 `"bulk"`，设置会被拒绝，因此不能通过 `tidb_dml_type = "bulk"` 启用 Pipelined DML。
...
- 仅适用于 TiDB X：TiDB X 只支持 `leader`。如果将该变量设置为其他读取模式，设置会被拒绝。
