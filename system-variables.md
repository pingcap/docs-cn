...
- 作用域：SESSION | GLOBAL
- 是否持久化到集群：是
- 默认值：在 v6.1.0 以及 v6.5.0 之后的版本中为 `ON` ，即默认开启。在 v6.1.0 和 v6.5.0 之间的版本为 `OFF`，即默认关闭。
- 自 v6.1.0 起，TiDB 的 [Join Reorder 算法](/join-reorder.md) 开始支持 Outer Join。该变量用于控制这个支持行为。默认关闭，即不启用 Outer Join 的 Join Reorder。
- 对于从 v6.1.0 之前版本升级到 v6.1.0 及之后的版本，该变量的默认值为 `ON`。对于从 v6.1.0 版本升级到之后的版本，该变量默认值跟随升级前的设定值。
