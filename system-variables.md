...
- 类型：布尔型
- 默认值：`OFF`
- 这个变量用来开启 Active PD Follower 特性（目前只适用于 Region 信息的相关请求）。当该值为 `OFF` 时，TiDB 仅会从 PD leader 获取 Region 信息。开启该特性之后，TiDB 在获取 Region 信息时会将请求均匀地发送到所有 PD 节点上，PD follower 也可以直接处理 Region 请求，从而降低 PD leader 的 CPU 压力。
