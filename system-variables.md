...
> 
> - 对于“集群持久化”为“是”的变量，当该全局变量被修改后，会通知所有 TiDB 服务器刷新其系统变量缓存。在集群中增加一个新的 TiDB 服务器时，或者重启已有的 TiDB 服务器时，都将自动使用该持久化变量。
> - 对于“集群持久化”为“否”的变量，对变量的修改只对当前连接的 TiDB 实例生效。如果需要保留设置的值，需要在 `tidb.toml` 配置文件中声明。
> 
