...
- 是否持久化到集群：是
- 类型：布尔型
- 默认值：在 v6.6.0 之前版本中为 `OFF`，即默认关闭外键约束检查。在 v6.6.0 及之后的版本中为 `ON`，即默认开启外键约束检查。
- 表示是否开启外键约束检查。
...
- 是否持久化到集群：是
- 类型：布尔型
- 默认值：在 v6.6.0 之前版本中为 `OFF`，即默认关闭 `FOREIGN KEY` 特性。在 v6.6.0 及之后的版本中为 `ON`，即默认开启 `FOREIGN KEY` 特性。
