...
- 作用域：SESSION | GLOBAL
- 默认值：`ON`
- 这个变量用来控制优化器是否可以把 null 值当做点来进行索引的访问。
- 比如在有多列索引 `index(a, b)` 及查询条件为 `a<=>null and b=1` 的情况下，如果此开关关闭，则不能把 `b=1` 用来进行索引访问，因为 null 值不能被当做固定前缀。
- 如果开关打开，则可以同时用 `a<=>null and b=1` 进行索引访问。
