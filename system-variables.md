...
- 该参数的取值不能超过 [`max_connections`](/tidb-configuration-file.md#max_connections)。若超过，则 TiDB 会采用 `max_connections` 的值。例如，若某用户的 `max_user_connections` 设置为 `2000`，而 `max_connections` 为 `1000`，则该用户实际可建立的最大连接数为 `1000`。
