...
### `tidb_generate_binary_plan` <span class="version-mark">从 v6.2 版本开始引入</span>

- 作用域：GLOBAL
- 是否持久化到集群：是
- 默认值：`ON`
- 这个变量用于指定是否在 slow log 和 statement summary 里包含以二进制格式编码的执行计划。TiDB Dashboard 中的图形化执行计划依赖该信息。
