...
- 如果开启 TiFlash 延迟物化功能（设置为 `ON` 时）, 当 `SELECT` 语句中包含过滤条件（ `WHERE` 子句）时，普通的处理方式是扫描查询所需列的全部数据后进行过滤。TiFlash 延迟物化功能可以先扫描过滤条件相关的列数据，过滤得到符合条件的行后，再扫描这些行的其他列数据，继续后续计算，从而减少扫描 IO 和数据解析的计算量。
