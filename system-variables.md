...
- 是否受 Hint [SET_VAR](/optimizer-hints.md#set_varvar_namevar_value) 控制：否
- 类型：字符串（只读）
- 默认值：来自 `[instance]` 配置项（代码默认读取 `plugin-audit-log-flush-interval`）
- 审计日志插件相关的实例级只读变量。
...
- 类型：布尔型
- 默认值：`ON`
- 控制 TiFlash Pipeline 模式开关（代码中变量名为 `tidb_enable_tiflash_pipeline_model`）。
