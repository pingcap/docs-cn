...
- 类型：布尔型
- 默认值：`OFF`
- 该变量控制优化器是否在会话级别或全局范围对 `SELECT` 列表中包含子查询的所有查询应用 [`NO_DECORRELATE()`](/optimizer-hints.md#no_decorrelate) Hint。
...
- 类型：布尔型
- 默认值：`OFF`
- 该变量控制优化器是否在会话级别或全局范围对包含子查询的所有查询应用 [`SEMI_JOIN_REWRITE()`](/optimizer-hints.md#semi_join_rewrite) Hint。
