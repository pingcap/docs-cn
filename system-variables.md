...
- 类型：布尔型
- 默认值：`ON`
- 此变量控制异步提交 (Async Commit) 中提交时间戳的计算方式。默认情况下（使用 `ON` 值），两阶段提交不从 PD 获取时间戳，这种情况下只保证因果一致性但不保证线性一致性。对于需要只保证因果一致性的场景，该变量为 `ON` 值可提升性能。详情参考 PingCAP 博文 [Async Commit 原理介绍](https://pingcap.com/zh/blog/async-commit-principle)。
- 如果要保证所有并发事务可线性化，可将该变量值设为 `OFF`，这样两阶段提交从 PD 服务器请求一个新的时间戳，并使用该时间戳计算最终提交的时间戳。
