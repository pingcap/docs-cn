...
- 该变量默认开启，能减少需要访问的索引数据量，提高查询执行速度。例如，在有多列索引 `index(a, b)` 且查询条件为 `a<=>null and b=1` 的情况下，优化器可以同时使用查询条件中的 `a<=>null` 和 `b=1` 进行索引访问。如果关闭该变量，因为 `a<=>null and b=1`包含 null 值的等值条件，优化器不会使用 `b=1` 进行索引访问。
