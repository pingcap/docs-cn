...
- 默认值：`86400`
- 范围：`[600, 31536000]`
- 这个变量用于指定活跃事务阻碍 GC Safe Point 推进的最大时间。变量值为整型值，单位是秒。每次进行 GC 时，默认 GC safe point 不会超过正在执行中的事务的开始时间。如果活跃事务运行时间未超过该值， GC Safe Point 一直会被阻塞不更新，直到活跃事务运行时间超过该值 Safe Point 才会正常往前推进。
