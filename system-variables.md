...
> **注意：**
> - 该参数是从 [`tidb_ddl_distribute_reorg`](#tidb_ddl_distribute_reorg-从-v6.6.0-版本引入) 参数改名而来。
> - 该功能目前为实验特性。
> - 当前启用此功能后，DDL, Import 的一些后端任务将会启动分布式执行任务的方式来利用集群中的多个 TiDB 节点来共同完成任务。

- 作用域：GLOBAL
- 是否持久化到集群：是
- 默认值：`OFF`
- 这个变量用于控制是否开启分布式执行。开启后 DDL，Import 等支持的任务将会在多个。
