...
- 类型：布尔型
- 默认值：`OFF`
- 当 TiDB 给 TiFlash 下发计算任务时，有三种方法（或协议）可供选择：Cop、BatchCop 和 MPP。相比于 Cop 和 BatchCop，MPP 协议更加成熟，提供更好的任务和资源管理。因此，更推荐使用 MPP 协议。

    * 0 或 OFF：意味着优化器仅生成使用 TiFlash MPP 协议的计划。
    * 1 或 ON：意味着优化器根据成本估算确定是否使用 Cop、BatchCop 和 MPP 协议。
