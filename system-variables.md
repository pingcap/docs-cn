...
> **特殊情况**：
> 若部分 TiDB 节点显式设置了 [`tidb_service_scope`](/system-variables.md#tidb_service_scope) 参数如设置为 group1，则分布式任务仅会在这些节点中调度执行。此时，即使 `tidb_max_dist_task_nodes` 设置了更大的值，实际使用的 TiDB 节点数也不会超过显式设置了 `tidb_service_scope` 的 TiDB 节点数。
> 例如：集群中有 10 个 TiDB 节点，其中只有 4 个节点设置了相同的 `tidb_service_scope` 值为 `group1`，你设置 tidb_max_dist_task_nodes = 5;最终实际参与任务执行的 TiDB 节点数为 4，而非 5。
