...
- 类型：布尔型
- 默认值：`OFF`
- 当系统中创建的 SQL 用户数量过多（例如超过 10 万），执行创建用户、修改密码、设置权限等操作时，性能可能会受到显著影响。这是因为每次用户更新操作都会触发全部用户数据的重新加载。
- 为了优化性能，建议在用户数量较多的场景下，将相关变量设置为 ON。
- `0` 或 `OFF`：每次执行创建用户、修改密码、设置权限等操作时，系统都会重新加载全部用户数据到内存中。在用户数量较多的场景下，此设置可能导致性能显著下降。
- `1` 或 `ON`：当设置为 `1` 或 `ON` 时，系统将采用更高效的用户数据加载策略：
    - 创建用户或对**未登录用户**执行修改密码、设置权限等操作时，系统不会重新加载全部用户数据到内存中。
    - 对**已登录用户**执行相关操作时，系统仅重新加载这些特定用户的更新数据到内存中。
