...
- 类型：布尔型
- 默认值：`OFF`
- 如果系统中创建的 SQL 用户数量过多(比如 10 万以上)，之后的创建用户、修改密码、设置权限等操作的性能会受到比较严重的影响，原因是每次用户更新，需要重新加载全部用户数据。在该场景下，建议将该变量设置为 `ON`。
    * `0` 或 `OFF`：执行创建用户、修改密码、设置权限等操作时，每次操作均会触发全部用户数据重新加载到内存。在用户数较多的场景，该设置会导致性能下降。
    * `1` 或 `ON`：
        - 创建用户、或对未登录用户执行修改密码、设置权限等操作时，均不会触发全部用户数据加载到内存。
        - 修改已登录用户的相关操作时，仅重新加载这些更新的用户数据到内存。
        - 在用户数较多的场景，性能更优。
