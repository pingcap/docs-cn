...
> **警告：**
>
> 统计信息版本 1（`tidb_analyze_version = 1`）不再支持用于新的统计信息收集。出于升级兼容性考虑，TiDB 仍会读取现有的版本 1 统计信息，但所有新的 `ANALYZE` 操作都使用统计信息版本 2（`tidb_analyze_version = 2`）。建议使用 `tidb_analyze_version = 2`。
...
- 这个变量用于控制 TiDB 收集统计信息的行为。
    - 从 v9.0.0 开始，版本 1（`tidb_analyze_version = 1`）不再支持用于新的统计信息收集。如果你尝试将此变量设置为 `1`，TiDB 会返回错误。更多信息，参见[统计信息的版本](/statistics.md#versions-of-statistics)。
    - 对于 TiDB Self-Managed，从 v5.3.0 开始，此变量的默认值已从 `1` 更改为 `2`。
    - 对于 TiDB Cloud，从 v6.5.0 开始，此变量的默认值已从 `1` 更改为 `2`。
    - 当你升级仍持久化 `tidb_analyze_version = 1` 的集群时，TiDB 会在升级期间将持久化的全局值重写为 `2`。请注意，升级后，现有的版本 1 统计信息不会自动转换为版本 2 统计信息。建议你[将使用统计信息版本 1 的现有对象迁移到版本 2](/statistics.md#switch-between-statistics-versions)。
