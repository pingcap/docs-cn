...
- 类型：布尔
- 默认值：`OFF`
- 这个变量用来控制是否强制 inline CTE。默认值为 `OFF`，即默认不强制 inline CTE，注意，此时依旧可以通过 `MERGE（）hint` 来开启个别 CTE 的 inline。如果设置为 `ON`，则当前 session 中所有查询的 CTE (recursive CTE 除外) 都会 inline 。
