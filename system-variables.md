...
> **警告：**
>
> 当前索引加速功能与 [PITR (Point-in-time recovery)](/point-in-time-recovery.md) 功能不兼容。在使用索引加速功能时，需要确保后台没有启动 PITR 备份任务，否则可能会出现非预期结果。
> 非预期情况如下：
> 如果先启动 PITR 任务，增加索引不会走快速添加模式，索引加速效果下降，不影响索引正确性。
> 如果先启动快速添加索引任务，再启动 PITR 任务会报错，启动 PITR 任务失败，正在添加索引的任务没有影响。
> 如果同时启动 PITR 任务和快速添加索引任务，会出现添加的索引，可能没有包括全量数据在此次 PiTR 备份中。
