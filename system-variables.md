...
- 在 TiDB X 中，该变量只支持设置为 `"standard"`，不支持设置为 `"bulk"`。如果设置为 `"bulk"`，设置会被拒绝，因此不能通过 `tidb_dml_type = "bulk"` 启用 Pipelined DML。
...
- 在 TiDB X 中，该变量只支持 `leader`。如果将该变量设置为其他读取模式，设置会被拒绝。
...
- 类型：枚举型
- 默认值：
    - 普通 TiDB：对于新创建的 v6.0.0 及以上集群，默认值为 `FAST`。对于升级版本的集群，如果升级前是低于 v6.0.0 的版本，升级后默认值为 `OFF`。
    - TiDB X：`STRICT`
- 可选值：`OFF`，`FAST`，`STRICT`
