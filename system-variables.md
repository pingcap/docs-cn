...
- 适用场景：所有偏 OLTP 的场景推荐开启。对于使用 `IndexLookUp` 和 `Limit` 并且 `Limit` 无法下推到 `IndexScan` 上的读请求，可能会出现读请求的延迟高、TiKV 的 Unified read pool CPU 使用率高的情况。在这种情况下，由于 `Limit` 算子只需要少部分数据，开启 `tidb_enable_paging`，能够减少处理数据的数量，从而降低延迟、减少资源消耗。此外，对于 Dumpling 数据导出或者全表扫这类的场景，开启 paging 后可以有效降低 TiDB 进程的内存消耗。
...
[`tidb_enable_paging`](/system-variables.md#tidb_max_paging_size-从-v630-版本开始引入), `tidb_min_paging_size`, [`tidb_max_paging_size`](/system-variables.md#tidb_max_paging_size-从-v630-版本开始引入) 等相关变量对 TPCH 的性能影响如上图所示，纵轴是执行时间，越小越好。
