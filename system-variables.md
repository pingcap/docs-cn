...
- 默认值：`-1`
- 范围：`-1` 或 `[1, 128]`
- 该变量定义了分布式框架任务可使用的 TiDB 节点数上限。默认值为 `-1`，表示自动模式。
- 在自动模式下，系统将按照 `min(3, tikv_nodes / 3)` 动态地计算该值，其中 `tikv_nodes` 表示集群中 TiKV 节点的数量。
- 若 `@@tidb_service_scope` 非空，则仅允许配置了相同 `@@tidb_service_scope` 的 TiDB 实例执行分布式框架子任务。
- 若集群实际可用实例数少于 `@@tidb_max_dist_task_nodes`，任务仍可正常执行，此时将使用所有可用实例。
