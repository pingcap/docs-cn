...
- 你也可以通过 Optimizer Hint `/*+ SET_VAR(TIKV_CLIENT_READ_TIMEOUT=N) */` 来设置查询语句中 TiDB 发送 TiKV RPC 读请求的超时时间。如果同时设置了 Hint 和系统变量，则 Hint 优先级高。
- 默认值 `0` 表示使用默认的超时时间（通常是 40 秒）。

> **注意：**
>
> - 常见是使用场景是，当一个普通查询的常规耗时是几毫秒，但偶尔遇到某个 TiKV 的网络不稳定或者某个 TiKV 的 I/O 抖动导致查询耗时超过一秒甚至十秒，您可以尝试通过 Optimizer Hint `/*+ SET_VAR(TIKV_CLIENT_READ_TIMEOUT=100) */` 为该查询语句设置 TiKV RPC 读请求超时时间为 100 毫秒，这样即使遇到某个 TiKV 很忙时，可以快速超时然后重新发送 RPC 请求给下一个 TiKV Region Peer 所在的 TiKV，通常两个 TiKV 同时出现 I/O 抖动的概率较低，所以该查询语句的耗时通常可以预期在几毫秒到一百零几毫秒之间。
> - 不建议将 `tikv_client_read_timeout` 的值设置的太小，否则 TiDB 集群在负载压力较大时会很容易导致请求超时，然后重试会进一步加大 TiDB 集群的压力。
