...
- 适用场景：所有偏 TP 的场景推荐开启。对于使用 `IndexLookUp` 和 `Limit` 并且 `Limit` 无法下推到 `IndexScan` 上的读请求，可能会出现读请求的延迟高、TiKV 的 Unified read pool CPU 使用率高的情况。在这种情况下，由于 `Limit` 算子只需要少部分数据，开启 `tidb_enable_paging`，能够减少处理数据的数量，从而降低延迟、减少资源消耗；对于 dumpling 数据导出，或者全表扫一类的场景，开启 paging 后可以有效降低 TiDB 进程的内存消耗。对于偏 AP 场景，并且以 TiKV 而非 TiFlash 作为存储引擎时，使用用 paging 可能导致部分场景下性能回退，用户可以考虑关闭 paging 或者通过 `tidb_min_paging_size` 和 `tidb_max_paging_size` 去调优。
...
`tidb_enable_paging`, `tidb_min_paging_size`, `tidb_max_paging_size` 等相关变量对 TPCH 的性能影响，纵轴是执行时间，越小越好。
