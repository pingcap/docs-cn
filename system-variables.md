...
- 该变量用来设置 DML 语句的执行方式。
    ...
    - `"bulk"` 表示使用批量 DML 执行方式，适合于处理因大量数据写入导致 TiDB 内存使用过多的情况。
        ...
        - 当遇到不支持或不兼容的情况时，`"bulk"` 模式会 fallback 到 `"standard"` 模式执行，并返回一条警告信息。也可通过 [`tidb_last_txn_info`](#tidb_last_txn_info-从-v409-版本开始引入) 查看 `pipelined` 字段，为 `true` 时说明是在 `"bulk"` 模式执行。
