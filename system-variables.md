...
>
> - 如果先启动 PITR 备份任务，再添加索引，此时即使索引加速功能打开，添加索引任务默认会自动回退到旧模式，即以较慢的速度添加索引。
> - 如果先启动添加索引加速任务，它会通过抛出错误的方式阻止 PITR 的日志备份任务启动，这不会影响正在添加索引的任务。在添加索引任务完成后，你需要重新启动 PITR 日志备份任务，并手动执行一次全量备份。
> - 如果同时启动 PITR 备份任务和添加索引加速任务，可能会由于两个任务无法察觉到对方而不会提示错误。此时 PITR 不会备份增加的索引数据。在添加索引任务完成后，你仍需要重新启动 PITR 日志备份任务，并手动执行一次全量备份。
