...
- 如果一个系统创建或者捕获了过多的绑定，导致绑定所使用的内存空间超过该阈值，会在日志中增加警告日志进行提示。在这种情况下，缓存中无法存放下所有可用的绑定，并且无法保证哪些绑定存在于缓存中，因此可能存在一些查询无法使用上可用绑定的情况。在这种情况下，强烈建议调大该变量的值，以此来保证所有可用的绑定能正常被使用。在变更变量值以后，需要执行命令 `admin reload bindings` 对绑定进行完全的重新加载，确保变量值变更生效。
