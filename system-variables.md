...
- 作用域：SESSION | GLOBAL
- 默认值：OFF
- 这个变量表示是否追踪聚合函数的内存使用情况。当开启该功能时，聚合函数内存会被统计，进而可能会造成整个 sql 内存统计值超阈值 [`mem-quota-query`](/tidb-configuration-file.md#mem-quota-query)，然后被 [`oom-action`](/tidb-configuration-file.md#oom-action) 定义的行为影响。
