...
- 从 v7.1.0 开始，该变量废弃，优化器将根据 [SQL 模式](/sql-mode.md) 及 TiFlash 副本的代价估算自行决定是否将查询下推至 TiFlash。需要注意的是，只有当前会话的 [SQL 模式](/sql-mode.md) 不包含 `STRICT_TRANS_TABLES` 和 `STRICT_ALL_TABLES`，即不是 `STRICT SQL Mode` 时，TiDB 才允许将包含增删改的 SQL 语句(如 `INSERT INTO SELECT`)中的读取操作下推至 TiFlash。
