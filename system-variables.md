...
- 如需对 `READ-UNCOMMITTED`，`SERIALIZABLE` 事务隔离级别进行兼容处理，请设置 [`tidb_skip_isolation_level_check`](#tidbskipisolationlevelcheck) 为 `TRUE`。请注意，这个操作将跳过 `transaction_isolation` 变量的设置检查，即使设置了错误的隔离级别，TiDB 也将不进行异常抛出。
