...
- 如果启用了该变量，且使用了 TiDB Data Migration (DM) 来迁移数据，建议将 [DM 任务配置文件](/dm/task-configuration-file-full.md#完整配置文件示例)里的 `session` 中的 `sql_require_primary_key` 设置为 `OFF`，否则会导致 DM 任务创建失败。
