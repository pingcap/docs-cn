...
### `tidb_opt_inline_cte` <span class="version-mark">从 v6.4 版本开始引入</span>

- 作用域：SESSION | GLOBAL
- 是否持久化到集群：是
- 类型：布尔
- 默认值：`OFF`
- 这个变量主要用来控制是否开启 inline CTE。默认值为 OFF，则默认不开启 inline CTE。（但依旧可以通过 MERGE（）hint 来开启个别 CTE 的 inline）。如果设置为 ON，则当前 session 中所有查询的 CTE 都会是 inline。
