...
> 6. 如果当前集群版本为 TiDB v7.6.0 - v8.2.0，升级目标版本为 v8.3.0 或更高版本，并且开启了加速建表 [`tidb_enable_fast_create_table`](/system-variables.md#tidb_enable_fast_create_table-new-in-v800) 系统变量，请先关闭该特性，等升级完成后再按需开启。否则集群中会残留部分该特性新增的元数据 KV。从 v8.3.0 开始，对该特性做了优化，升级到高更高版本，不会再产生并残留该类元数据 KV。
