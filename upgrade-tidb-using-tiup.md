...
>
> TiUP 版本 <= v1.0.8 可能无法正确获取 TiFlash 的数据目录, 需要确认 `data_dir` 与 TiFlash 配置的 `path` 值是否一致。若不一致需要如下把 TiFlash 的 `data_dir` 改成与 `path` 一致的值：
>
> 1. 运行 `tiup cluster edit-config <cluster-name>` 修改配置文件
>
