...
- Upgrade from TiDB 4.0 versions to TiDB 6.2.
- Upgrade from TiDB 5.0-5.4 versions to TiDB 6.2.
- Upgrade from TiDB 6.0 to TiDB 6.2.
- Upgrade from TiDB 6.1 to TiDB 6.2.
...
> **Note:**
>
> If your cluster to be upgraded is v3.1 or an earlier version (v3.0 or v2.1), the direct upgrade to v6.2.0 is not supported. You need to upgrade your cluster first to v4.0 and then to v6.2.0.

## Upgrade caveat

- TiDB currently does not support version downgrade or rolling back to an earlier version after the upgrade.
- For the v4.0 cluster managed using TiDB Ansible, you need to import the cluster to TiUP (`tiup cluster`) for new management according to [Upgrade TiDB Using TiUP (v4.0)](https://docs.pingcap.com/tidb/v4.0/upgrade-tidb-using-tiup#import-tidb-ansible-and-the-inventoryini-configuration-to-tiup). Then you can upgrade the cluster to v6.2.0 according to this document.
- To update versions earlier than v3.0 to v6.2.0:
    ...
    4. Upgrade the cluster to v6.2.0 according to this document.
...
1. Upgrade the TiUP version. It is recommended that the TiUP version is `1.10.3` or later.
...
2. Upgrade the TiUP Cluster version. It is recommended that the TiUP Cluster version is `1.10.3` or later.
...
> + After the upgrade, you want to use v6.2.0's default parameter values for the unmodified configuration items.
...
> **Note:**
>
> Before you upgrade the cluster to v6.2.0, make sure that the parameters you have modified in v4.0 are compatible in v6.2.0. For details, see [TiKV Configuration File](/tikv-configuration-file.md).
>
...
For example, if you want to upgrade the cluster to v6.2.0:
...
## TiDB 6.2.0 compatibility changes

- See TiDB 6.2.0 Release Notes for the compatibility changes.
