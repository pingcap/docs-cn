...
- 如果集群中包含基于旧架构实现的 TiCDC 历史版本（`v6.5.0` 到 `v8.5.5`），不建议在 TiDB 滚动升级期间持续运行 changefeed。若目标 TiDB 版本高于 `v8.1.x`，必须先升级 TiCDC。升级到 `v8.5` 时，建议按“暂停所有 changefeed -> 升级 TiCDC -> 升级 TiDB 集群 -> 恢复 changefeed”的顺序执行。更多说明请参考 [TiCDC 历史版本滚动升级兼容性说明](/ticdc/ticdc-compatibility.md#历史版本滚动升级兼容性说明)。
