...
> **注意：**
>
> - 如果原集群是 3.0 或 3.1 或更早的版本，不支持直接升级到 v8.1.0 及后续修订版本。你需要先从早期版本升级到 4.0 后，再从 4.0 升级到 v8.1.0 及后续修订版本。
...
- 使用 TiDB Ansible 管理的 4.0 版本集群，需要先按照 [4.0 版本文档的说明](https://docs.pingcap.com/zh/tidb/v4.0/upgrade-tidb-using-tiup)将集群导入到 TiUP (`tiup cluster`) 管理后，再按本文档说明升级到 v8.1.1 版本。
- 若要将 v3.0 之前的版本升级至 v8.1.1 版本：
...
以下为从 v8.0.0 升级至当前版本 (v8.1.1) 所需兼容性变更信息。如果从 v7.6.0 或之前版本升级到当前版本，可能也需要考虑和查看中间版本 [release notes](/releases/release-notes.md) 中提到的兼容性变更信息。

- TiDB v8.1.0 release notes 中的[兼容性变更](/releases/release-8.1.0.md#兼容性变更)
- TiDB v8.1.1 release notes 中的[兼容性变更](/releases/release-8.1.1.md#兼容性变更)
...
> **注意：**
>
> 升级到 v8.1.1 版本前，请确认已在 4.0 修改的参数在 v8.1.1 版本中是兼容的，可参考 [TiKV 配置文件描述](/tikv-configuration-file.md)。
...
- 集群 DDL 情况：
    ...
    - 如果你采用的是[平滑升级](/smooth-upgrade-tidb.md)，则不需要检查集群的 DDL 情况，无需等待 DDL job 完成或取消正在进行的 DDL job。但如果是升级到 v8.1.0 或更高版本，且开启了[分布式执行框架](/tidb-distributed-execution-framework.md)，建议先关闭分布式执行框架后，然后再升级，否则升级期间添加的索引可能会出现和数据不一致的问题，导致升级失败。 
...
之后通过 `upgrade` 命令添加 `--offline` 参数来进行停机升级，其中 `<cluster-name>` 为集群名，`<version>` 为升级的目标版本，例如 `v8.1.1`。
...
可以指定 `--force`，升级时会跳过 `PD transfer leader` 和 `TiKV evict leader` 过程，直接重启并升级版本，对线上运行的集群性能影响较大。命令如下，其中 `<version>` 为升级的目标版本，例如 `v8.1.1`：
