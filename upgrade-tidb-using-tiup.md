...
- TiDB v6.5.12 [release notes](/releases/release-6.5.12.md)
...
> **注意：**
>
> 升级到 6.5.12 版本前，请确认已在 4.0 修改的参数在 6.5.12 版本中是兼容的，可参考 [TiKV 配置文件描述](/tikv-configuration-file.md)。
...
```
tiup cluster upgrade <cluster-name> v6.5.12
```
...
之后通过 `upgrade` 命令添加 `--offline` 参数来进行停机升级，其中 `<cluster-name>` 为集群名，`<version>` 为升级的目标版本，例如 `v6.5.12`。
...
可以指定 `--force`，升级时会跳过 `PD transfer leader` 和 `TiKV evict leader` 过程，直接重启并升级版本，对线上运行的集群性能影响较大。命令如下，其中 `<version>` 为升级的目标版本，例如 `v6.5.12`：
