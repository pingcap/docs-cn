...
- 集群 DDL 情况：
    ...
    - 如果你没有采用[平滑升级](/smooth-upgrade-tidb.md)，建议使用 [`ADMIN SHOW DDL`](/sql-statements/sql-statement-admin-show-ddl.md) 命令查看集群中是否有正在进行的 DDL job。如需升级，请等待 DDL 执行完成或使用 [`ADMIN CANCEL DDL`](/sql-statements/sql-statement-admin-cancel-ddl.md) 命令取消该 DDL job 后再进行升级。
