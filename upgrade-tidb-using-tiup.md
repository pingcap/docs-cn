...
> 6. 如果当前集群版本为 TiDB v7.6.0 - v8.2.0，升级的目标版本为 v8.3.0 或更高版本，并且通过 [`tidb_enable_fast_create_table`](/system-variables.md#tidb_enable_fast_create_table-new-in-v800) 系统变量开启了加速建表，请先关闭加速建表功能，等升级完成后再按需开启。否则，集群中会残留该功能新增的部分元数据 KV。从 v8.3.0 开始，对该功能做了优化，升级到更高版本时，不会再产生并残留该类元数据 KV。
