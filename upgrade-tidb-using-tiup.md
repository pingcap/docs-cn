...
- 使用 TiUP 从 TiDB 4.0 版本升级至 TiDB 7.4。
- 使用 TiUP 从 TiDB 5.0-5.4 版本升级至 TiDB 7.4。
- 使用 TiUP 从 TiDB 6.0-6.6 版本升级至 TiDB 7.4。
- 使用 TiUP 从 TiDB 7.0-7.3 版本升级至 TiDB 7.4。
...
> **注意：**
>
> - 如果原集群是 3.0 或 3.1 或更早的版本，不支持直接升级到 v7.4.0 及后续修订版本。你需要先从早期版本升级到 4.0 后，再从 4.0 升级到 v7.4.0 及后续修订版本。
> - 如果原集群是 6.2 之前的版本，升级到 6.2 及以上版本时可能会遇到升级卡住的情况，你可以参考[如何解决升级卡住的问题](#42-升级到-v620-及以上版本时如何解决升级卡住的问题)。
...
- 使用 TiDB Ansible 管理的 4.0 版本集群，需要先按照 [4.0 版本文档的说明](https://docs.pingcap.com/zh/tidb/v4.0/upgrade-tidb-using-tiup)将集群导入到 TiUP (`tiup cluster`) 管理后，再按本文档说明升级到 v7.4.0 版本。
- 若要将 v3.0 之前的版本升级至 v7.4.0 版本：
...
查阅 TiDB v7.4.0 release notes 中的[兼容性变更](/releases/release-7.4.0.md#兼容性变更)。如果有任何变更影响到了你的升级，请采取相应的措施。
...
> **注意：**
>
> 升级到 v7.4.0 版本前，请确认已在 4.0 修改的参数在 v7.4.0 版本中是兼容的，可参考 [TiKV 配置文件描述](/tikv-configuration-file.md)。
...
之后通过 `upgrade` 命令添加 `--offline` 参数来进行停机升级，其中 `<cluster-name>` 为集群名，`<version>` 为升级的目标版本，例如 `v7.4.0`。
...
从 v6.2.0 开始，TiDB 改进了并发 DDL 任务处理方式。v6.2.0 及以上版本默认使用[并发 DDL 框架](/ddl-introduction.md#tidb-在线-ddl-异步变更的原理)，将 DDL 作业从 KV 队列转移到表队列。这一变化可能导致升级卡住。下面是一些常见的升级场景及解决方案：

- 加载插件导致的卡住

    升级过程中加载插件时可能需要执行 DDL 语句，此时可能会卡住。
...
- 使用 `kill -9` 命令停机升级导致的卡住
    ...
    - 应对已发生的问题：重启受影响的 TiDB 节点。如果问题刚发生，建议等待 2 分钟后再重启。
...
- DDL Owner 变更导致的卡住
    ...
    **解决方案**：
    ...
    1. 终止卡住的 TiDB 节点（避免使用 `kill -9`）。
    2. 重新启动新版本 TiDB 节点。

### 4.3 升级过程中 evict leader 等待时间过长，如何跳过该步骤快速升级

可以指定 `--force`，升级时会跳过 `PD transfer leader` 和 `TiKV evict leader` 过程，直接重启并升级版本，对线上运行的集群性能影响较大。命令如下，其中 `<version>` 为升级的目标版本，例如 `v7.4.0`：
