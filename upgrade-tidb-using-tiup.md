...
- 2.1 及之后版本启用了并行 DDL，早于 2.0.1 版本的集群，无法滚动升级到 4.0 版本，可以选择下面两种方案：
    - 停机升级，直接从早于 2.0.1 的 TiDB 版本升级到 4.0 版本，最后使用 TiUP Cluster 提供的 `import` 命令替换为使用 TiUP 管理工具。
    - 滚动升级，滚动升级到 2.0.1 或者之后的 2.0.x 版本，再滚动升级到 4.0 版本，最后使用 TiUP Cluster 提供的 `import` 命令替换为使用 TiUP 管理工具。
