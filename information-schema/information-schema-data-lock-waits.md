...
> * 由于实现限制，目前对于乐观事务被阻塞的情况 `SQL_DIGEST` 和 `SQL_DIGEST_TEXT` 字段为 `null`，如果需要知道相关的 SQL，可以将此表与 [`CLUSTER_TIDB_TRX`](/information-schema/information-schema-tidb-trx.md) 进行 `JOIN` 来获得对应事务的所有 SQL。 
