...
`PLACEMENT_RULES` 表提供所有已明确配置的 [Placement Rules in SQL](/placement-rules-in-sql.md) 信息，包括放置策略 (placement policy) 和直接放置 (directly attached rule) 信息。
...
`PLACEMENT_RULES` 表只显示明确配置的规则。如要查看 Placement Rules 放置规则的规范版本（包括附加到表格的放置策略），请改用 `SHOW PLACEMENT` 语句：
