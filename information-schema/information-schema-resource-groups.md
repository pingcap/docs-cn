...
```sql
mysql> CREATE RESOURCE GROUP rg1 RRU_PER_SEC=1000 WRU_PER_SEC=2000 ; -- 使用 RU_MODE 创建资源组 rg1
...
mysql> SELECT * FROM information_schema.placement_policies WHERE NAME = 'rg1'; -- 显示资源组 rg1 运行时状态。
+--------+---------+------+-----------+------+-----------+------+----------------+-----------------+
...
```sql
mysql> CREATE RESOURCE GROUP rg2 CPU="8" IO_READ_BANDWIDTH="1000Mi" IO_WRITE_BANDWIDTH="1000Mi"; -- 使用 RAW_MODE 创建资源组 rg2
Query OK, 0 rows affected (0.20 sec)
mysql> show create resource group rg2; -- 显示 rg2 资源组的定义。
+----------------+-----------------------------------------------------------------------------------------------+
| Resource_Group | Create Resource Group                                                                         |
+----------------+-----------------------------------------------------------------------------------------------+
...
+----------------+-----------------------------------------------------------------------------------------------+
1 row in set (0.00 sec)
mysql> SELECT * FROM information_schema.resource_groups WHERE NAME = 'rg2'; -- 显示资源组 rg2 运行时状态。
+--------+----------+------+-----------+------+-----------+------+----------------+-----------------+
| NAME   | MODE     | RRU  | RRU_TOKEN | WRU  | WRU_TOKEN | CPU  | READ_BANDWIDTH | WRITE_BANDWIDTH |
+--------+----------+------+-----------+------+-----------+------+----------------+-----------------+
