...
悲观事务假定并发事务会发生冲突，所以每一条 SQL 执行后都会检测冲突，只有在确保事务一定能够执行成功后，才开始提交。另有[乐观事务](#乐观事务)。
...
事务指一系列有限的数据库操作序列。TiDB 中的事务具有 [ACID](#ACID) 四大特性。
...
由事务控制语句定义开始和结束的事务为显式事务。无需定义事务的开始的事务为隐式事务。