...
你可以通过观察 PD 相关的 Metrics 或使用 pd-ctl 工具等方式查看调度系统状态。更具体的信息可以参考 [PD 监控](/dev/reference/key-monitoring-metrics/pd-dashboard.md)和 [PD Control](/dev/reference/tools/pd-control.md)。
...
使用 pd-ctl 可以从以下三个方面来调整 PD 的调度策略。更具体的信息可以参考 [PD Control](/dev/reference/tools/pd-control.md)。
...
- 找不到节点来转移 Region。例如相同 Label 的替代节点容量都不足 20%，PD 为了避免爆盘的风险会停止调度。这种情况需要添加更多节点，或者删除一些数据释放空间。
