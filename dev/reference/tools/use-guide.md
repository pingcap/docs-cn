...
title: TiDB 生态工具使用指南
category: reference
aliases: ['/docs-cn/dev/how-to/migrate/from-mysql/', '/docs-cn/dev/how-to/migrate/incrementally-from-mysql/', '/docs-cn/dev/how-to/migrate/overview/']
---
...
[BR](/dev/reference/tools/br/br.md) 是 TiDB 进行分布式备份恢复的命令行工具，用于对 TiDB 集群进行数据备份和恢复。相比 Mydumper 和 Loader，BR 更适合大数据量的场景，有更高效的备份和恢复效率。

以下是 BR 的一些基本信息：

- [备份输出和恢复输入的文件类型](/dev/reference/tools/br/br.md#备份文件类型)：SST + `backupmeta` 文件
