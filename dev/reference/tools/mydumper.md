...
+ To ensure backup consistency for TiDB, this optimized Mydumper tool sets the value of [tidb_snapshot](/dev/how-to/get-started/read-historical-data.md#how-tidb-reads-data-from-history-versions) to specify the point in time when the data is backed up instead of using `FLUSH TABLES WITH READ LOCK`.
