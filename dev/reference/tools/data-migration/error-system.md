...
| `code=32001` | Dump 部件异常                                              | 如果报错 `msg` 包含 `mydumper: argument list too long.`，则需要用户根据 black-white list 手动在 task.yaml 的 mydumper extra-args 参数中配置 -x 正则表达式设置要导出的库表；例如要导出所有库中表名字为 hello 的表 `-x ~*.hello`；如果要导出所有表，可使用 `-x ~*.*` 语句。 |
