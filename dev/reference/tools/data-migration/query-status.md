...
```
{
    ...
    "tasks": [      # 迁移 task 列表
        ...
        {
            "taskName": "test-3",
            "taskStatus": "Error - Relay status is Error",  # 该任务的某个 Sync 类型的子任务在同步时 Relay 插件出错
            "workers": [
                "127.0.0.1:8263",
                "127.0.0.1:8264"
            ]
...
| 任一 Sync 类型子任务处于 “Running” 状态但其 Relay 插件已停止运行（处于 Error/Paused/Stopped 状态） | Error - Relay status is Error/Paused/Stopped |
