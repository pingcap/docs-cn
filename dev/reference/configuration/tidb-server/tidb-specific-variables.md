...
这个变量用于设置当前 session 的事务模式，默认是乐观锁模式。 TiDB 3.0 加入了悲观锁模式，将 `tidb_txn_mode` 设置为 `'pessimistic'` 后，这个 session 执行的所有事务都会进入悲观事务模式。更多关于悲观锁的细节，可以参考 [TiDB 悲观事务模式](/reference/transactions/transaction-pessimistic.md)。
