...
+ SQL executor
    ...
    - Support using the [`EXPLAIN ANALYZE` statement](/dev/reference/sql/statements/explain-analyze.md) to check the runtime statistics including the execution time and the number of returned rows of each operator
...
+ Server
    ...
    - [Add the `auto_analyze_ratio` system variables to contorl the ratio of Analyze](/dev/faq/tidb.md#whats-the-trigger-strategy-for-auto-analyze-in-tidb)

    - [Add the `tidb_retry_limit` system variable to control the automatic retry times of transactions](/dev/reference/configuration/tidb-server/tidb-specific-variables.md#tidb_retry_limit)

    - [Add the `tidb_disable_txn_auto_retry` system variable to control whether the transaction retries automatically](/dev/reference/configuration/tidb-server/tidb-specific-variables.md#tidb_disable_txn_auto_retry)

    - [Support using`admin show slow` statement to obtain the slow queries](/dev/how-to/maintain/identify-abnormal-queries/identify-slow-queries.md#admin-show-slow-command)

    - [Add the `tidb_slow_log_threshold` environment variable to set the threshold of slow log automatically](/dev/reference/configuration/tidb-server/tidb-specific-variables.md#tidb_slow_log_threshold)

    - [Add the `tidb_query_log_max_len` environment variable to set the length of the SQL statement to be truncated in the log dynamically](/dev/reference/configuration/tidb-server/tidb-specific-variables.md#tidb_query_log_max_len)
...
+ Optimize the scheduler
    ...
    - [Add more commands to control the scheduling policy](/dev/reference/tools/pd-control.md#config-show--set-option-value)
