...
在多可用区相对单可用区性能对比中，因为测试时 (2019.08) GCP 没有 region 有 3 个可用区同时提供 c2 机器，我们选择了如何机器型号进行测试:
...
在单可用于多可用集群对比测试中，因为 GCP 没有区域可以同时在 3 可用区里申请
c2-standard-16 机型。我们选择 n1-standard-16 机型测试。

### 操作系统及参数

GKE 支持两种操作系统：COS 和 Ubuntu 。在 Point Select 测试中进行了[对比测试](TODO)。其余测试中，同一使用 Ubuntu 系统进行测试。
...
在使用 Point Select 测试针对不同操作系统、不通网络情况做了对比测试后，对 OLTP 测试集其他测试分别做了测试。这些测试统一使用 Ubuntu 系统、Host 模式并在集群使用 Service 访问 TiDB 集群。
...
此次测试主要将典型共有云部署 Kubernetes 运行 TiDB 集群的几种场景使用 sysbench 做了测试，了解不同因素可能带来的影响。从整体看，主要有以下几点：
...
但要注意的是，这些因素可能随着时间变化，不同公有云下的表现可能会略有不同。同时，sysbench 测试用例并不能代表真实应用场景，在做选择前需做实际生产测试，并综合不通选择背后的代价（操作系统差异、Host 网络的限制等）。
