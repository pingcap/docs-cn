...
4. Login the newly added PD node and edit the starting script:
    ...
    3. Manually start the PD service in the newly added PD node:
        ...
        > **Note:**
        >
        > Before start, you need to ensure that the `health` status of the newly added PD node is "true", using [PD Control](/dev/reference/tools/pd-control.md). Otherwise, the PD service might fail to start and an error message `["join meet error"] [error="etcdserver: unhealthy cluster"]` is returned in the log.
