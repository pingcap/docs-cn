...
## 第 1 步：从系统中导出 Parquet 文件
从 Aurora 中导出 Parquet 文件请参照[从 Amazon Aurora 迁移数据到 TiDB](/migrate-aurora-to-tidb.md)。本文主要介绍如何从 Hive 中导出 Parquet 文件。

Hive 中每个表都能通过标注：`stored as parquet location '/path/in/hdfs'` 的形式将表数据导出到 Parquet 文件中。利用这个特点，如果我们需要导出一张名叫 test 的表，只需要在 Hive 中输入：
```sql
create table temp stored as parquet location '/path/in/hdfs'
as select * from test;
```
这样，表数据就成功导出到了 hdfs 系统里。接下来，可以使用 `hdfs -get` 命令把这个 Parquet 文件导出到本地：
```shell
hdfs dfs -get /path/in/hdfs /path/in/local
``` 
在 Hive 导出的 Parquet 文件可能不带有 `.parquet` 的后缀，这样可能无法被 Lightning 正确识别。因此，在进行导入之前，还需要对导出的文件进行重命名，加上这个后缀。

在完成导出之后，如果需要将 hdfs 里导出的 Parquet 删掉，可以直接将这个临时表删掉：
```
drop table test;
```
...
将所有 Parquet 文件放到统一目录下，例如 `/data/my_datasource/` 或 `s3://my-bucket/sql-backup`。Lightning 将递归地寻找该目录下及其子目录内的所有 `.parquet` 文件。

## 第 3 步：定义目标表结构

要导入 TiDB，就必须为 Parquet 文件提供表结构。导出的数据可以通过以下任一方法创建表结构：
