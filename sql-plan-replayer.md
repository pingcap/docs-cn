...
导出的 ZIP 文件结构取决于你导出的是单条 SQL 语句还是多条 SQL 语句：

- 对于单条 SQL 语句，ZIP 文件包含 `sql/sql0.sql` 和 `explain.txt`。
- 对于多条 SQL 语句，ZIP 文件包含 SQL 文件，例如 `sql/sql0.sql` 和 `sql/sql1.sql`，对应的 explain 文件，例如 `explain/explain0.txt` 和 `explain/explain1.txt`，以及所引用表的表结构和统计信息文件。
