...
[表](/develop/dev-guide-schema-design-overview.md#表-table)是集群中的一种逻辑对象，它从属于[数据库](/develop/dev-guide-schema-design-overview.md#数据库-database)。其用于保存从 SQL 中发送而来的数据。表以行和列的形式组织数据记录。一个表有至少一个列，每行数据都拥有所有列的数据。
