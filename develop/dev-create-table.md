...
此页面提供了一个创建表的最佳实践指南，并提供了一个基于 TiDB 的 [bookshop](/develop/dev-bookshop-schema-design.md) 数据库的示例。
...
- [创建一个数据库](/develop/dev-create-database.md)。

## 创建表

[表](/develop/dev-schema-design-overview.md#表-table)是集群中的一种逻辑对象，用于存储从应用程序的持久层或其他 SQL 中发送的数据。表以行和列的形式组织数据记录。
...
- 请使用有意义的表名，例如，若你需要创建一个用户表，你可以使用名称：`user`, `t_user`, `users` 等，或遵循你公司或组织的命名规范。如果你的公司或组织没有相应的命名规范，可参考[表命名规范](/develop/dev-object-naming-guidelines.md#表命名规范)。请勿使用这样的表名，如：`t1`, `table1` 等。
...
- 请使用有意义的列名，我们推荐你遵循公司或组织的表命名规范。如果你的公司或组织没有相应的命名规范，可参考[列命名规范](/develop/dev-object-naming-guidelines.md#字段命名规范)。
...
> **注意：**
>
> 如果你的集群，不包含 TiFlash 节点，此 SQL 语句将会报错：`1105 - the tiflash replica count: 1 should be less than the total tiflash server count: 0` 你可以[使用 TiDB Cloud (DevTier) 构建 TiDB 集群](/develop/dev-build-cluster-in-cloud.md#第-1-步创建免费集群) 来创建一个含有 TiFlash 的免费集群。
...
- 遵循 SQL 开发规范中的[建表删表规范](/develop/dev-sql-development-specification.md#建表删表规范)，建议业务应用内部封装建表删表语句增加判断逻辑。

#### `CREATE TABLE` 执行的示例

按以上步骤创建所有表后，我们的 `dbinit.sql` 文件应该类似于[数据库初始化](/develop/dev-bookshop-schema-design.md#数据库初始化-dbinitsql-脚本)所示。若需查看表信息详解，请参阅[数据表详解](/develop/dev-bookshop-schema-design.md#数据表详解)。
...
请注意，到目前为止，我们所创建的所有表都不包含二级索引。添加二级索引的指南，请参考[创建二级索引](/develop/dev-create-secondary-indexes.md)。
