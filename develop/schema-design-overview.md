...
表包含数据**行**。每行数据中的每个值都属于一个特定的**列**。每列都只允许单一数据类型的数据值。列可添加[约束](/constraints)来进一步限定。你还可以添加[生成列（实验特性）](/generated-columns)用于计算。
...
|                         [列式存储 (TiFlash)](/tiflash/tiflash-overview)                         |    Y     |    Y     |    Y     |    Y     |    Y     |    Y     |
|                            [RocksDB 引擎](/storage-engine/rocksdb-overview.md)                            |    Y     |    Y     |    Y     |    Y     |    Y     |    Y     |
|                              [Titan 插件](/storage-engine/titan-overview)                              |    Y     |    Y     |    Y     |    Y     |    Y     |    Y     |
|                         [不可见索引](/sql-statements/sql-statement-add-index.md)                          |    Y     |    Y     |    Y     |    Y     |    Y     |    N     |
|              [复合主键](/constraints#%E4%B8%BB%E9%94%AE%E7%BA%A6%E6%9D%9F)              |    Y     |    Y     |    Y     |    Y     |    Y     |    Y     |
|              [唯一约束](/constraints#%E5%94%AF%E4%B8%80%E7%BA%A6%E6%9D%9F)              |    Y     |    Y     |    Y     |    Y     |    Y     |    Y     |
|                          [整型主键上的聚簇索引](/constraints)                           |    Y     |    Y     |    Y     |    Y     |    Y     |    Y     |
|                      [复合或非整型主键上的聚簇索引](/constraints)                       |    Y     |    Y     |    Y     |    Y     |    Y     |    N     |

## 其他对象

TiDB 支持一些和**表**同级的对象：

- [视图](/views): 视图是一张虚拟表，该虚拟表的结构由创建视图时的 `SELECT` 语句定义，TiDB 目前不支持物化视图。
- [序列](/sql-statements/sql-statement-create-sequence.md): 创建和存储顺序数据。
- [临时表](/temporary-tables): 临时表是数据不持久化的表。

## 访问控制

TiDB 支持基于用户或角色的访问控制。你可以通过[角色](/role-based-access-control)或直接指向[用户](/user-account-management)，从而授予**用户**查看、修改或删除 数据对象和数据模式的[权限](/privilege-management)。
...
此处摘录一些常见的对象大小限制，详细使用限制请查阅[此文档](/tidb-limitations)。
...
|     分区数     |                                                                                                                   8192                                                                                                                    |
|    单行大小    | 默认为 6MB，可通过 [txn-entry-size-limit](/tidb-configuration-file#txn-entry-size-limit-span-classversion-mark%E4%BB%8E-v50-%E7%89%88%E6%9C%AC%E5%BC%80%E5%A7%8B%E5%BC%95%E5%85%A5span) 配置项调整 |
| 单行内单列大小 |                                                                                                                    6MB                                                                                                                    |
