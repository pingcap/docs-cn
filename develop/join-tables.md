...
TiDB 在 SQL 语法层面上不支持 `LEFT SEMI JOIN table_name`，但是在执行计划层面，[子查询相关的优化](/subquery-optimization)会将 `semi join` 作为改写后的等价 JOIN 查询默认的连接方式。
...
- [Index Join](/explain-joins#index-join)
- [Hash Join](/explain-joins#hash-join)
- [Merge Join](/explain-joins#merge-join)

如果发现 TiDB 的优化器没有按照最佳的 Join 算法去执行。你也可以通过 [Optimizer Hints](/optimizer-hints) 强制 TiDB 使用更好的 Join 算法去执行。
...
- [INL_JOIN(t1_name [, tl_name ...])](/optimizer-hints#inl_joint1_name--tl_name-)
- [INL_HASH_JOIN(t1_name [, tl_name ...])](/optimizer-hints#inl_hash_join)
- [HASH_JOIN(t1_name [, tl_name ...])](/optimizer-hints#hash_joint1_name--tl_name-)
...
关于该算法的实现细节和限制你可以通过查看[Join Reorder 算法简介](/join-reorder)章节进行了解。

## 扩展阅读

- [用 EXPLAIN 查看 JOIN 查询的执行计划](/explain-joins)
- [Join Reorder 算法简介](/join-reorder)
