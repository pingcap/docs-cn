...
需要注意的是，设定的时间戳或时间戳的范围不能过早或晚于当前时间。此外 `NOW()` 默认精确到秒，当精度要求较高时，需要添加参数，例如 `NOW(3)` 精确到毫秒，可以参考[文档](https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_now)
