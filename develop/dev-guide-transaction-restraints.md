...
- 最大单行记录容量为 120 MiB。
    ...
    - 注意，TiKV 也有类似的限制。若单个写入请求的数据量大小超出 [`raftstore.raft-entry-max-size`](/tikv-configuration-file.md#raft-entry-max-size)（默认值为 8 MiB），TiKV 会拒绝处理该请求。当表的一行记录较大时，需要同时调整这两个限制。
