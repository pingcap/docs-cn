...
在使用该函数时，你需要将 `${tidb_host}`、`${tidb_port}`、`${tidb_user}`、`${tidb_password}`、`${tidb_db_name}` 等替换为你的 TiDB 集群的实际值。因为 TiDB Cloud Starter 要求使用 TLS (SSL) connection，因此在连接到 TiDB Cloud Starter 时 `${use_ssl}` 的值应为 `true`。
