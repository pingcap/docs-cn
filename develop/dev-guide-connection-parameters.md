...
但是 [`wait_timeout`](/system-variables.md#wait_timeout) 的默认值比较大，在事务已启动但未提交或回滚的情况下，可能需要更细粒度的控制和更短的超时，以避免持有锁的时间过长。TiDB 在 v7.6.0 引入了 [`tidb_idle_transaction_timeout`](/system-variables.md#tidb_idle_transaction_timeout) 用来控制用户会话中事务的空闲超时。
