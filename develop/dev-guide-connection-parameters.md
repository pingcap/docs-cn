...
TiDB 提供两个与 MySQL 兼容的超时控制参数，**wait_timeout** 和 **max_execution_time**。这两个参数分别控制与 Java 应用连接的空闲超时时间和连接中 SQL 执行的超时时间，即控制 TiDB 与 Java 应用的连接最长闲多久和最长忙多久。**wait_timeout** 参数在 TiDB 5.4 版本及以后，默认值为 28800 秒（即 8 小时），在此之前，默认值为 0，即默认允许链接空闲无限长的时间。而 **max_execution_time** 参数的默认值为 0，即默认允许一个 SQL 语句执行无限的长的时间。
