...
在实际的使用当中，请根据具体的[场景](/stale-read#%E5%9C%BA%E6%99%AF%E6%8F%8F%E8%BF%B0)判断是否适合在 TiDB 当中开启 Stale Read 功能。如果你的应用程序不能容忍读到非实时的数据，请勿使用 Stale Read，否则读到的数据可能不是最新成功写入的数据。
...
过期的数据在 TiDB 当中会由[垃圾回收器](/garbage-collection-overview)进行回收，数据在被清除之前会被保留一小段时间，这段时间被称为 [GC Life Time (默认 10 分钟)](/system-variables#tidb_gc_life_time-span-classversion-mark%E4%BB%8E-v50-%E7%89%88%E6%9C%AC%E5%BC%80%E5%A7%8B%E5%BC%95%E5%85%A5span)。每次进行 GC 时，将以当前时间减去该时间周期的值作为 **GC Safe Point**。如果尝试读取 GC Safe Point 之前数据，TiDB 会报如下错误：
...
- [Stale Read 功能的使用场景](/stale-read)
- [使用 AS OF TIMESTAMP 语法读取历史数据](/as-of-timestamp#%E8%AF%AD%E6%B3%95%E6%96%B9%E5%BC%8F)
- [通过系统变量 tidb_read_staleness 读取历史数据](/tidb-read-staleness)
