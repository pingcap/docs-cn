...
- 数据导出工具：[Dumpling](/dumpling-overview)。可以导出 MySQL 或 TiDB 的数据到本地或 Amazon S3 中。
- 数据导入工具：[TiDB Lightning](/tidb-lightning/tidb-lightning-overview)。可以导入 `Dumpling` 导出的数据、CSV 文件，或者 [Amazon Aurora 生成的 Apache Parquet 文件](/migrate-aurora-to-tidb)。同时支持在本地盘或 [Amazon S3 云盘](/br/backup-and-restore-storages)读取数据。
- 数据同步工具：[TiDB Data Migration](/dm/dm-overview)。可同步 MySQL、MariaDB、Amazon Aurora 数据库到 TiDB 中。且支持分库分表数据库的迁移。
...
更多有关热点问题的处理办法，请参考 [TiDB 热点问题处理](/troubleshoot-hot-spot-issues)文档。
