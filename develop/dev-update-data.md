...
- 若需使用 `UPDATE` 语句更新数据，需先[插入数据](/develop/dev-insert-data.md)
...
假设某位作者改名为 Helen Haruki，需要更改我们的 [authors](/develop/dev-bookshop-schema-design.md#authors-表) 表。假设他的唯一标识 `id` 为 1，即过滤器应为：`id = 1`。
...
例如，我们需要更新 [ratings](/develop/dev-bookshop-schema-design.md#ratings-表) 表来写入用户对书籍的评价，如果用户还未评价此书籍，将新建一条评价，如果用户已经评价过，那么将会更新他之前的评价。
