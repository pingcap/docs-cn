...
- 阅读[数据库模式概览](/develop/dev-schema-design-overview.md)，并[创建数据库](/develop/dev-create-database.md)、[创建表](/develop/dev-create-table.md)、[创建二级索引](/develop/dev-create-secondary-indexes.md)。
...
有关如何使用此 SQL，可查阅[连接到 TiDB 集群](/develop/dev-build-cluster-in-cloud.md#第-2-步连接到集群)文档部分，按文档步骤使用客户端连接到 TiDB 集群后，输入 SQL 语句即可。
...
在设计表时需要考虑是否存在大量插入行为，若有，需在表设计期间对热点进行规避。请查看 [创建表 - 选择主键](/develop/dev-create-table.md#选择主键) 部分，并遵从 [主键选择的最佳实践](/develop/dev-create-table.md#主键选择的最佳实践)。

更多有关热点问题的处理办法，请参考 [TiDB 热点问题处理](/troubleshoot-hot-spot-issues.md)文档。

## 主键为 `AUTO_RANDOM` 表插入数据

在我们插入的表主键为 `AUTO_RANDOM` 时，这时默认情况下，不能指定主键。例如 [bookshop](/develop/dev-bookshop-schema-design.md) 数据库中，我们可以看到 [users 表](/develop/dev-bookshop-schema-design.md#users-表) 的 `id` 字段含有 `AUTO_RANDOM` 属性。
...
在 TiDB 中，使用 HTAP 能力无需你在插入数据时进行额外操作。不会有任何额外的插入逻辑，由 TiDB 自动进行数据的一致性保证。你只需要在创建表后，[开启列存副本同步](/develop/dev-create-table.md#使用-htap-能力)，就可以直接使用列存副本来加速你的查询。
