...
- [Model constructor] 不支持`sequelize.queryInterface.showIndex(Model.tableName);`。
- [Model references] 不支持`sequelize.options.databaseVersion`。
- [QueryInterface addColumn] 不支持使用`queryInterface.addColumn`添加外键引用。
