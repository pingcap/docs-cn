...
1. 在 {{{ .starter }}} 实例的概览页面，点击右上角的 **Connect**，然后在弹出的对话框中获取你的数据库连接字符串。连接字符串格式如下：
...
3. 使用 Prisma schema 同步你的数据库。你可以手动在 {{{ .starter }}} 实例中创建数据库表，也可以使用 Prisma CLI 自动创建：

    ```
    npx prisma db push
    ```

   该命令会通过传统的 TCP 连接在 {{{ .starter }}} 实例中创建 `user` 表，而不是通过 `@tidbcloud/prisma-adapter` 的 HTTPS 连接。这是因为它使用了与 Prisma Migrate 相同的引擎。关于该命令的更多信息，请参见 [Prototype your schema](https://www.prisma.io/docs/concepts/components/prisma-migrate/db-push)。
