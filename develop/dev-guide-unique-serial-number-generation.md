...
最后，需要注意的是，上述两种方案生成的 ID 都不够随机，不适合直接作为 TiDB 表的主键。实际使用过程中可以对生成的 ID 进行位反转（bit-reverse）后得到一个较为随机的新 ID。例如，经过位反转后，ID `00000010100101000001111010011100` 变成了 `00111001011110000010100101000000` ， ID `11111111111111111111111111111101` 变成了 `10111111111111111111111111111111` 。
