...
> **注意：**
>
> 本文档适用于 TiDB Cloud Starter、TiDB Cloud Dedicated 以及本地部署的 TiDB。
...
- （推荐方式）参考[创建 TiDB Cloud Starter 集群](/develop/dev-guide-build-cluster-in-cloud.md#第-1-步创建-tidb-cloud-集群)，创建你自己的 TiDB Cloud 集群。
...
6. 编辑 `.env` 文件，按照以下方式设置 `DATABASE_URL` 环境变量，并将占位符 `{}` 替换为连接对话框中相应的连接参数：
   ...
   > **注意**
   >
   > 对于 TiDB Cloud Starter，当使用 Public Endpoint 时，必须使用 `ssl_mode=verify_identity` 查询参数启用 TLS 连接。
...
> **注意**
>
> 对于 TiDB Cloud Starter，当使用 Public Endpoint 时，**必须**通过在 `DATABASE_URL` 中设置 `ssl_mode` 查询参数为 `verify_identity` 来启用 TLS 连接，但是你**不需要**通过 `DATABASE_URL` 指定 SSL CA 证书，因为 mysql2 gem 会按照特定的顺序搜索现有的 CA 证书，直到找到相应的文件。
