...
* 处理方法：
    ...
    * 观察 [TIKV Details 面板](/grafana-tikv-dashboard.md#pd) 中的 PD 监控信息，查看 'Store Slow Score' 监控指标，找出指标数超过 80 的节点，该节点即为被检测到的慢节点。
    *  观察 [Raft IO dashboard](/grafana-tikv-dashboard.md#raft-io)，看延迟是否升高。如果延迟很高，表明磁盘可能有瓶颈。一个能缓解但不怎么安全的办法是将 sync-log 改成 false。
    * 调大 TiKV `raftstore.inspect-interval` 参数，提高延迟检测的超时上限。
    * 如果需要更进一步的分析报警的 TiKV 节点的性能问题，找到优化方法，可以参考 [性能分析和优化](/performance-tuning-methods.md#storage-async-write-durationstore-duration-%E5%92%8C-apply-duration)。
...
* 处理方法：
    ...
    1. 观察 [Raft Propose dashboard](/grafana-tikv-dashboard.md#raft-propose)，看这个报警的 TiKV 节点是否明显有比其他 TiKV 高很多。如果是，表明这个 TiKV 上有热点，需要检查热点调度是否能正常工作。
    2.  观察 [Raft IO dashboard](/grafana-tikv-dashboard.md#raft-io)，看延迟是否升高。如果延迟很高，表明磁盘可能有瓶颈。一个能缓解但不怎么安全的办法是将 `sync-log` 改成 `false`。
    3. 观察 观察 [Raft Process dashboard](/grafana-tikv-dashboard.md##raft-process)，看 tick duration 是否很高。如果是，需要在 `[raftstore]` 配置下加上 `raft-base-tick-interval = “2s”`。
...
* 处理方法：
    ...
    1. 观察 [Raft Propose dashboard](/grafana-tikv-dashboard.md#raft-propose)，看这个报警的 TiKV 节点的 ‘99% Propose wait duration per server’ 是否明显有比其他 TiKV 高很多。如果是，表明这个 TiKV 上有热点，需要检查热点调度是否能正常工作。
    2. 观察 [Raft IO dashboard](/grafana-tikv-dashboard.md#raft-io)，看延迟是否升高。如果延迟很高，表明磁盘可能有瓶颈。一个能缓解但不怎么安全的办法是将 sync-log 改成 false。 
    3. 如果需要更进一步的分析报警的 TiKV 节点的性能问题，找到优化方法，可以参考 [性能分析和优化](/performance-tuning-methods.md#storage-async-write-durationstore-duration-%E5%92%8C-apply-duration)。
