...
| **使用限制** | <ul><li>如果导出后计划往非 TiDB 的数据库恢复，建议使用 Dumpling。</li><li>如果导出后计划往另一个 TiDB 恢复，建议使用 [BR](/br/backup-and-restore-tool.md)。 </li></ul> |

## [TiDB Lightning](/tidb-lightning/tidb-lightning-overview.md)

| 使用场景 | <span style="font-weight:normal">用于将数据全量导入到 TiDB。</span> |
|---|---|
| **上游（或输入源文件）** | <ul><li>Dumpling 输出的文件</li><li>CSV 文件</li><li>从 Hive 或者 Aurora 导出的 Parquet 文件</li><li>从本地盘或 Amazon S3 云盘读取数据</li></ul>|
| **下游** | TiDB |
...
|---|---|
| **上游** | TiDB |
| **下游（或输出文件）** | SST，backup.meta 文件，backup.lock 文件 |
...
|---|---|
| **上游** | TiDB |
| **下游（或输出文件）** | TiDB，MySQL，Kafka，增量备份文件 |
| **主要优势** | <ul><li>支持实时备份和恢复。</li><li>备份 TiDB 集群数据，同时可以用于 TiDB 集群故障时恢复。</li></ul> |
| **使用限制** | 与部分 TiDB 版本不兼容，不能一起使用。推荐使用 [TiCDC](/ticdc/ticdc-overview.md) 替代 TiDB Binlog。|
