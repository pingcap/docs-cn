...
- 当 `worker-count > 1` 时，不支持表路由。如果在包含外键的表上使用表路由，请将 `worker-count` 设置为 `1`。
...
- 当 `UPDATE` 修改主键或唯一键值时，在安全模式下 `ON UPDATE CASCADE` 无法被正确同步。DM 会将此类语句改写为 `DELETE` + `REPLACE`，从而触发 `ON DELETE` 行为而不是 `ON UPDATE` 行为。在这种情况下，DM 会拒绝该语句并暂停任务。未修改键值的 `UPDATE` 语句可以被正确同步。
