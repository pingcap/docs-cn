...
- **多 worker 因果关系**：当 `foreign_key_checks=1` 且 `worker-count > 1` 时，DM 会在任务启动时从下游 schema 中读取外键关系并注入因果键。这可确保父表行的 DML 操作先于其依赖的子表行操作完成，从而在多个 worker 之间保持 binlog 顺序。从 v8.5.7 开始，在此模式下，DM 支持静态的一对一路由，例如 schema 或表重命名。

> **注意：**
>
> 在迁移包含外键的表之前，请确保上游 schema 使用标准的外键定义。例如，子表外键应引用父表中的完整主键或唯一键。非标准定义（例如仅引用复合主键的一部分）可能会导致上游数据库中的级联行为不一致。DM 不会在同步过程中修正此类 schema 问题。

同步包含外键约束的表存在以下限制：

- 当你在安全模式下启用 `foreign_key_checks=1` 时，DM 不支持修改主键或唯一键值的 `UPDATE` 语句。DM 会暂停任务，并报错：`safe-mode update with foreign_key_checks=1 and PK/UK changes is not supported`。如需同步此类语句，请将 `safe-mode` 设置为 `false`。
- 当 `foreign_key_checks=1` 时，DM 不支持在同步过程中执行创建、修改或删除外键约束的 DDL 语句。
- 从 v8.5.7 开始，当 `foreign_key_checks=1` 且 `worker-count > 1` 时，DM 仅支持对包含外键的表使用静态一对一路由规则。在此模式下，DM 仍会拒绝将任务中的多个源表映射到同一个目标表的路由规则。对于非一对一路由，请将 `worker-count` 设置为 `1` 或修改路由规则。
- 当 `foreign_key_checks=1` 时，DM 不支持 `syncer.compact` 或 `syncer.multiple-rows`，因为这些选项可能会改变 DML 语句边界和外键执行语义。
- 当 `foreign_key_checks=1` 且 `worker-count > 1` 时，DM 会拒绝可能改变外键因果关系语义的热更新配置，包括更新 `worker-count`、`case-sensitive`、路由规则、`block-allow-list` 或 `black-white-list` 规则、binlog 事件过滤规则以及 `foreign_key_checks`。如需更改这些设置，请停止任务并使用修改后的配置重新启动任务。
- 当 `foreign_key_checks=1` 时，`block-allow-list` 必须包含外键依赖链中的所有祖先表。如果过滤掉祖先表，DM 会在增量复制期间报错并暂停任务。
- 当 `foreign_key_checks=1` 时，源端与下游的外键元信息必须保持一致。如果 DM 检测到不一致，请运行 `binlog-schema update --from-target` 以重新同步元信息。
- 当你在安全模式下启用 `foreign_key_checks=1` 时，如果某个 `UPDATE` 修改了主键或唯一键值，DM 无法正确同步 `ON UPDATE CASCADE`。这是因为 DM 会将此类语句改写为 `DELETE` + `REPLACE`，从而触发 `ON DELETE` 行为而不是 `ON UPDATE` 行为。在这种情况下，DM 会拒绝该语句并暂停任务。DM 可以正确同步未修改键值的 `UPDATE` 语句。
