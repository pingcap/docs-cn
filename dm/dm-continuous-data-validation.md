...
5. 如果当前配置是 full mode，会将变动行和下游数据库中获取的行数据进行每列对比；如果配置的是 fast mode，则只会判断这一行是否还存在：
    ...
    - 而对于某些已经在很长时间（由用户定义）内都没有校验成功的行，则将这些行定义为 `error row`，写入到下游的 meta 库中，用户可以通过查询迁移任务信息来获取错误行的数量等信息。详见[查看增量校验的状态](#查看增量校验的状态)以及[处理增量校验错误](#处理增量校验错误)。
## 当前版本使用限制
