...
本文介绍了增量数据校验的原理，以及如何使用增量数据校验功能。使用 TiDB Data Migration (DM) 开启增量校验功能的场景中的一些功能特性以及限制。
...
5. 如果当前配置是 full mode，会将变动行和下游数据库中获取的行数据进行每列对比；如果当前配置是 fast mode，则只会判断这一行是否还存在。
    - 如果校验成功，则将该行从内存中删除。
