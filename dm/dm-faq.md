...
- 方法二：
    ...
    2. 下载 [tidb-community-server 离线镜像包](https://download.pingcap.org/tidb-community-server-v6.0.0-linux-amd64.tar.gz)，并进行解压，将其中的 grafana-{version}-**.tar.gz 文件解压后，用解压出的 public/ 文件夹替换前面所描述的文件夹，运行 `tiup dm restart $cluster_name -R grafana` 重启 grafana 服务监控。
