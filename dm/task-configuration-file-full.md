...
syncers:                             # sync 处理单元的运行配置参数
  global:                            # 配置名称
    ...
    safe-mode: false
    # 安全模式的持续时间
    # 如 不设置或者设置为 ""，则默认为 `checkpoint-flush-interval`（默认为 30s）的两倍，即 60s。
    # 如 设置为 "0s"，则在 DM 自动进入安全模式的时候报错。
    # 如 设置为 "1m30s"，则在该任务异常暂停时，记录 safemode_exit_point 失败，或是 DM 进程异常退出时，把安全模式持续时间调整为 1 分 30 秒。详情可见 https://docs.pingcap.com/zh/tidb/stable/dm-safe-mode#自动开启。
