...
当 TiKV `--store` 指定的路径没有数据文件夹时，用之前参数重新启动就会报这个错误。请尝试用 pdctl 的 [store 删除](https://github.com/pingcap/pd/tree/master/pdctl#store-delete-)功能，删除之前的 store, 然后重新启动 TiKV 即可。
