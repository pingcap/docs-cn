...
* 参考 [TiDB Operator](/v3.1/tidb-in-kubernetes/deploy/tidb-operator.md) 完成集群中的 TiDB Operator 部署；
...
另外 TiDB 集群不同组件对磁盘的要求不一样，所以部署集群前要根据当前 Kubernetes 集群支持的存储类型以及使用场景为 TiDB 集群各组件选择合适的存储类型，通过修改 `values.yaml` 中各组件的 `storageClassName` 字段设置存储类型。关于 Kubernetes 集群支持哪些[存储类型](/v3.1/tidb-in-kubernetes/reference/configuration/storage-class.md)，请联系系统管理员确定。

> **注意：**
>
> 如果创建集群时设置了集群中不存在的存储类型，则会导致集群创建处于 Pending 状态，需要[将集群彻底销毁掉](/v3.1/tidb-in-kubernetes/maintain/destroy-tidb-cluster.md)。
...
其它更多配置参数请参考 [TiDB 集群部署配置文档](/v3.1/tidb-in-kubernetes/reference/configuration/tidb-cluster.md)。
