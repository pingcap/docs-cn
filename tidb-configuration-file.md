...
+ 默认值：true
+ 当该配置项值以及 `enable-global-kill` 为 `true` 时，生成 32 位 connection ID，从而支持在 MySQL 命令行中通过 <kbd>ctrl</kbd>+<kbd>c</kbd> 终止查询或连接。注意，当集群中 TiDB 实例数量超过 2048 或者单个 TiDB 实例的同时连接数超过 1048576 后，由于 32 位 connection ID 空间不足，会自动升级 64 位 connection ID。升级过程业务以及已建立的连接不受影响，但后续的新建连接将无法用过 MySQL 命令行 <kbd>ctrl</kbd>+<kbd>c</kbd> 终止。
