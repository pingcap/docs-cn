...
+ 对于使用 [`bulk` 方式](/system-variables.md#tidb_dml_type-从-v800-版本开始引入)执行的事务，最大 TTL 可以超过该配置的限制，为 24 小时和该配置项的较大者。
...
+ 当一个 session 变量 `tidb_dml_type` 设置为 `"bulk"` 时，该配置项在该 session 中效果等于设置为 `false`。
