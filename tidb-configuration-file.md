...
+ 在 v6.5.0 及之后的版本中，不再推荐使用该配置项，事务的内存大小会被累计入所在会话的内存使用量中，并由 [tidb_mem_quota_query](/system-variables.md#tidb_mem_quota_query) 变量在单个会话内存超阈值时采取控制行为。为了向前兼容，由低版本升级至 v6.5.0 及更高版本时，该配置项的行为如下所述:
	+ 若该配置项设置值不等于默认值（104857600），升级前后该配置项对单个事务大小的限制行为不会发生变化，该变量仍会生效，事务内存大小不由 tidb_mem_quota_query 控制。
	+ 若该配置未被设置，或设置值等于默认值（104857600），升级后事务内存大小将会累计入所在会话的内存使用量中，由[tidb_mem_quota_query 变量控制。
