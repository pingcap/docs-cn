...
+ 如果 tidb 写日志卡住，超过这里配置的超时时间，则让 tidb 进程崩溃
+ 默认值: 0 (0 表示不设置超时)
+ 单位：秒
+ 在一些用户场景，tidb 的日志所在的磁盘是热插拔的盘或者通过网络挂载的盘，磁盘设备可能会永久丢失，这种场景下 tidb 无法自动恢复，会在写日志操作上永久阻塞。tidb 进程看起来还活着，但是又不响应请求。这项配置用于处理这样的场景。
