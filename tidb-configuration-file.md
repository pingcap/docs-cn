...
> - 在 v8.5.3 到 v8.5.6 及其之间的版本中，TiDB 重新引入遥测功能，但其行为已更改为仅将遥测相关信息输出到日志文件，不再通过网络发送给 {{{ .company }}}。
...
> **警告：**
>
> 该配置项仅用于维护目的，请勿在处理常规工作负载的 TiDB 节点上启用。
...
+ `label` 以键值对的形式绑定一些属性，我们把这些属性叫做标签 (label)。
+ tidb-server 通过绑定标签，目前可用于 [在三数据中心下就近读取数据](/best-practices/three-dc-local-read.md)
