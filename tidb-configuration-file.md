...
+ 是否开启 TiDB 遥测功能。
+ 默认值：true
+ 如果所有 TiDB 实例上该选项都设置为 `false`，那么将完全禁用 TiDB 遥测功能，且忽略 [`tidb_enable_telemetry`](/tidb-specific-system-variables.md#tidb_enable_telemetry) 系统变量。参阅[遥测](/telemetry.md)章节了解该功能详情。
