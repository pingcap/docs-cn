...
> **注意：**
>
> 1. 标签 `zone` 在 TiDB 中具有特殊用途，用于指定服务器的所在的区域信息，当设置此标签为非空值时，对应的值会被自动用于 [`txn-score`](/system-variables.md#txn_scope) 和 [`Follower read`](/follower-read.md) 等功能。
> 2. 标签 `group` 在 TiDB-Operator 中具有特殊用途，如果使用 TiDB-Operator 部署集群，最好不要手动指定此标签。
