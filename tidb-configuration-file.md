...
+ 默认值：false
+ 当 `force-init-stats` 为 `true` 时，TiDB 启动时会等到统计信息初始化完成后再对外提供服务。在表和分区数量较多的情况下，将 `force-init-stats` 设置为 `true` 可能会让 TiDB 从启动到开始对外提供服务的时间变长。当 `force-init-stats` 为 `false` 时，TiDB 在统计信息初始化未完成时也可以对外提供服务，但在统计信息初始化未完成时，优化器会用 pseudo stats 进行决策，有可能产生不合理的执行计划。
