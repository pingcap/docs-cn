...
- 使用 TiCDC 同步单行 size 比较大 (> 1k) 的宽表时候，推荐设置 TiCDC 参数 [per-table-memory-quota](/ticdc/ticdc-server-config.md)，使得 per-table-memory-quota = ticdcTotalMemory / (tableCount * 2)，ticdcTotalMemory 是一个 TiCDC 节点的内存，tableCount 是希望一个 TiCDC 节点同步的表的数量
