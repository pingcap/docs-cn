...
- 暂不支持 TiDB 4.0 [SEQUENCE DDL](/sql-statements/sql-statement-create-sequence/) 和 [SEQUENCE 函数](/sql-statements/sql-statement-create-sequence/#sequence-函数)。在上游 TiDB 使用 SEQUENCE 时，TiCDC 将会忽略掉上游执行的 SEQUENCE DDL/函数，但是使用 SEQUENCE 函数的 DML 可以正确的同步。
