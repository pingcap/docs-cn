...
- 从 BR 8.2.0 起，BR 恢复对 TiCDC 的限制被放宽：如果所恢复的数据的 BackupTS（即备份时间）早于正在工作的 Changefeed 的 [CheckpointTS](/ticdc/ticdc-architecture.md#checkpointts)（即记录当前同步进度的时间戳），BR 数据恢复可以正常进行。考虑到 `BackupTS` 的时间通常较早，此时可以认为绝大部分场景下，BR 支持对 TiCDC 正在同步的表和库进行数据恢复。