...
目前 TiCDC 将数据变更信息适配对应的协议时，对于特定的 `UPDATE` 事件，可能会将其拆成一条 `DELETE` 事件和一条 `INSERT` 事件。详见[MySQL Sink 拆分所有 `UPDATE` 事件](/ticdc/ticdc-behavior-change.md#mysql-sink-拆分所有-update-事件)及[非 MySQL Sink 拆分 `UPDATE` 主键或唯一键事件](/ticdc/ticdc-behavior-change.md#非-mysql-sink-拆分-update-主键或唯一键事件)。
