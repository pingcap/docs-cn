...
从 v6.2 版本开始，TiCDC 支持拆分单表事务功能，可大幅降低同步大事务的延时和内存消耗。因此，建议用户在业务对事务原子性要求不高的场景下，通过设置 [`transaction-atomicity` 参数](/ticdc/manage-ticdc.md#sink-uri-配置-mysqltidb)打开拆分事务功能以解决可能出现的同步延迟和 OOM 问题。
