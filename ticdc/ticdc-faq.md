...
从 v6.1.1、v6.2 版本开始，TiCDC 支持拆分单表事务功能，可大幅降低同步大事务的延时和内存消耗。在用户业务对事务原子性要求不高的场景下，可以通过设置 [`transaction-atomicity` 参数](/manage-ticdc#sink-uri-配置-mysqltidb)打开拆分事务功能以解决可能出现的同步延迟和 OOM 问题。

如果遇到上述错误，建议将包含大事务部分的增量数据通过 BR 进行增量恢复，具体操作如下：
