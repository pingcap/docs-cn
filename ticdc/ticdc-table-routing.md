...
| 现象 | 可能原因 | 解决方法 |
| :--- | :--- | :--- |
| 创建或更新 Changefeed 时报 `CDC:ErrInvalidTableRoutingRule` 错误。 | `matcher` 语法无效，或 `target-schema`、`target-table` 包含未知占位符或不匹配的大括号。 | 检查 `matcher` 是否符合[表库过滤语法](/cdc-table-route-feature/table-filter.md#表库过滤语法)，并确保 `target-schema` 和 `target-table` 只使用字面文本、`{schema}` 和 `{table}`。 |
