...
- 将多个源库同步到同一个目标库，并通过保持目标表名唯一区分源库，例如将 `tenant_001.orders` 同步到 `tenant_mirror.tenant_001_orders`。
- 构建迁移、容灾、归档或影子 Changefeed，避免写入与上游同名的下游对象。
- 向 MQ 消费程序或存储服务暴露稳定的库表名。

> **注意：**
>
> 表路由仅支持上游表到下游目标表的一对一映射，不支持将多张上游表合并到同一张下游表。
...
你可以通过 [`sink.dispatchers`](/ticdc/ticdc-changefeed-config.md#dispatchers) 的以下字段配置表路由功能。
...
- 只有设置了 `target-schema` 或 `target-table` 的 `sink.dispatchers` 规则才会参与表路由。
- 如果一张表匹配多条表路由规则，只有 `sink.dispatchers` 中第一条匹配的规则会生效。
- `matcher` 始终匹配上游库表名，而不是路由后的目标库表名。
- Changefeed 配置项 `case-sensitive` 只影响表路由的 `matcher` 是否大小写敏感，不会改写 `{schema}` 和 `{table}` 展开结果的大小写。详情参见 [`case-sensitive`](/ticdc/ticdc-changefeed-config.md#case-sensitive)。
...
同一条 dispatcher 规则可以同时包含表路由字段和已有的分发字段：
...
表路由不会改变 Topic 或 Partition 的分发结果，`topic` 和 `partition` 分发器仍然按照上游表 `sales.orders` 进行匹配和分发计算。
...
| Kafka Sink 和 Pulsar Sink | 协议中的 `payload` 字段值和 DDL 事件中的 `query` 字段值使用路由后的目标库表名；编码协议中的 `schema`、`table` 字段值也会使用路由后的目标库表名。 |
| Cloud Storage Sink | TiCDC 根据路由后的目标库表名生成对应的存储路径、schema 文件、表定义文件和数据文件。 |

## DDL 行为

启用表路由后，TiCDC 会改写 DDL 语句，使结构化 DDL 元数据和 SQL 文本中的库表名保持一致。
...
如果 DDL 语句中包含表引用，且这些表引用匹配表路由规则，TiCDC 也会改写被引用的表名。例如，`CREATE VIEW` 语句中的表引用，以及 `ALTER TABLE` 语句中的外键引用都可以被改写。

对于 `CREATE DATABASE`、`DROP DATABASE` 和 `ALTER DATABASE ... CHARACTER SET/COLLATE` 这类库级 DDL，如果库名匹配表路由规则，TiCDC 会改写库名。**如果同一个上游库匹配多条表路由规则，且这些规则解析出的目标库名不同，TiCDC 无法为该库级 DDL 确定唯一目标库，Changefeed 会返回表路由错误。**

创建或更新 Changefeed 时，TiCDC 会基于当前同步范围内的表检查目标表冲突。运行期间，TiCDC 会在同步 `CREATE TABLE`、`RENAME TABLE`、`DROP TABLE`、`DROP DATABASE` 等 DDL 时更新冲突检测状态。库级 DDL 是否能确定唯一目标库，会在同步对应 DDL 时判断。

## 路由冲突检测

当两张不同的上游表在同一个 Changefeed 内被路由到同一个下游 `(schema, table)` 时，会发生路由冲突。TiCDC 不支持将多张上游表合并到同一张下游表。
...
如果 `sales.orders` 和 `crm.orders` 都在同步范围内，这两张表都会被路由到 `archive.orders`。TiCDC 会拒绝创建或更新 Changefeed，并返回 `CDC:ErrTableRouteConflict` 错误。

Changefeed 运行期间，如果 `CREATE TABLE` 或 `RENAME TABLE` 等 DDL 导致两张在当前同步范围内的上游表被路由到同一张目标表，Changefeed 会失败并返回 `CDC:ErrTableRouteConflict` 错误。

如果上游表被删除或重命名，TiCDC 会解除该上游表与目标表之间的路由关系。之后，新的上游表可以继续使用该目标表，但在同一时刻同一个目标表只能对应一张正在被当前 Changefeed 同步的上游表。

> **警告：**
>
> 路由冲突检测仅限于单个 Changefeed。如果多个 Changefeed 写入同一个下游系统，请确保这些 Changefeed 的表路由规则不会路由到相同的目标对象。

## 故障排查

| 现象 | 可能原因 | 解决方法 |
| :--- | :--- | :--- |
| 创建或更新 Changefeed 时报 `CDC:ErrInvalidTableRoutingRule` 错误。 | `matcher` 语法无效，或 `target-schema`、`target-table` 包含未知占位符或不匹配的大括号。 | 检查 `matcher` 是否符合[表库过滤语法](/table-filter.md#表库过滤语法)，并确保 `target-schema` 和 `target-table` 只使用字面文本、`{schema}` 和 `{table}`。 |
| MQ Topic 名仍然使用上游库表名。 | 表路由不会改变 Topic 或 Partition 分发规则。 | 如果需要修改 Topic 名，请在 `sink.dispatchers` 中单独配置 `topic`。 |
| DDL 同步时报 `CDC:ErrTableRoutingFailed` 错误。 | 该 DDL 无法安全地进行表路由改写，或库级 DDL 存在目标库歧义。 | 检查 DDL 类型和路由规则。对于库级 DDL，确保同一个上游库只能解析到一个目标库。 |
| Changefeed 运行中失败并报 `CDC:ErrTableRouteConflict` 错误。 | 新建表或重命名表后，两个不同的上游表被路由到同一个下游表。 | 调整表路由规则或上游 DDL，确保单个 Changefeed 内每个目标表在同一时刻只对应一张正在被当前 Changefeed 同步的上游表。 |
