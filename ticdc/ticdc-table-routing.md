...
创建或更新 Changefeed 时，TiCDC 会基于当前复制范围内的表检查目标表冲突。运行期间，TiCDC 会在同步 `CREATE TABLE`、`RENAME TABLE`、`DROP TABLE`、`DROP DATABASE` 等 DDL 时更新冲突检测状态。库级 DDL 是否能确定唯一目标库，会在同步对应 DDL 时判断。
...
Changefeed 运行期间，如果 `CREATE TABLE` 或 `RENAME TABLE` 等 DDL 导致两个存活的上游表路由到同一张目标表，Changefeed 会失败并返回 `CDC:ErrTableRouteConflict` 错误。

如果上游表被删除或重命名，TiCDC 会释放旧源表名占用的目标表。之后，新的源表可以继续使用该目标表，前提是同一时刻没有两个存活的上游表被路由到同一个目标表。
...
| Changefeed 运行中失败并报 `CDC:ErrTableRouteConflict` 错误。 | 新建表或重命名表后，两个不同的上游表被路由到同一个下游表。 | 调整表路由规则或上游 DDL，确保单个 Changefeed 内每个目标表在同一时刻只对应一个存活的上游表。 |
