...
- 将多个源库同步到同一个目标库，并通过保持目标表名唯一区分来源，例如将 `tenant_001.orders` 同步到 `tenant_mirror.tenant_001_orders`。
...
表路由功能使用 [`sink.dispatchers`](/ticdc/ticdc-changefeed-config.md#dispatchers) 作为配置入口。
