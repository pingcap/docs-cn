---
title: TiCDC 表路由
summary: 了解 TiCDC 新架构中的表路由配置，包括通过 target-schema 和 target-table 改写下游库名和表名、冲突检测、DDL 改写、限制和排查方法。
---
...
> **注意：**
>
> 表路由仅支持上游表到下游目标表的一对一映射，不支持将多张上游表合并到一张下游表。
...
| Kafka Sink 和 Pulsar Sink | 协议中的 `payload` 字段值和 DDL 事件中的 `query` 字段值使用路由后的目标库表名；编码协议中的 `schema`、`table` 字段值也是路由后的目标库表名。 |
| Cloud Storage Sink | 根据路由后的目标库表名输出对应的存储路径、schema 文件、表定义文件和数据文件。 |

## DDL 行为

启用表路由后，TiCDC 会改写 DDL 语句，使结构化 DDL 字段和 SQL 文本使用一致的目标库表名。
...
对于 `CREATE DATABASE`、`DROP DATABASE` 和 `ALTER DATABASE ... CHARACTER SET/COLLATE` 这类库级 DDL，如果库名匹配表路由规则，TiCDC 会改写库名。**如果同一个上游库匹配多条表路由规则，但这些规则解析出不同的目标库名，TiCDC 无法为该库级 DDL 确定唯一目标库，Changefeed 会返回表路由错误。**
...
| DDL 语句报 `CDC:ErrTableRoutingFailed` 错误。 | 该 DDL 语句无法安全地进行表路由改写，或库级别路由存在歧义。 | 调整路由规则。 |
