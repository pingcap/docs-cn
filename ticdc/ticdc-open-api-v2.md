...
| `use_file_backend`        | `BOOL` 类型，是否将 redo log 存储到本地文件中。（非必选）              || `encoding_worker_num`        | `INT` 类型，redo 中编解码 worker 的数量。（非必选）              |`flush_worker_num`        | `INT` 类型，redo 中上传文件 worker 的数量。（非必选）              |
