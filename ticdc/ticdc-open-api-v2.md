...
| `sync_point_interval`     | `UINT64` 类型，纳秒级时间，`sync point`功能开启时，对齐上下游 snapshot 的时间间隔,默认值为 10m，最小值为 30s。（非必选） |
| `sync_point_retention`    | `UINT64` 类型，纳秒级时间,`sync point`功能开启时，功能在下游表中保存的数据的时长，超过这个时间的数据会被清理,默认值为 24h。（非必选） |
