...
- `total`: 表示有一共有多少个资源。
- `items`: 当次请求返回的资源在这个数组里，数组所有的元素都是同种资源.数组中的元素个数总是小于等于 `total`。
...
- `liveness`: 该节点的 liveness 状态， 0 表示是正常的，1 表示处于 `graceful shutdown` 状态
...
| 参数名              | 说明                                                    |
|:-----------------|:------------------------------------------------------|
| `flush_interval` | `BOOLEAN` 类型，是否开启 bdr 同步复制。（非必选）                      |
| `level`          | `STRING` 类型，同步数据的一致性界别（非必选）                           |
| `max_log_size`   | `BOOLEAN` 类型，是否检查同步任务的开始时间超过了 GC 时间，默认值为 `true`。（非必选） |
| `storage`        | `STRING` 类型，同步任务一致性配置。（非必选）                           |
