...
`key` 和 `value` 分别对应 Kafka 消息中的同名字段。消费者可以通过解析这两部分的数据，还原大消息的内容。只有 Open Protocol 编码的 Kafka 消息包含 `key` 字段。将 `key` 和 `value` 编码到同一个 JSON 对象中，一次性发送完整的消息。对于其他协议，`key` 字段始终为空。
