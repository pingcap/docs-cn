...
| `required-acks`      | 在 Produce 请求中使用的配置项，用于告诉 broker 需要收到多少副本确认后才进行响应。以下三个常量值都是有效的：0（NoResponse: 不发送任何响应，只有 TCP ACK），1（WaitForLocal: 仅等待本地提交成功后再响应） 和 -1（WaitForAll: 等待所有同步副本提交后再响应。要配置的最小同步副本数量通过 broker 的 [min.insync.replicas](https://kafka.apache.org/33/documentation.html#brokerconfigs_min.insync.replicas) 配置项进行配置）。                      |
