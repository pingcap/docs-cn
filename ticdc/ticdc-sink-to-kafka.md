...
# 默认为空，消息超过大小限制后，同步任务失败，该参数 v7.3.0 引入。
...
Kafka 消费者收到消息之后，首先检查 `onlyHandleKey` 字段，如果存在该字段，那么必然为 true，表示该消息只包含 Handle-Key 部分数据。用户需要查询上游 TiDB，通过 [`tidb_snapshot` 读取历史数据](/read-historical-data.md)来获取完整的数据。
