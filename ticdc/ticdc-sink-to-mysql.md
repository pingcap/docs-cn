...
若需要对 Sink URI 中的数据库密码使用 base64 进行编码，可以参考如下操作:

```shell
echo -n '123456' | base64   # 假设加密前的密码为 123456
MTIzNDU2   # 加密后的密码
```

> **注意：**
>
> 当 Sink URI 中包含特殊字符时，如 `! * ' ( ) ; : @ & = + $ , / ? % # [ ]`，需要对 URI 特殊字符进行转义处理。你可以在 [URI Encoder](https://meyerweb.com/eric/tools/dencoder/) 中对 URI 进行转义。
