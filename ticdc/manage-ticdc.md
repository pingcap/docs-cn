...
* TiCDC 推荐用户自行创建 Kafka Topic，你至少需要明确该 Topic 的 [`max.message.bytes`](https://kafka.apache.org/documentation/#topicconfigs_max.message.bytes) 和该 Topic 的 partitions 个数。在创建 changefeed 的时候，使用这两项分别设置 `sink-uri` 中的 `max-message-bytes` 和 `partition-num` 参数。
...
* TiCDC 建议使用 `canal-json` 协议。

如果 topic 已经存在，并且有 `max.message.bytes` 为 `10485760`，partitions 的个数为 3，可以参考如下 `sink-uri`:
