...
> **注意：**
>
> TiCDC 监听的 IP 和端口对应为 `cdc server` 启动时指定的 `--addr` 参数。从 TiCDC v6.2 开始，cdc cli 都将通过 TiCDC 的 Open API 直接与 TiCDC server 进行交互，通过`--server`参数指定 TiCDC 的 server 地址，而`--pd` 参数将被废弃，不再推荐使用。
