...
> **警告：**
>
> 没有有效索引的表，INSERT, REPLACE 等操作不具备可重入性，因此会有数据冗余的风险。CDC 在同步过程中只保证数据至少分发一次，因此开启该特性同步没有有效索引的表，数据冗余一定会出现。如果不能接受数据冗余，建议增加有效索引，譬如增加使用 AUTO RANDOM 的 PRIMARY KEY 列。
