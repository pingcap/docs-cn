...
目前 HTTP 接口提供一些基础的查询和运维功能，总体功能和 cli 类似。HTTP 接口的请求体与返回值数据格式统一为 JSON 格式数据。在以下接口描述中，假设 TiCDC server 的监听 IP 地址为 `127.0.0.1`，端口为 `8300`（在启动 TiCDC server 时通过 `--addr=ip:port` 指定绑定的 IP 和端口）。
...
`changefeed_id`、`start_ts`、`target_ts`、`sink_uri` 的含义和格式与本页上方使用 cli 创建同步任务时所作的解释相同，此处不再赘述，下面会对一些需要补充说明的参数进行进一步阐述。
...
以下请求会更新 ID 为 test1的同步任务的 mounter_worker_num 为 32。
...
该接口是一个同步接口，请求成功会返回指定 processor 的详细信息。
...
```shell
 curl -X POST http://127.0.0.1:8300/api/v1/changefeeds/test/tables/rebalance_table
```
...
```shell
 curl -X POST http://127.0.0.1:8300/api/v1/changefeeds/test/tables/move_table -d 'target_capture_id=6f19a6d9-0f8c-4dc9-b299-3ba7c0f216f5&table_id=49'
```
