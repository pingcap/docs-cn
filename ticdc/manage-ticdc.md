...
本部分介绍如何使用 `cdc cli` 工具来管理集群状态和数据同步。`cdc cli` 是指通过 `cdc` binary 运行 `cli` 子命令，如果通过 TiUP 部署 TiCDC，可以使用 `tiup ctl cdc` 命令（注意这里不需要再输入 cli 子命令）。在以下接口描述中，假设 PD 的监听 IP 地址为 `10.0.10.25`，端口为 `2379`，如果使用 TiUP，可以将 `cdc cli` 替换为 `tiup ctl cdc`。
