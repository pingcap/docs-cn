...
本部分介绍如何使用 TiUP 来升级 TiCDC 集群。在以下例子中，假设需要将 TiCDC 组件和整个 TiDB 集群升级到 v6.1.3。
...
| `transaction-atomicity`      | 指定事务的原子性级别（可选），默认值为 `none`，表示 TiCDC 会拆分单表事务。当该值为 `table` 时 TiCDC 保证单表事务的原子性  |
