...
- 当下游为 MQ 类 Sink 时，`dispatchers` 用于配置 event 分发器。从 v8.5.7 起，对于 [TiCDC 新架构](/ticdc/ticdc-architecture.md)，`dispatchers` 也可以用于配置表路由，将上游表映射到指定的下游库名或表名。更多信息，参见 [TiCDC 表路由](/ticdc/ticdc-table-routing.md)。
