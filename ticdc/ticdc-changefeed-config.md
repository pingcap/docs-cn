...
>>>>>>> af5e5278a0 (ticdc: fix wrong config value of `region-threshold` (#20597))
```toml
...
flush-worker-num = 8
# redo log 文件的压缩行为，可选值为 "" 和 "lz4"。默认值为 ""，表示不进行压缩。从 v6.5.6 和 v7.1.3 版本开始支持。
compression = ""
# redo log 上传单个文件的并发数，默认值为 1，表示禁用并发。从 v6.5.6 和 v7.1.3 版本开始支持。
flush-concurrency = 1
