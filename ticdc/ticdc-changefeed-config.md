...
- 在 TiCDC 老架构中，该功能只在 Kafka changefeed 上生效，暂不支持 MySQL changefeed。
- 在 TiCDC 新架构中，该功能对所有类型下游的 changefeed 生效。详情请参考[新功能介绍](/ticdc/ticdc-architecture.md#新功能介绍)。
- `enable-table-across-nodes` 开启后，有两种分配模式：

    1. 按 Region 的数量分配，即每个 TiCDC 节点处理 Region 的个数基本相等。当某个表 Region 个数大于 [`region-threshold`](/ticdc-changefeed-config.md#region-threshold) 值时，会将表分配到多个节点处理。
