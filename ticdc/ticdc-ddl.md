...
虽然 TiCDC 在大多数情况下会按顺序同步 DDL，但如果下游是 TiDB，TiCDC 会异步执行执行创建和添加索引的 DDL 操作，即 TiCDC 将 `ADD INDEX` 和 `CREATE INDEX` DDL 同步到下游执行后，会立刻返回，而不会等待 DDL 操作完成。这样可以减小对 Changefeed 同步延迟的影响，避免阻塞后续的 DML 执行。
