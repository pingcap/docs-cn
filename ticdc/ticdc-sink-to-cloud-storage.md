...
> **注意：**
>
> 表的版本仅在发生 DDL 操作后才改变：表的版本为该 DDL 在上游 TiDB 执行结束的 TSO。但是，表版本的变化并不意味着表结构的变化。例如，在表中的某一列添加注释，不会导致 schema 文件内容发生变化。
...
当 DDL 事件引起表的版本变更时，TiCDC 将会切换到新的路径下写入数据变更记录，例如 `test.table1` 的版本变更为 `441349361156227074` 时将会在 `s3://bucket/bbb/ccc/test/table1/441349361156227074/2022-01-02/CDC000001.csv` 路径中写入数据。并且，当 DDL 事件发生时，TiCDC 将生成一个文件存储表结构信息。
...
一个名如 `schema_441349361156227074_3131721815.json` 文件如下：
...
一个名为 `schema_441349361156227000_3131721815.json` 的文件如下:
...
本章节主要介绍 schema 文件中使用的各种数据类型。数据类型定义为 `T(M[, D])`，详见[数据类型概述](/data-type-overview.md#数据类型概述)。
