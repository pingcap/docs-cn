...
上述内容表明该目录下 CDC000001.csv 到 CDC000004.csv 文件已被占用，当 CDC 集群中发生表调度或者节点重启时，新的节点会读取 Index 文件，并判断 CDC000005.csv 是否被占用：如果未被占用，则新节点会从 CDC000005.csv 开始写文件，否则从 CDC000006.csv 开始写文件，这样可防止覆盖掉其他节点写入的数据。
