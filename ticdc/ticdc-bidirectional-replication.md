...
- TiCDC 双向复制功能支持超过 2 个集群的双向同步，但是不支持多个集群级联模式的同步，即 TiDB A -> TiDB B ->  TiDB C -> TiDB A 的环形复制方式，这种部署方式如果其中一个链路出现问题则会影响整个数据同步链路。  这种场景下，该功能需要每个集群都要与其他集群两两互相连接，即 TiDB A <-> TiDB B, TiDB B <->  TiDB C, TiDB C <-> TiDB A
