...
- 因为该命令没有指定 TiDB/PD/TiKV 各组件的版本，默认情况下，它会使用各组件的最新发布版本，假设当前为 v{{{ .tidb-version }}}，则该命令相当于 tiup playground:1.12.3 v{{{ .tidb-version }}}
...
首先，你需要复制 PD 的[配置模版](https://github.com/tikv/pd/blob/release-7.5/conf/config.toml)。假设你将复制的配置文件放置在 `~/config/pd.toml`，按需修改一些内容后，执行以下命令可以覆盖 PD 的默认配置：
