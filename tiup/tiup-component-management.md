...
示例三：使用 TiUP 安装 {{{ .tidb-version }}} 版本的 TiKV

{{< copyable "shell-regular" >}}

```shell
tiup install tikv:{{{ .tidb-version }}}
```
...
示例一：运行 {{{ .tidb-version }}} 版本的 TiDB

{{< copyable "shell-regular" >}}

```shell
tiup tidb:{{{ .tidb-version }}}
```
...
示例一：卸载 {{{ .tidb-version }}} 版本的 TiDB

{{< copyable "shell-regular" >}}

```shell
tiup uninstall tidb:{{{ .tidb-version }}}
```
