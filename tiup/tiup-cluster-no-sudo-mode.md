...
2. 在每台部署目标机器上，为 `tidb` 用户启动 `systemd user` 模式。该步骤是必须的，请勿跳过。
    ...
    3. 使用 `root` 用户执行以下命令，为 systemd 用户实例启用驻留。

        ```bash
        loginctl enable-linger tidb
        ```

        您可以阅读 systemd 文档 [systemd 用户实例的自动启动](https://wiki.archlinux.org/title/Systemd/User#Automatic_start-up_of_systemd_user_instances) 以了解更多。
