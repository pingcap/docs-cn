...
- `listen_host`：默认使用的 listen host。如果为空，每个实例会通过判断自身的 `Host` 字段是否包含 `:` 来自动设置成 `::` 或者 `0.0.0.0`。tiup-cluster v1.14.0 引入该配置
...
> **注意：**
>
> 对于 TiDB、TiKV、PD、TiCDC 等本身共用版本号的组件。没有完整的测试来保证它们能够在跨版本混部的场景下能够正常工作。请仅用于测试场景或者在 support 的帮助下进行操作。

`component_versions` 用于指定某个组件的版本号。当不配置 component_versions 时，各个组件要么使用和 TiDB 集群一致的版本号（如 pd、tikv），要么使用最新版（如 alertmanager）。当配置该字段时，对应的组件将会固定使用对应的版本，并且在后续的扩容和升级集群操作中都使用该版本。用户应当只在需要特别固定某个组件的版本号的时候才进行相应配置。主要包含以下字段：
