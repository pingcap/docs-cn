...
该命令需要提供集群的名字、集群使用的 TiDB 版本（例如 `v6.5.9`），以及一个集群的拓扑文件。
...
假如我们想要使用 TiDB 的 v6.5.9 版本，集群名字为 `prod-cluster`，则执行以下命令：

{{< copyable "shell-regular" >}}

```shell
tiup cluster deploy -p prod-cluster v6.5.9 /tmp/topology.yaml
```
...
----          ----  -------    ----                                               ----------
prod-cluster  tidb  v6.5.9    /root/.tiup/storage/cluster/clusters/prod-cluster  /root/.tiup/storage/cluster/clusters/prod-cluster/ssh/id_rsa
```
...
例如，把集群升级到 v6.5.9 的命令为：

{{< copyable "shell-regular" >}}

```bash
tiup cluster upgrade tidb-test v6.5.9
```
...
ID      Time                       Command
--      ----                       -------
4BLhr0  2024-04-12T13:25:09+08:00  /home/tidb/.tiup/components/cluster/v1.11.3/cluster deploy test v6.5.9 /tmp/topology.yaml
4BKWjF  2024-04-12T23:36:57+08:00  /home/tidb/.tiup/components/cluster/v1.11.3/cluster deploy test v6.5.9 /tmp/topology.yaml
4BKVwH  2024-04-12T23:02:08+08:00  /home/tidb/.tiup/components/cluster/v1.11.3/cluster deploy test v6.5.9 /tmp/topology.yaml
4BKKH1  2024-04-12T16:39:04+08:00  /home/tidb/.tiup/components/cluster/v1.11.3/cluster destroy test
4BKKDx  2024-04-12T16:36:57+08:00  /home/tidb/.tiup/components/cluster/v1.11.3/cluster deploy test v6.5.9 /tmp/topology.yaml
```
...
- 部署集群：`tiup cluster deploy <cluster-name> <version> <topo> --ssh=system`，其中 `<cluster-name>` 为集群名称，`<version>` 为 TiDB 集群版本（例如 `v6.5.9`），`<topo>` 为拓扑文件路径
