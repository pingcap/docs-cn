...
该命令需要提供集群的名字、集群使用的 TiDB 版本（例如 `v6.5.2`），以及一个集群的拓扑文件。
...
假如我们想要使用 TiDB 的 v6.5.2 版本，集群名字为 `prod-cluster`，则执行以下命令：
...
ID      Time                       Command
--      ----                       -------
4BLhr0  2023-03-10T13:25:09+08:00  /home/tidb/.tiup/components/cluster/v1.11.3/cluster deploy test v6.5.2 /tmp/topology.yaml
...
- 部署集群：`tiup cluster deploy <cluster-name> <version> <topo> --ssh=system`，其中 `<cluster-name>` 为集群名称，`<version>` 为 TiDB 集群版本（例如 `v6.5.2`），`<topo>` 为拓扑文件路径
