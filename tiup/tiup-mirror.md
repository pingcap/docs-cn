...
4. 限定只克隆组件的特定版本
    ...
    - 只想克隆 TiDB 的 v{{{ .tidb-version }}} 版本，则执行 `tiup mirror clone <target-dir> --tidb v{{{ .tidb-version }}}`
    - 只想克隆 TiDB 的 v{{{ .tidb-version }}} 版本，以及 TiKV 的所有版本，则执行 `tiup mirror clone <target-dir> --tidb v{{{ .tidb-version }}} --tikv all`
    - 克隆一个集群的所有组件的 v{{{ .tidb-version }}} 版本，则执行 `tiup mirror clone <target-dir> v{{{ .tidb-version }}}`
