...
routes:
  sale-route-rule:
    schema-pattern: "store_*"                               # 合并 store_01, store_02 库到下游 store 库
    table-pattern: "sale_*"                                 # 合并上述库中的 sale_01, sale_02 表到下游 sale 表
    target-schema: "store"
    target-table:  "sale"
    # 可选配置：提取各分库分表的源信息并写入下游的用户自建列用于标识合表中各行数据来源，如配置该项需要提前在下游手动创建合表，具体可参考下面的 Table routing 详细用法
    # extract-table:                                        # 提取分表去除 sale_ 的后缀信息写入下游合表 c_table 列，例如来自 sale_01 分表的数据会提取 01 写入下游 c_table 列
    #   table-regexp: "sale_(.*)"
    #   target-column: "c_table"
    # extract-schema:                                       # 提取分库去除 store_ 的后缀信息写入下游合表 c_schema 列，例如来自 store_02 分库的数据会提取 02 写入下游 c_schema 列
    #   schema-regexp: "store_(.*)"
    #   target-column: "c_schema"
    # extract-source:                                       # 提取数据库源实例信息写入 c_source 列，例如来自 mysql-01 数据源实例的数据会提取 mysql-01 写入下游 c_source 列
    #   source-regexp: "(.*)"
