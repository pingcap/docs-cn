...
#### 3.4.21 TiKV 容易出现 OOM

TiKV 的内存占用主要来自于 RocksDB 的 block-cache，默认为系统总内存的40%。当 TiKV 容易出现 OOM 时，检查 `block-cache-size` 配置是否过高。还需要注意，当单机部署了多个 TiKV 时，需要显式配置该参数，以防止多个实例占用过多系统内存导致 OOM。
