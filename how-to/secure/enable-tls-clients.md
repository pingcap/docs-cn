...
另外，与 MySQL 一致，TiDB 加密连接是以单个连接为单位的，默认情况下是可选的，因而对于开启了加密连接支持的 TiDB 服务端，客户端既可以选择通过加密连接安全地连接到该 TiDB 服务端，也可以选择使用普通的非加密连接。如需强制要求客户端使用加密连接可以通过以下两种方式:

1. 通过在启动参数中配置 `--require-secure-transport` 强制要求所有用户只能通过加密连接连接启动的 TiDB 实例
2. 通过在创建用户 (`create user`)，赋予权限 (`grant`) 或修改已有用户 (`alter user`) 时指定 `require ssl` 强制要求指定用户只能通过加密连接连接， TiDB 以创建用户为例:
...
> **注意：**
>
> 如果登录用户已配置使用[TiDB 证书鉴权功能](/reference/security/cert-based-authentication.md#配置登陆时需要校验的用户证书信息)校验用户证书信息，对应用户必须使用加密连接且提供有要求内容的证书才能连接 TiDB
...
默认情况，服务端对客户端的身份验证是可选的。若客户端在 TLS 握手时未出示自己的身份证书，也能正常建立 TLS 连接。但也可以通过在创建用户 (`create user`)，赋予权限 (`grant`) 或修改已有用户 (`alter user`)时指定 `require 509` 强制要求客户端必须提供身份证明， TiDB 以创建用户为例:
...
> **注意：**
>
> 如果登录用户已配置使用[TiDB 证书鉴权功能](/reference/security/cert-based-authentication.md#配置登陆时需要校验的用户证书信息)校验用户证书信息，对应用户必须提供有效身份信息且提供有要求内容的证书才能连接 TiDB
