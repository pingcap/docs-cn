...
3. 自定义制作的离线镜像，或调整已有离线镜像中的内容
    ...
    1. 在制作离线镜像时，可通过参数指定具体的组件和版本等信息，获得不完整的离线镜像。例如，要制作一个只包括 v1.10.3 版本 TiUP 和 TiUP Cluster 的离线镜像，可执行如下命令：
    ...
    5. 上述步骤完成后，通过 `tiup list` 命令检查执行结果。在本文例子中，使用 `tiup list tiup` 和 `tiup list cluster` 均应能看到对应组件的 `v1.10.3` 版本出现在结果中。
...
- `tidb-test` 为部署的集群名称。
- `v6.2.0` 为部署的集群版本，可以通过执行 `tiup list tidb` 来查看 TiUP 支持的最新可用版本。
