...
3. 部署 TiDB 集群：
    ...
    ```shell
    tiup cluster deploy tidb-test {{{ .tidb-version }}} ./topology.yaml --user root [-p] [-i /home/root/.ssh/gcp_rsa]
    ```

以上部署示例中：

- `tidb-test` 为部署的集群名称。
- `{{{ .tidb-version }}}` 为部署的集群版本，可以通过执行 `tiup list tidb` 来查看 TiUP 支持的最新可用版本。
