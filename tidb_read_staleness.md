...
系统变量 `tidb_read_staleness` 用于设置当前会话期待读取的历史数据所处时刻，其数据类型为 int 类型，作用域为 `SESSION`。设置该变量后，TiDB 会从所设置的值的秒前至当前时间的范围内选出一个尽可能新的时间戳。随后，所有的读操作都会依据这个时间戳来读取数据。比如，如果该变量的值设置为 `-5`，TiDB 会从 5 秒前至当前时间的范围内选取一个尽可能新的时间戳，将其用作为时间戳。此时，TiDB 会在保证 TiKV 拥有对应历史版本的数据的同时选出一个尽量新的时间戳。
...
5. 设置一个特殊的环境变量 `tidb_read_staleness`。
    ...
    >
    > 由于时间范围是在 5 秒内，所以第 3、4 步需要在 5 秒内完成，否则查询结果会是最新值，用户可根据自己的实际操作情况调整时间范围。
