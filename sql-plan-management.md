...
> 例如：
> ```sql
> mysql> create table t(a int, b int, key idx(a));
Query OK, 0 rows affected (0.21 sec)

mysql> create session binding for select * from t where a in (?) using select /*+ use_index(t, idx) */ * from t where a in (?);
...
mysql> drop session binding for select * from t where a in (?);
Query OK, 0 rows affected (0.00 sec)
