...
Plan Baseline 是一组被允许用于 SQL 语句优化器的可接受计划。在典型的应用场景中，数据库仅在验证计划的表现良好后才将其添加到计划 Baseline 中。在这种情况下，计划包括所有与计划相关的信息（例如，SQL计划标识符、提示集、绑定值和优化器环境），优化器需要这些信息才能重新生成执行计划。
