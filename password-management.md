...
TiDB 支持通过设置密码过期策略，要求用户定期修改密码，从而提高密码的安全性。数据库管理员能够手动使账户密码过期，也可以建立密码自动过期策略。
...
对于通过 `CREATE ROLE` 语句创建的角色，由于角色不需要设置密码，所以该角色的密码为空，此时对应的 `password_expired` 属性为 `'Y'`，即角色的密码处于手动过期状态。如此设计的目的是防止出现角色锁定状态被解除后以空密码登录到 TiDB：当该角色被 `ALTER USER ... ACCOUNT UNLOCK` 命令解锁后，此时该角色处于可登录的状态，但是密码为空；此时密码处于手动过期的状态，因此用户必须为该角色设置有效的密码。
