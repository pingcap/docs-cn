...
在 TiDB 中，密码复杂度检查默认未开启。通过配置密码复杂度相关的系统变量，你可以开启密码复杂度检查，并确保账户设置的密码符合密码复杂度策略。

密码复杂度策略支持以下功能：

- 对采用明文方式设置用户密码的 SQL 语句（包括 `CREATE USER`、`ALTER USER`、`SET PASSWORD` ），系统会根据密码复杂度策略检查密码。如果密码不符合要求，则拒绝该密码。
- 可以使用 SQL 函数 [`VALIDATE_PASSWORD_STRENGTH()`](https://dev.mysql.com/doc/refman/5.7/en/encryption-functions.html#function_validate-password-strength) 评估给定密码的强度。该函数接受一个密码参数，并返回一个从 0（弱）到 100（强）的整数。评估结果将基于当前配置的密码复杂度策略，密码复杂度配置改变后，同一个密码的评估结果可能与之前不同。

> **注意：**
>
> - 对于 `CREATE USER` 语句，即使该账户最初被锁定，也必须提供满足密码复杂度策略的密码，否则将账户解锁后，将出现不符合密码复杂度策略的密码也可以访问该账户。
> - 对密码复杂度策略的变更不影响已存在账户的密码，只会对新设置的密码产生影响。
...
TiDB 支持通过设置密码过期策略，要求用户定期修改密码，从而提高密码的安全性。具有 `CREATE USER` 权限的数据库管理员可以手动将指定账户的密码设置为过期，也可以建立密码自动过期策略。
...
对于通过 `CREATE ROLE` 语句创建的角色，由于角色不需要设置密码，所以该角色对应的密码字段为空，此时对应的 `password_expired` 属性为 `'Y'`，即角色的密码处于手动过期状态。如此设计的目的是防止出现角色锁定状态被解除后以空密码登录到 TiDB：当该角色被 `ALTER USER ... ACCOUNT UNLOCK` 命令解锁后，此时该角色处于可登录的状态，但是密码为空；此时密码处于手动过期的状态，因此必须设置一个有效的密码。
...
你可以通过 TiDB 系统变量 [`disconnect_on_expired_password`](/system-variables.md#disconnect_on_expired_password) 控制服务器是断开链接还是处于沙盒模式。

- 默认情况下，`disconnect_on_expired_password` 为 `ON`，表示当密码过期后，服务器将直接断开客户端的连接。
- 如果禁用 `disconnect_on_expired_password `，则服务端处于沙盒模式，服务端允许用户建立连接，但只能执行密码重置操作，密码重置后将允许用户正常执行各类 SQL 命令。
