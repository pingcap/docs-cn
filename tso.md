...
注意由于 TSO 时间戳依据事务进行分配的，所以该时间戳是在带有 `BEGIN; ...; ROLLBACK` 的事务中生成的。
...
`>> 18` 操作表示按位[右移](/functions-and-operators/bit-functions-and-operators.md) 18 位，用于提取物理时间戳。由于物理时间戳以毫秒为单位，与更常见的以秒为单位的 UNIX 时间戳格式不同，因此需要除以 1000 将其转换为与 [`FROM_UNIXTIME()`](/functions-and-operators/date-and-time-functions.md) 兼容的格式。这个过程与 `TIDB_PARSE_TSO()` 的功能一致。
...
可以看到以 `system:` 开头的行中的物理时间戳，以 `logic:` 开头的行中的逻辑时间戳。
