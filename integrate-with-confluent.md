...
2. 创建一个 changefeed，将增量数据输出到 Confluent Cloud：
    ...
    - 如果命令长时间没有返回，请检查当前执行命令所在服务器到 Confluent Cloud 之间网络可达性，参考 [Test connectivity to Confluent Cloud | Confluent Documentation](https://docs.confluent.io/cloud/current/networking/testing.html)。
...
- 连接到 Snowflake 前，为 Snowflake 添加 Private Key，参考 [Key Pair Authentication & Key Pair Rotation ‒ Snowflake Documentation](https://docs.snowflake.com/en/user-guide/key-pair-auth.html)。
