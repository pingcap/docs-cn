...
2. 将存储介质的 I/O 调度器设置为 noop。对于高速 SSD 存储介质，内核的 I/O 调度操作会导致性能损失。将调度器设置为 noop ，不对内核做任何操作，尽可能快地将 I/O 请求下发给硬件，能获取更好的性能。同时，noop 调度器也有较好的普适性。
3. 调整 CPU 频率的 cpufreq 模块选用 performance 模式。将 CPU 频率固定在其支持的最高运行频率上，不进行动态调节，以获取最佳的性能。
...
3. 执行以下命令查看磁盘的唯一标识 `ID_SERIAL`。
    ...
    > **注意：**
    >
    > 如果有多个磁盘都分配了数据目录，需要多次执行以上命令，记录所有磁盘的唯一标识。
