...
在配置互信时，建议在所有的目标机器上创建并使用 `tidb` 用户。一般情况下，系统并不强制要求各节点上的用户相同。但是，如果你计划使用备份恢复工具 (BR)，强烈建议所有 BR 和 TiDB 操作使用同一用户。如果要在 NFS 等网络存储上使用，需要保证此用户的 UID/GID 相同，因为 NFS 依赖底层的 UID/GID 来识别文件权限。如果各节点 UID/GID 不一致，或者执行 BR 的用户与运行 TiDB 的用户不同（尤其是在无 `sudo` 权限模式下），很容易导致备份或恢复时出现权限拒绝 (Permission Denied) 的错误。
