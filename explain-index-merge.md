...
> - 当 [Optimizer Fix Control 52869](/optimizer-fix-controls.md#52869-从-v810-版本开始引入) 为 `OFF` 时，如果查询有除了全表扫描以外的单索引扫描方式可以选择，优化器不会自动选择索引合并，只能通过 Hint 指定使用索引合并。从 v8.5.7 和 v9.0.0 开始，该开关的默认值变为 `ON`，上述限制默认已被解除，优化器可以在更多查询中自动选择索引合并。
