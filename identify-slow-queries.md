...
* `Stats`：表示这个语句涉及表的统计信息的健康状态、内部版本号、总行数、修改行数以及加载状态。`pseudo` 状态表示统计信息状态不健康。如果有尝试使用但没有完整加载的统计信息，会在之后输出其内部状态。例如，`t1:439478225786634241[105000;5000][col1:allEvicted][idx1:allEvicted]` 表示本次查询优化过程中使用了 `t1` 表上的统计信息，其内部版本号为 `439478225786634241`，统计信息中维护的总行数是 `105000` 行，自上次收集统计信息以来记录了 `5000` 行修改，其中 `col1` 列与 `idx1` 索引对应的统计信息没有完整加载。
* `Succ`：表示语句是否执行成功。
