...
加悲观锁需要向 TiKV 写入数据，要经过 Raft 提交并 apply 后才能返回，相比于乐观事务，不可避免的会增加部分延迟。为了降低加锁的开销，TiKV 实现了 pipelined 加锁流程：当数据满足加锁要求时，TiKV 立刻通知 TiDB 执行后面的请求，并异步写入悲观锁，从而降低大部分延迟，显著提升悲观事务的性能。但当出现网络隔离或者节点宕机时，悲观锁异步写入有可能失败，导致无法阻塞其他修改相同数据的事务，会影响依赖加锁/等锁机制的业务逻辑的正确性，也有较低概率导致事务提交失败，但不会影响事务正确性。如果业务逻辑依赖加锁/等锁机制或者即使在集群异常情况下也要保证事务提交的成功率，应关闭该功能。
...
若集群是 v4.0.9 及以上版本，也可通过[在线修改 TiKV 配置](/dynamic-config.md#在线修改-tikv-配置) 功能动态开启：
