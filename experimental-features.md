...
+ 提升优化器选择索引的稳定性（v5.0 实验特性）
    ...
    + 重构统计信息模块，帮助优化器选择相对较优的索引，包括从 `CMSKetch` 和直方图中删除 `TopN` 值，为索引的直方图维护 Bucket NDV。详情参阅 [统计信息简介 - `tidb_analyze_version = 2` 的介绍](https://docs.pingcap.com/zh/tidb/dev/statistics)。
