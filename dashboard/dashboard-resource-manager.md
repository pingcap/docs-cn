...
- [根据实际负载估算容量](/sql-statements/sql-statement-calibrate-resource.md#根据实际负载估算容量) (Calibrate by Workload)
    ...
    - 估算容量功能需要监控指标数据，包括 `resource_manager_resource_unit`、`process_cpu_usage`、`tikv_cpu_quota`、`tidb_server_maxprocs`。如果对应监控数据为空，会有对应监控项名称的报错，如 `Error 1105 (HY000): metrics ‘resource_manager_resource_unit’ is empty`。当完全没有负载时，`resource_manager_resource_unit` 为空，也会报告此错误。
