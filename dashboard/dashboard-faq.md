...
可能的原因是你开启了 TiDB 的 `prepared-plan-cache` 功能。在某些版本的 TiDB 中，`prepared-plan-cache` 作为实验性功能存在一些缺陷，开启后可能会导致 TiDB Dashboard（及其他应用）出现该问题。请依据[文档](/tidb-configuration-file.md#prepared-plan-cache)关闭 `prepared-plan-cache` 功能。
