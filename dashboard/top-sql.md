...
- 登录 TiDB Dashboard，点击左侧面板中的 **Top SQL**，然后点击页面右上角的齿轮按钮。
...
* 如果图表中显示的数据已过时，你可以点击刷新按钮，也可以在刷新下拉列表中选择是否自动刷新，以及自动刷新间隔。
* 列表中展示了选中节点、选中时间范围内消耗负载最多的 5 个查询。
* 点击选中列表中的某个查询，可以查看针对这类查询的详细信息。例如，这类查询的执行计划、各个执行计划在这个节点上的 Call/sec （平均每秒请求数）、 Scan Rows/sec （平均每秒扫描行数）、 Scan Indexes/sec （平均每秒扫描索引数），以及 Latency/call （平均延迟）。
