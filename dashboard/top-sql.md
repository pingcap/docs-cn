...
对于后续新扩容的 TiKV 节点，这个开关不会自动生效。你需要重新在 Top SQL 设置面板中手工操作一次 **开启 TiKV 网络 IO 采集（多维度）** 开关并保存，使配置重新下发到所有 TiKV 节点。如果你希望新节点自动开启该能力，可以在 TiUP 集群拓扑的 `server_configs.tikv` 下增加以下配置，并通过 TiUP 重新下发 TiKV 配置：
