...
- Region health：集群所有 Region 的状态，通常情况下，pending 或 down 的 peer 应该少于 100，miss 的 peer 不能一直大于 0
...
- 99% Operator step duration：99% 已完成的 operator 的步骤所花费的最长时间
- 50% Operator step duration：50% 已完成的 operator 的步骤所花费的最长时间
...
- Hot Region's leader distribution：每个 TiKV 实例上是写入热点的 leader 的数量
...
- Hot Region's leader distribution：每个 TiKV 实例上是读取热点的 leader 的数量
...
- Rule checker：replica checker 的状态
...
- Heartbeat region evnent QPS: 心跳处理 region 的 QPS, 包括更新缓存和持久化
