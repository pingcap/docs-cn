...
- [TiDB 集群部署](#tidb-集群部署)
  - [1. 准备环境](#准备环境)
  - [2. 在中控机上安装 Ansible](#在中控机上安装-ansible)
  - [3. 下载 TiDB-Ansible 至中控机](#下载-tidb-ansible-至中控机)
  - [4. 分配 TiDB 集群资源](#分配-tidb-集群资源)
  - [5. 部署 TiDB 集群](#部署-tidb-集群)
  - [6. 启动 TiDB 集群](#启动-tidb-集群)
- [TiDB 基本操作](#tidb-基本操作)
  - [创建、查看和删除数据库](#创建查看和删除数据库)
  - [创建、查看和删除表](#创建查看和删除表)
  - [创建、查看和删除索引](#创建查看和删除索引)
  - [增删改查数据](#增删改查数据)
  - [创建、授权和删除用户](#创建授权和删除用户)
- [TiDB 集群监控](#tidb-集群监控)
  - [重要监控指标详解](#重要监控指标详解)
- [TiDB 集群扩容缩容方案](#tidb-集群扩容缩容方案)
- [集群销毁](#集群销毁)
...
5.2 连网下载 TiDB、TiKV 和 PD binaries：

  ```
  ansible-playbook local_prepare.yml
  ```

5.3 初始化目标机器的系统环境，修改内核参数：
...
PD | Store Status  -- down store | TiKV 异常节点数量 | 如果大于 0，证明有节点不正常
...
TiDB | handle_requests_duration_seconds | 请求 PD 获取 TSO 响应时间 | 小于 100ms
TiDB | tidb server QPS | 集群的请求量 | 这个和业务相关
TiDB | connection count | 从业务服务器连接到数据库的连接数 | 和业务相关。但是如果连接数发生跳变，需要查明原因。比如突然掉为 0，可以检查网络是否中断；如果突然上涨，需要检查业务。
TiDB | statement count | 单位时间内不同类型语句执行的数目 | 这个和业务相关
TiDB | Query Duration 99th percentile | 99% 的 query 时间 |
TiKV | 99% & 99.99%  scheduler command duration | 99% & 99.99% 命令执行的时间 | 99% 小于 50ms；99.99% 小于 100ms
TiKV | 95% & 99% storage async_request duration | 95%  & 99% Raft 命令执行时间 | 95% 小于 50ms；99% 小于 100ms
...
TiKV | 95% send_message_duration_seconds | 95% 发送消息的时间 | 小于 50ms
TiKV | leader/region | 每个 TiKV 的 leader/region 数量 | 和业务相关
...
4. 登录新增的 PD 节点，编辑启动脚本：`{deploy_dir}/scripts/run_pd.sh`     
    ...
    4.2 添加 `join="http://172.16.10.1:2379"`。IP 地址 (172.16.10.1) 可以是集群内现有 PD IP 地址中的任意一个。  
      
    4.3 在新增 PD 节点中手动启动 PD 服务：  
    `{deploy_dir}/scripts/start_pd.sh`   
      
    4.4 使用 `pd-ctl` 检查新节点是否添加成功：  
