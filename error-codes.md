...
* Error Number: 8028
    ...
    在关闭元数据锁的情况下，当事务执行时，TiDB 表结构发生了变化是无法被事务感知到的。因此，TiDB 在事务提交时，会对事务涉及表的结构进行检查。如果事务执行中表结构发生了变化，则事务将提交失败，并返回该错误。遇到该错误，应用程序可以安全地重新执行整个事务。
    在打开元数据锁的情况下，非 RC 隔离级别中，如果事务开始到初次访问一个表之间，该表进行了有损的列类型变更操作（例如 int 类型变成 char 类型），则访问该表的语句报错，事务不会自动回滚，用户可以继续执行其他语句，并决定是否回滚或者提交事务。
