...
* Error Number: 8158
    ...
    传入的文件路径不合法。请根据具体的错误提示进行处理。S3/GCS 路径可参考[外部存储](/br/backup-and-restore-storages.md)。
...
* Error Number: 8166
    ...
    某些选项只能在特定的条件下才可以使用。请根据具体的错误提示进行处理。请参考 [`IMPORT INTO`](/sql-statements/sql-statement-import-into.md) 查看支持的选项。
...
* Error Number: 8173
    ...
    执行 `IMPORT INTO` 时，TiDB 会对当前环境进行检查，比如检查下游表是否为空等。请根据提示进行处理。
