...
- SQL 返回 `ERROR 1105 (HY000): Out Of Memory Quota![conn_id=54]`。注意如果配置了 `tidb_mem_quota_query`，此报错为正常行为，不属于故障。这是由于数据库层面 SQL 发生 'Out of Memory Quota'。该场景主要是数据库的内存使用控制行为导致的。
