...
The global variables associated with the `CREATE INDEX` statement are `tidb_ddl_reorg_worker_cnt`, `tidb_ddl_reorg_batch_size` and `tidb_ddl_reorg_priority`. Refer to [TiDB-specific system variables](/v2.1/reference/configuration/tidb-server/tidb-specific-variables.md#tidb-ddl-reorg-worker-cnt) for details.
