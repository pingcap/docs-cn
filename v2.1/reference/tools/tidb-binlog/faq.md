...
category: FAQ
aliases: ['/docs-cn/v2.1/faq/tidb-binlog/']
---
...
1. 使用 [binlogctl 将该 Pump 状态修改为 offline](/v2.1/reference/tools/tidb-binlog/maintain.md)（丢失这个 Pump 的数据）
...
2. 使用 [binlogctl 将老的 Drainer 状态修改成 offline](/v2.1/reference/tools/tidb-binlog/maintain.md)。

假如 checkpoint 不在，可以如下处理：

1. 获取之前 Drainer 的 checkpoint `commit-ts`，做为新部署 Drainer 的 `initial-commit-ts` 配置来部署新的 Drainer。
2. 使用 [binlogctl 将老的 Drainer 状态 修改成 offline](/v2.1/reference/tools/tidb-binlog/maintain.md)。
