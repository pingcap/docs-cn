...
DM replicates the upstream MySQL or MariaDB instance table that matches the [`schema-pattern`/`table-pattern` rule provided by Table selector](/v2.1/reference/tools/data-migration/table-selector.md) to the downstream `target-schema`/`target-table`.
...
- [`schema-pattern`/`table-pattern`](/v2.1/reference/tools/data-migration/table-selector.md): the binlog events or DDL SQL statements of upstream MySQL or MariaDB instance tables that match `schema-pattern`/`table-pattern` are filtered by the rules below.
...
> **Note:**
>
> The column mapping is not recommended as the primary solution due to its usage restrictions. The preferable solution is [handling conflicts of auto-increment primary key](/v2.1/reference/tools/data-migration/usage-scenarios/best-practice-dm-shard.md#handle-conflicts-of-auto-increment-primary-key).
...
- [`schema-pattern`/`table-pattern`](/v2.1/reference/tools/data-migration/table-selector.md): to execute column value modifying operations on the upstream MySQL or MariaDB instance tables that match the `schema-pattern`/`table-pattern` filtering rule.
...
See the `replicate lag` in the [binlog replication](/v2.1/reference/tools/data-migration/monitor.md#binlog-replication) processing unit of DM monitoring metrics.
