...
2. 设置参数：
    ...
    **配置项层次结构使用 `.` 表示**。
    ...
    关于组件的更多配置参数说明，可参考 [tidb `config.toml.example`](https://github.com/pingcap/tidb/blob/v5.0.0/config/config.toml.example)、[tikv `config.toml.example`](https://github.com/tikv/tikv/blob/v5.0.0/etc/config-template.toml) 和 [pd `config.toml.example`](https://github.com/pingcap/pd/blob/v5.0.0/conf/config.toml)。
...
如果要调整 tidb-server 中事务大小限制参数 `txn-total-size-limit` 为 `1G`，该参数位于 [performance](https://github.com/pingcap/tidb/blob/v5.0.0/config/config.toml.example) 模块下，调整后的配置如下：
