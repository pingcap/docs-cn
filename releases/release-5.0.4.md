...
aliases: ['/zh/tidb/dev/release-5.0.4/','/zh/tidb/v5.0/release-5.0.4','/zh/tidb/v5.4/release-5.0.4','/zh/tidb/v6.1/release-5.0.4','/zh/tidb/v6.5/release-5.0.4','/zh/tidb/v7.1/release-5.0.4','/zh/tidb/v7.5/release-5.0.4','/zh/tidb/v8.1/release-5.0.4']
---
...
+ TiDB
    ...
    + 以下 Bug 修复涉及执行结果变化，可能引起兼容性变化：
        ...
        - 修复了 `BIT` 类型值做除法时查询结果不正确的问题 [#23479](https://github.com/pingcap/tidb/issues/23479)
...
+ TiDB
    ...
    - 增大 `split region` 的速度上限，使 `split table` 和 `presplit` 更稳定 [#22969](https://github.com/pingcap/tidb/issues/22969)
    ...
    - 在 MPP 模式中彻底下推 `COUNT(DISTINCT)` 聚合函数 [#25861](https://github.com/pingcap/tidb/pull/25861)
    - 当聚合函数在 `EXPLAIN` 语句中不能被下推时打印警告日志 [#25736](https://github.com/pingcap/tidb/pull/25736)
    - 在 Grafana 监控中为 `TiFlashQueryTotalCounter` 加上错误标签 [#25327](https://github.com/pingcap/tidb/pull/25327)
...
+ TiKV
    ...
    - 避免在低写入流量下误报 "GC can not work" 错误 [#10662](https://github.com/tikv/tikv/pull/10662)
    - 使数据库在备份时总是与原始集群大小相匹配 [#10643](https://github.com/tikv/tikv/pull/10643)
...
+ Tools
    ...
    + TiCDC
        ...
        - 禁止使用不同的 major 和 minor 版本启动 TiCDC 节点 [#2598](https://github.com/pingcap/tiflow/pull/2598)
        - 当某张表的 Region 从某个 TiKV 节点全部迁移走时，减少 goroutine 资源的使用 [#2284](https://github.com/pingcap/tiflow/issues/2284)
        - 移除 `file sorter` 文件排序器 [#2326](https://github.com/pingcap/tiflow/pull/2326)
        - 总是从 TiKV 获取行变更的历史值 (old value)，输出会根据 `enable-old-value` 配置进行调整 [#2301](https://github.com/pingcap/tiflow/issues/2301)
        ...
        - 为所有 KV 客户端创建全局共享的 gRPC 连接池 [#2533](https://github.com/pingcap/tiflow/pull/2533)
...
+ Tools
    ...
    + TiCDC
        ...
        - 修复重新调度一张表时多个处理器将数据写入同一张表引发的数据不一致的问题 [#2417](https://github.com/pingcap/tiflow/pull/2417)
        - 减小 gRPC 窗口来避免 Region 数量过多时触发内存溢出 [#2724](https://github.com/pingcap/tiflow/pull/2724)
        - 修复内存压力大时 gRPC 连接频繁断开的错误 [#2202](https://github.com/pingcap/tiflow/issues/2202)
        - 修复 TiCDC 在处理无符号 `TINYINT` 类型时崩溃的问题 [#2648](https://github.com/pingcap/tiflow/issues/2648)
        - 修复 TiCDC Open Protocol 在上游插入事务并删除同一行数据的情况下输出空值的问题 [#2612](https://github.com/pingcap/tiflow/issues/2612)
        - 修复同步任务从一个表结构变更的 finish TS 开始时 DDL 处理失败的问题 [#2603](https://github.com/pingcap/tiflow/issues/2603)
        - 修复无响应的下游中断 old owner 中的同步任务直到该任务超时的问题 [#2295](https://github.com/pingcap/tiflow/issues/2295)
        - 修复元信息管理问题 [#2558](https://github.com/pingcap/tiflow/pull/2558)
        - 修复 TiCDC 切换 owner 后出现数据不一致的问题 [#2230](https://github.com/pingcap/tiflow/issues/2230)
        - 修复 `capture list` 命令输出中出现已过期 capture 的问题 [#2388](https://github.com/pingcap/tiflow/issues/2388)
        - 修复集成测试中遇到的由于 DDL Job 重复导致的 `ErrSchemaStorageTableMiss` 错误 [#2422](https://github.com/pingcap/tiflow/issues/2422)
        - 修复遇到 `ErrGCTTLExceeded` 错误时 changefeed 无法被删除的问题 [#2391](https://github.com/pingcap/tiflow/issues/2391)
        - 修复同步数据量大的表到 cdclog 失败的问题 [#1259](https://github.com/pingcap/tiflow/issues/1259) [#2424](https://github.com/pingcap/tiflow/issues/2424)
        - 修复客户端向后兼容性问题 [#2373](https://github.com/pingcap/tiflow/issues/2373)
        - 修复 `SinkManager` 中对 map 的不安全并发访问 [#2299](https://github.com/pingcap/tiflow/pull/2299)
        - 修复 owner 在执行 DDL 语句时崩溃可能导致 DDL 任务丢失的问题 [#1260](https://github.com/pingcap/tiflow/issues/1260)
        - 修复在 Region 初始化时立刻执行清锁的问题 [#2188](https://github.com/pingcap/tiflow/issues/2188)
        - 修复创建新的分区表时部分分区被重复分发的问题 [#2263](https://github.com/pingcap/tiflow/pull/2263)
        - 修复同步任务已删除但 TiCDC 持续报警的问题 [#2156](https://github.com/pingcap/tiflow/issues/2156)
