...
* BR 快照恢复速度最高提升 10 倍（实验特性）[#33937](https://github.com/pingcap/tidb/issues/33937) @[3pointer](https://github.com/3pointer) **tw@Oreoxmt** <!--1647-->
    ...
    目前，新的并行恢复算法为实验特性，你可以配置 `br` 新增的命令行参数 `--granularity=coarse-grained` 使用新算法，同时通过设置 `--tikv-max-restore-concurrency` 控制每个 TiKV 节点下载任务的并发度。例如：
...
* 系统变量 [`tidb_disable_txn_auto_retry`](/system-variables.md#tidb_disable_txn_auto_retry) 将从 v8.0.0 开始废弃，后续不再支持乐观事务的自动重试。
* 废弃功能 2
