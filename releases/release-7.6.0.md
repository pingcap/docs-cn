...
* 跨数据库绑定执行计划 [#48875](https://github.com/pingcap/tidb/issues/48875) @[qw4990](https://github.com/qw4990) **tw@Oreoxmt** <!--1613-->
    ...
    针对这种应用场景，TiDB 引入了 "跨数据库绑定"，用一个执行计划绑定匹配模式相同的 SQL，即使他们运行在不同数据库上。比如，当我们创建绑定时在表名前增加通配符`*`如下，无论 `t1` 和 `t2` 位于哪个数据库，TiDB 都会尝试用此数据库绑定来生成执行计划，而不需要为每个数据库上的 SQL 单独创建。
    ...
    由于 "通用绑定" 会引入一个很小的系统开销（小于1%）， 默认将其关闭。 有需要的用户通过设置变量 [`tidb_opt_fuzzy_binding`]() 为 `YES` 开启。
