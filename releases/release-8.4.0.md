...
<tbody>
  ...
  <tr>
    ...
    <td>引入系统变量 <code>tidb_auto_analyze_concurrency</code> 控制 TiDB 集群中自动统计信息收集操作的并发度，TiDB 会根据节点规模和硬件规格自动确定扫描任务的并发度。该功能通过充分利用系统资源，提高统计信息收集效率，从而减少手动调优，并确保集群性能稳定。</td>
  </tr>
  <tr>
    <td rowspan="1">SQL</td>
...
| [`tidb_auto_analyze_concurrency`](/system-variables.md#tidb_auto_analyze_concurrency-从-v840-版本开始引入)| 新增 | 设置 TiDB 集群中自动统计信息收集操作的并发度。在 v8.4.0 之前，该并发度固定为 `1`。你可以根据集群资源情况提高该并发度，从而加快统计信息收集任务的执行速度。|
...
+ TiDB
    ...
    - 支持通过系统变量 `tidb_auto_analyze_concurrency` 设置 TiDB 集群中自动统计信息收集操作的并发度 [#53460](https://github.com/pingcap/tidb/issues/53460) @[hawkingrei](https://github.com/hawkingrei)
