...
* 调整 [TiDB Lightning 在线文档](/tidb-lightning/tidb-lightning-overview.md)，使其目录结构更加合理和清晰。同时对文档中关于“后端模式”的描述进行了修改，使用物理导入模式替代原有 `local backend`，使用逻辑导入模式替代原有 `tidb backend`，以降低新用户的理解难度。
