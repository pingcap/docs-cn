...
+ TiDB
    ...
    - 通过减少过多的初始 Chunk 分配，提升 Join 和 `UPDATE`/`DELETE` 语句的内存使用效率，尤其适用于高并发负载和处理宽行的场景 [#68545](https://github.com/pingcap/tidb/issues/68545) @[solotzg](https://github.com/solotzg) <!-- component: execution --> <!-- pr: https://github.com/pingcap/tidb/pull/69965 -->
...
+ 工具
    ...
    + TiCDC
        ...
        - 优化忽略删除事件时 TiCDC Changefeed 的扫描性能，减少包含大量删除操作的工作负载在追赶历史数据期间不必要的 DML 解码 [#5430](https://github.com/pingcap/ticdc/issues/5430) @[asddongmen](https://github.com/asddongmen) <!-- component: cdc --> <!-- pr: https://github.com/pingcap/ticdc/pull/5767 -->
...
+ TiDB
    ...
    - 修复包含 `MODIFY COLUMN` 的多 Schema 变更会回退到事务模式回填，而不是使用 Ingest 或分布式回填的问题 [#70136](https://github.com/pingcap/tidb/issues/70136) @[joechenrh](https://github.com/joechenrh) <!-- component: ddl --> <!-- pr: https://github.com/pingcap/tidb/pull/70188 -->
    ...
    - 修复查询将 `IN` 子查询用作嵌套 `NOT IN` 表达式的操作数时，TiDB 可能发生 panic 并终止用户会话的问题 [#64854](https://github.com/pingcap/tidb/issues/64854) @[hawkingrei](https://github.com/hawkingrei) <!-- component: planner --> <!-- pr: https://github.com/pingcap/tidb/pull/70259 -->
    - 修复原本可安全使用执行计划缓存的查询在使用 `JSON_EXTRACT()` 时，无法使用预处理或非预处理执行计划缓存的问题 [#69522](https://github.com/pingcap/tidb/issues/69522) @[winoros](https://github.com/winoros) <!-- component: planner --> <!-- pr: https://github.com/pingcap/tidb/pull/70257 -->
    ...
    - 修复 `COM_STMT_SEND_LONG_DATA` 可以为单个预处理语句无限累积参数数据的问题。TiDB 现在使用会话级 `max_allowed_packet` 设置限制累计数据大小，并在超过限制时返回数据包过大的错误 [#69693](https://github.com/pingcap/tidb/issues/69693) @[bb7133](https://github.com/bb7133) <!-- component: sql-infra --> <!-- pr: https://github.com/pingcap/tidb/pull/70231 -->
    ...
    - 修复无特权用户可以通过 `INFORMATION_SCHEMA.USER_ATTRIBUTES` 读取其他用户属性的问题 [#70277](https://github.com/pingcap/tidb/issues/70277) @[djshow832](https://github.com/djshow832) <!-- component: sql-infra --> <!-- pr: https://github.com/pingcap/tidb/pull/70317 -->
    ...
    - 修复在悲观事务中执行 `LOAD DATA LOCAL INFILE` 时，遇到可重试的锁冲突后可能在内部重试，导致客户端连接状态不同步，并返回无效序列错误而不是原始死锁错误的问题 [#69793](https://github.com/pingcap/tidb/issues/69793) @[lance6716](https://github.com/lance6716) <!-- component: transaction, sql-infra, execution --> <!-- pr: https://github.com/pingcap/tidb/pull/70194 -->
    - 修复执行 `ALTER TABLE ... REORGANIZE PARTITION` 时，重建的全局索引可能缺少分区顺序位于重组分区之后、但未参与重组的分区中的行对应的索引项，导致使用这些索引的查询遗漏数据行，并且受影响的值无法正确执行唯一性约束的问题 [#70023](https://github.com/pingcap/tidb/issues/70023) @[mjonss](https://github.com/mjonss) <!-- component: ddl --> <!-- pr: https://github.com/pingcap/tidb/pull/70479 -->
...
+ TiKV
    ...
    - 修复 TiKV Coprocessor 计算下推的 `LIKE` 表达式时，如果涉及格式错误的 UTF-8 输入或模式、`BIT` 值或某些排序规则，TiKV 可能发生 panic 的问题 [#66597](https://github.com/pingcap/tidb/issues/66597) [#67082](https://github.com/pingcap/tidb/issues/67082) [#19811](https://github.com/tikv/tikv/issues/19811) @[jebter](https://github.com/jebter) <!-- component: execution, tikv --> <!-- pr: https://github.com/tikv/tikv/pull/19893 --> <!-- pr: https://github.com/tikv/tikv/pull/19894 -->
    ...
    - 修复仅启用后台任务资源管控时，TiKV 会不必要地将事务调度器切换为优先级调度，从而可能导致写密集型工作负载出现 5% 到 10% 性能下降的问题 [#19858](https://github.com/tikv/tikv/issues/19858) @[glorv](https://github.com/glorv) <!-- component: tikv --> <!-- pr: https://github.com/tikv/tikv/pull/19895 -->
    ...
    - 修复当内存引擎缓存预热卡住，且在 ACK 截止时间前重复收到 Leader Transfer 请求时，Region Leader 转移可能被无限期阻塞的问题 [#19776](https://github.com/tikv/tikv/issues/19776) @[overvenus](https://github.com/overvenus) <!-- component: tikv --> <!-- pr: https://github.com/tikv/tikv/pull/19921 -->
...
+ 工具
    ...
    + 备份恢复 (BR)
        ...
        - 修复停止日志备份任务后，BR 日志备份会残留过期 GC Safepoint，可能影响清理和 Safepoint 管理的问题 [#19832](https://github.com/tikv/tikv/issues/19832) @[Leavrth](https://github.com/Leavrth) <!-- component: br --> <!-- pr: https://github.com/tikv/tikv/pull/19911 -->
        - 修复多个恢复任务并发运行时，BR 无法正确更新 SST 下载限速，可能导致其中一个任务的限速变更不生效的问题 [#19454](https://github.com/tikv/tikv/issues/19454) @[Leavrth](https://github.com/Leavrth) <!-- component: br --> <!-- pr: https://github.com/tikv/tikv/pull/19924 -->
    ...
    + TiCDC
        ...
        - 修复 TiCDC Kafka Sink 校验中的多个问题，包括意外创建 Topic 或产生其他仅应在启动阶段发生的副作用、对已有 Topic 跳过 Schema Registry 检查，以及对非 TiCDC 创建的 Topic 错误校验 `replication-factor` [#5618](https://github.com/pingcap/ticdc/issues/5618) [#5720](https://github.com/pingcap/ticdc/issues/5720) @[3AceShowHand](https://github.com/3AceShowHand) <!-- component: cdc --> <!-- pr: https://github.com/pingcap/ticdc/pull/5811 -->
        - 修复启用 Claim-Check 后，TiCDC Kafka Sink 会为每个 Encoder 创建独立的存储客户端，导致资源使用量增加的问题 [#5719](https://github.com/pingcap/ticdc/issues/5719) @[3AceShowHand](https://github.com/3AceShowHand) <!-- component: cdc --> <!-- pr: https://github.com/pingcap/ticdc/pull/5811 -->
        - 修复 TiCDC Kafka Sink 错误的分类或封装可能不一致，导致重试判断和问题排查更加困难的问题 [#5790](https://github.com/pingcap/ticdc/issues/5790) @[3AceShowHand](https://github.com/3AceShowHand) <!-- component: cdc --> <!-- pr: https://github.com/pingcap/ticdc/pull/5811 -->
        - 修复批量创建大量空闲 Changefeed 时，由于 Coordinator 未遵循配置的 Scheduler 并发限制，TiCDC 的内存和 CPU 使用量可能出现峰值的问题 [#4831](https://github.com/pingcap/ticdc/issues/4831) @[wlwilliamx](https://github.com/wlwilliamx) <!-- component: cdc --> <!-- pr: https://github.com/pingcap/ticdc/pull/5396 -->
        ...
        - 修复 TiCDC 新架构中由于硬编码了最高兼容版本，TiCDC 可能错误拒绝较新版本的 PD、TiKV 或 TiCDC 的问题 [#4681](https://github.com/pingcap/ticdc/issues/4681) @[wlwilliamx](https://github.com/wlwilliamx) <!-- component: cdc --> <!-- pr: https://github.com/pingcap/ticdc/pull/5327 -->
