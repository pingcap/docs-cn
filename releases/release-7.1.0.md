...
| TiKV | [`region-compact-check-step`](/tikv-configuration-file.md#region-compact-check-step) | 修改 | 当启动分区 Raft KV (storage.engine="partitioned-raft-kv") 时，默认值为 5。 |
| TiKV | [`region-compact-min-redundant-rows`](/tikv-configuration-file.md#region-compact-min-redundant-rows) | 新增 | 触发 RocksDB compaction 需要的冗余的 MVCC 数据行数。默认值为 50000。 |
| TiKV | [`region-compact-redundant-rows-percent`](/tikv-configuration-file.md#region-compact-redundant-rows-percent) | 新增 | 触发 RocksDB compaction 需要的冗余的 MVCC 数据行所占比例。默认值为百分之 20。 |
