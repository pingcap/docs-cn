...
<tbody>
  <tr>
    <td rowspan="3">可扩展性与性能<br /></td>
      <td>TiKV 支持分区 Raft KV 存储引擎（实验特性）</td>
...
| TiKV | [`storage.engine`](/tikv-configuration-file.md#storageengine-从-v660-版本开始引入) | 新增 | 用于设置 engine 类型，可选值有 `raft-kv` 和 `partitioned-raft-kv`。该配置只能在创建新集群时指定，且后续无法更改。 |
