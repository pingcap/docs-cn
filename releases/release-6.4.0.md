...
| TiCDC | [`transaction-atomicity`](/ticdc/ticdc-sink-to-mysql.md#sink-uri-配置-mysqltidb) | 修改 | 默认值由 `table` 改为 `none`。该修改可降低同步延迟，确保系统不出现 OOM，同时，修改默认值后，系统只拆分少量的事务，而不是拆分所有事务。 |
