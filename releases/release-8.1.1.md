...
- 将变量 [`tidb_enable_parallel_hashagg_spill`](/system-variables.md#tidb_enable_parallel_hashagg_spill-从-v800-版本开始引入) 的默认值从 `ON` 修改为 `OFF`，以避免落盘导致查询结果出错的问题。对于从 v8.0.0 或 v8.1.0 升级到 v8.1.1 的集群，该变量会保持升级前的默认值 `ON`，建议将其手动修改为 `OFF` [#55290](https://github.com/pingcap/tidb/issues/55290) @[xzhangxian1008](https://github.com/xzhangxian1008)
