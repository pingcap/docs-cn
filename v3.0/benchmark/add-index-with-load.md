...
category: benchmark
aliases: ['/docs-cn/benchmark/add-index-with-load.md/']
---
...
3 TiDB，3 TiKV，3 PD on K8s cluster。
...
- `tidb_ddl_reorg_worker_cnt` 与 `tidb_ddl_reorg_batch_size` 非常大时，`admin show ddl` 命令可以看到 ddl job 的多次重试（例如 `Write conflict, txnStartTS 410327455965380624 is stale [try again later], ErrCount:38, SnapshotVersion:410327228136030220`），此时 Add Index 会持久非常久才能完成。

## 测试方案2 -- Add Index 目标列不涉及写入（仅查询）
...
对于 Add Index 的目标列仅有查询负载时，Add Index 对负载的影响不明显。

## 测试方案3 -- 集群负载不涉及 Add Index 目标列
...
对于 Add Index 的目标列与负载无关时，Add Index 对负载的影响不明显。
