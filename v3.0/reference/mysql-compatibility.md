...
> **注意：**
>
> 本页内容仅涉及 MySQL 与 TiDB 的总体差异。关于[安全特性](/v3.0/reference/security/compatibility.md)、[悲观事务模型](/v3.0/reference/transactions/transaction-pessimistic.md#和-mysql-innodb-的差异)的兼容信息请查看各自具体页面。
...
目前 TiDB 中进行数据插入时，TiDB 会保留 `VARCHAR` 类型数据的行末空格，会插入截断空格后的 `CHAR` 类型数据，在没有索引的情况下和 MySQL 行为保持一致。如果 `VARCHAR` 类型数据中存在 `UNIQUE` 索引，MySQL 在判断是否重复的时候，和处理 `CHAR` 类型数据一样，先截取 `VARCHAR` 数据末行空格再作判断；TiDB 则是按照保留空格的情况处理。
