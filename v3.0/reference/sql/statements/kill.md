...
* The `KILL TIDB` statement is a TiDB extension. If you are certain that the session you are attempting to kill is on the same TiDB server, set [`compatible-kill-query = true`](/v3.0/reference/configuration/tidb-server/configuration-file.md#compatible-kill-query) in your configuration file.
