...
In v3.0.x, v2.1.13 and later versions, TiDB supports a new feature called [Split Region](/v3.0/reference/sql/statements/split-region.md). This new feature provides the following new syntaxes:
...
To avoid the hotspot problem in this situation, you can use `SHARD_ROW_ID_BITS` and `PRE_SPLIT_REGIONS` when creating a table. For more details about `PRE_SPLIT_REGIONS`, refer to [Pre-split Regions](/v3.0/reference/sql/statements/split-region.md#pre_split_regions).
