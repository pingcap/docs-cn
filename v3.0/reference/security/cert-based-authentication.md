...
+ `require issuer`：指定签发用户证书的 CA 证书的 `subject` 内容。配置内容对应[生成 CA 密钥和证书](#生成-ca-密钥和证书) 中输入的内容。可以使用命令 `openssl x509 -noout -subject -in ca-cert.pem | sed 's/.\{8\}//'  | sed 's/, /\//g' | sed 's/ = /=/g' | sed 's/^/\//'` 来获取对应 `ca-cert` 的 `subject`。
...
通过更新服务端密钥和证书，可以为整个 TiDB 集群和用户更换新的 CA 证书、新的客户端密钥和证书、新的服务端密钥和证书。
