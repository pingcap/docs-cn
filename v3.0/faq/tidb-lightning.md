...
>**Note:**
>
> See also [TiDB Lightning Troubleshooting](/v3.0/how-to/troubleshoot/tidb-lightning.md) if you have encountered errors.
...
If the [TiDB-backend](/v3.0/reference/tools/tidb-lightning/tidb-backend.md) is chosen, or the target database is used to store checkpoints, it additionally requires these privileges:
...
3. If the modification previously has changed any table, [remove the corresponding checkpoint](/v3.0/reference/tools/tidb-lightning/checkpoints.md#--checkpoint-remove) too.
...
6. [Destroy the failed tables and checkpoints](/v3.0/how-to/troubleshoot/tidb-lightning.md#checkpoint-for-has-invalid-status)
...
TiDB Lightning only supports the SQL dump generated by [Mydumper](/v3.0/reference/tools/mydumper.md) or [CSV files](/v3.0/reference/tools/tidb-lightning/csv.md) stored in the local file system.
...
It is not recommended to directly use `nohup` in the command line to start `tidb-lightning`. You can [start `tidb-lightning`](/v3.0/reference/tools/tidb-lightning/deployment.md#step-4-start-tidb-lightning) by executing a script.
...
1-Gigabit network cards can only provide a total bandwidth of 120 MB/s, which has to be shared among all target TiKV stores. TiDB Lightning can easily saturate all bandwidth of the 1-Gigabit network and bring down the cluster because PD is unable to be contacted anymore. To avoid this, set an *upload speed limit* in [Importer's configuration](/v3.0/reference/tools/tidb-lightning/config.md#tikv-importer):
