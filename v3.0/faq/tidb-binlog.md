...
- 确认 pump 监控面板 **gc_tso** 时间是否与配置一致，版本 <= 3.0.0 的 pump 会保证非 offline 状态 drainer 消费了数据才会 gc，如果有不再使用的 drainer 需要使用 binlogctl 下线。
...
如果 pump 没法恢复, 可以如下处理:
    1. 使用 [binlogctl 修改这个 pump 状态成 offline](/v3.0/how-to/maintain/tidb-binlog.md)(丢失这个pump的数据)
...
2. 使用 [binlogctl 修改老的 drainer 状态成 offline](/v3.0/how-to/maintain/tidb-binlog.md)
...
TiDB 配置开启 `ignore-error` 写 binlog 失败后触发 critical error 告警， 后续都不会再写 binlog, 所以会有 binlog 数据丢失, 如
果要恢复同步需要如下处理：
