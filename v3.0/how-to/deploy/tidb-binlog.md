...
> - 下游使用 MySQL 或 TiDB 时应当保证上下游数据库的 sql_mode 具有一致性；同时如果存在上游 TiDB 能运行但下游 MySQL 不支持的 DDL 语句时（例如下游 MySQL 使用 InnoDB 引擎时同步语句`CREATE TABLE t1(a INT) ROW_FORMAT=FIXED;`） drainer 也会同步失败，此时可以在 drainer 配置中跳过该事务同时在下游手动执行兼容的语句。
