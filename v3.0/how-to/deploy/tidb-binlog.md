...
> - 下游使用 MySQL 或 TiDB 时应当保证上下游数据库的 sql_mode 具有一致性；同时如果存在上游 TiDB 支持但下游 MySQL 不支持的 DDL 语句时（例如`create table binlog_split_region(a int, b int) pre_split_regions=3;`） drainer 也会同步失败，此时可以在 drainer 配置中跳过该事务同时在下游手动执行兼容的语句。
