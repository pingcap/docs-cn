...
TiDB 提供两种语句级别的 Stale Read 使用方式，分别是指定一个精确的时间点和一个时间范围：
...
TIDB  提供 Session 级别的 Stale Read 使用方式，指定一个时间范围:

- 指定时间范围: 如需在 Session 级别让 TiDB 开启后续的查询读取在一个时间范围内尽可能新的数据并不破坏隔离级别，你可以通过设置一个 Session 变量来指定一个时间范围。要使用该方式，请参阅 [通过系统变量 `tidb_read_staleness` 读取历史数据'](/tidb_read_staleness.md)文档。