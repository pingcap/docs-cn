...
- [概述](#概述)
- [1. 准备机器](#1-准备机器)
- [2. 在中控机器上安装配置 Ansible](#2-在中控机器上安装配置-ansible)
- [3. 下载 TiDB-Ansible](#3-下载-tidb-ansible)
- [4. 分配机器资源，编辑 inventory.ini 文件](#4-分配机器资源编辑-inventoryini-文件)
- [5. 部署任务](#5-部署任务)
- [6. 测试集群](#6-测试集群)
- [7. 滚动升级](#7-滚动升级)
- [常见运维操作汇总](#常见运维操作汇总)
...
5.2 使用 `local_prepare.yml` playbook, 联网下载 master 版本的 binary 到中控机：
...
5.2 使用 `local_prepare.yml` playbook, 联网下载 master 版本 binary 到中控机：
...
- 使用 wget 下载 binary，解压后手动替换 binary 到 `tidb-ansible/resource/bin/`
