...
> - 考虑到新用户短期只能使用低性能的测试机来进行 TiDB 功能性测试，以及学习、熟悉 TiDB 的搭建、扩容、迁移等操作。若出现导入线上较大 SQL 到 TiDB 失败的问题，请考虑可能是由于测试机性能过低引起的，可尝试使用高性能服务器来验证。

### 生产环境

| 组件 | CPU | 内存 | 硬盘类型 | 单块硬盘大小 | 网络 | 实例数量(最低要求) |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| TiDB | 32核+ |  128 GB+  | SSD | 500 GB+ | 2块+ 万兆网卡| 2|
| PD | 16核+ | 32 GB+ | SSD | 200 GB+| 2块+ 万兆网卡| 3|
| TiKV|  32核+  |  128 GB+  | SSD | 200~500 GB | 2块+ 万兆网卡| 3|
