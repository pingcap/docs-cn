...
- `state_name=Disconnected`: The heartbeats of this store cannot be detected currently, which may be caused by a failure or network interruption.
...
- `state_name=Offline`: This store is in the process of offline, but the store is still in service.
- `state_name=Tombstone`: This store has completed the process of offline and has no data on it, so the instance can be shut down.
