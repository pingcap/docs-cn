...
- `AUTO_RANDOM` 列是否有符号位取决于该列是否具有 `UNSIGNED` 属性。不具有 `UNSIGNED` 属性的列有 1 个符号位，具有 `UNSIGNED` 属性的列没有符号位。
- `AUTO_RANDOM` 列隐式分配的值始终为正数。对于有符号位的列，隐式分配值的符号位始终为 `0`。该规则不适用于显式插入的值。
- 保留位的长度为 `64-R`，保留位的内容始终为 `0`。
- 自增位至少为 27 位。对于有符号列，必须满足 `R-1-S >= 27`；对于无符号列，必须满足 `R-S >= 27`。
