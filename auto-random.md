...
TiDB 自动分配的 `AUTO_RANDOM(S)` 列值共有 64 位。其中，`S` 表示分片位的数量，取值范围是 `1` 到 `15`，默认为 `5`。
...
`AUTO_RANDOM` 列隐式分配的值和自增列类似，也遵循 session 变量 [`auto_increment_increment`](/system-variables.md#auto_increment_increment) 和 [`auto_increment_offset`](/system-variables.md#auto_increment_offset) 的控制，其中隐式分配值的自增位 (ID) 将满足等式 `(ID - auto_increment_offset) % auto_increment_increment == 0`。
