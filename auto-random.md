...
>
> AUTO_RANDOM 的第二个参数 `R` 往往在应用程序的数值类型无法表示完整的 64 位整数时有用。例如 JSON number 类型的取值范围为 `[-(2^53)+1, (2^53)-1]`，而 TiDB 很容易会为 `AUTO_RANDOM(5)` 的列分配出该范围以外的整数，导致应用程序读取该列的值时出现异常。此时可以用 `AUTO_RANDOM(5, 54)` 代替，TiDB 不会分配长度超过 54 位的整数。
