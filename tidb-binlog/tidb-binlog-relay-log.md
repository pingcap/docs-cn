...
Drainer 在将数据同步到下游之前，会先将数据写入到 relay log 文件中，当一个 relay log 文件大小达到 10MB（默认）后就会开始将数据写入到下一个 relay log 文件中。当 drainer 将数据成功同步到下游后，就会自动清理除当前正在写入的 relay log 文件外的其他已完成同步的 relay log 文件。
