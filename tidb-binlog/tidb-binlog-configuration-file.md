...
* 当设置为 true 时：
Drainer 会直接读取 checkpoint ts 时的 table info 。
由于直接读取特定时间点的表信息，这种方式通常更加高效。
但是，它受到 GC 机制的限制。因为 GC 可能会删除旧的数据版本，所以如果 checkpoint ts 太旧，那么对应时间点的表信息可能已经被 GC 删除，导致无法直接读取。
