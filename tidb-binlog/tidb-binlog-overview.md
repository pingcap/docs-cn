...
* TIDB Binlog 和 TiDB 在 v5.1 版本中解决了 v5.0 版本中引入的聚簇索引与 TiDB Binlog 不兼容问题。在升级 TiDBlog 和 TIDB 到 v5.1 版本后：开启 TiDB Binlog 后 TiDB 支持创建聚簇索引表；聚簇索引表的数据插入、删除和更新动作支持通过 TiDB Binlog 同步到下游。对于聚簇索引表同步需注意:

    - 如果从 v5.0 版本手动控制组件升级顺序进行升级，请确保将 TiDB Binlog 升级至 v5.1 版本后再将将 TiDB Server 升级至 v5.1 版本。
