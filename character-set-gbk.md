...
* 如果查询条件中包含对字符串前缀的`LIKE`过滤，比如 `LIKE 'prefix%'`，并且该列被设置为 gbk 的排序规则（`gbk_bin` 或 `gbk_chinese_ci`），那么 TiDB 优化器暂时无法把这个过滤条件转化为范围扫描 (Range Scan)，而是用全量扫描代替。这类 SQL 有可能造成超出预期的资源消耗。
