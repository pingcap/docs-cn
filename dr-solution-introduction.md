...
本文首先会对TiDB 提供的各种容灾方案进行介绍。之后会根据用户对容灾系统的 RPO 和 RTO 要求，以及系统的业务重要性，给出推荐的容灾解决方案。
...
这种架构看起来非常简洁，可用性比较高，最大的错误容忍目标可以做到 Region 级别，写能力也能够得到扩展，RPO 在秒级别，RTO 在分钟级别，甚至更低。如果 RPO 为 0 并不是必须满足的要求，推荐在重要生产系统使用该容灾方案。对于该方案的详细信息，请参考[使用跨区域主备容灾集群](./dr-secondary-cluster.md)。
...
该方案最大的错误容忍目标可以达到 Region 级别，写能力也能够得到扩展，并且 RPO 为0，RTO 也可以达到分钟级别，甚至更低。如果 RPO 为 0 是必须满足的要求，推荐在重要生产系统使用该容灾方案。对于该方案的详细信息，请参考[同区域三中心 (AZ) 部署](/dr-multi-replica.md)。
...
基于备份恢复的容灾方案，目前，RPO 低于 5 分钟，而 RTO 则取决于需要恢复的集群数据大小，对于v6.5.0 版本的 BR，其恢复速度可以参考对应的文档了解更多信息。通常来说，大部分客户会把跨 Region 的备份作为数据安全的最后一道防线，是大多数系统都需要的。  对于该方案的详细信息，请参考[进行集群数据备份和恢复](/dr-backup-restore.md)。

另外，从 v6.5.0 版本开始，BR 支持基于 AWS 上的 EBS 快照的快速备份。如果你在 AWS 上运行 TiDB 集群，要求备份过程对集群没有任何影响，并且要求恢复的时间尽量短，可以考虑使用该特性来降低系统的 RTO。
