---
title: TiDB 容灾方案概述
summary: 了解 TiDB 提供的几种容灾方案，包括基于备份与恢复的容灾、基于主备集群的容灾和基于单集群多副本的容灾。
---

# TiDB 容灾方案概述

本文首先会介绍容灾解决方案涉及的基本概念，核心组件 TiDB、TiCDC 及 BR 的架构，然后会介绍提供的各种容灾方案，之后会根据不同的 RPO 和 RTO 要求，以及系统的业务重要性，给出推荐的容灾解决方案。
...
该方案最大的错误容忍目标可以达到 Region 级别，写能力也能够得到扩展，并且 RPO 为 0，RTO 也可以达到分钟级别，甚至更低。如果 RPO 为 0 是必须满足的要求，推荐在重要生产系统使用该容灾方案。对于该方案的详细信息，请参考[同区域三中心 (AZ) 部署](/dr-multi-replica.md)。
...
| 基于 TiCDC 的主备集群容灾方案 | 中等 | 单个 Region | < 10 秒 | < 5 分钟 | Region 之间的网络延迟要求小于 100 ms。 | 对灾备和响应时间有明确要求 (RPO > 0) 的重要生产系统。 |
| 基于多副本的单集群容灾方案 | 高 | 单个 Region | 0 | 分钟级 | Region 之间的网络延迟要求小于 30 ms。 | 对灾备和响应时间有明确要求 (RPO=0) 的重要生产系统。 |
| 多副本与 TiCDC 相结合的容灾解决方案 | 高 | 多个 Region | < 10 秒 | < 5 分钟 | 对于通过多副本进行容灾的区域，网络延迟建议小于 30 ms。对于第三区域与其他区域之间，建议延迟小于 100 ms。 | 对灾难恢复和响应时间有严格要求的关键生产系统。 |
