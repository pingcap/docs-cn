...
> 2. 执行 `tiup ctl:<cluster-version> pd -u http://<pd_ip>:<pd_port> [-i] config set enable-placement-rules true` 命令，以开启 PD 的 Placement Rules 功能。或通过 [pd-ctl](/pd-control.md) 执行对应的命令。
