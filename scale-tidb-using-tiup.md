...
> 2. 执行 `tiup ctl pd -u <pd-host>:<pd-port> config set enable-placement-rules true` 命令，以开启 PD 的 Placement Rules 功能。或通过 pd-ctl（[下载地址](https://download.pingcap.org/tidb-v4.0.0-rc.2-linux-amd64.tar.gz)）执行对应的命令。
