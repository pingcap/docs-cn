...
> **注意：**
>
> TiKV 中的 PD Client 会缓存 PD 节点列表，但在 4.0.3 版本之前不会定期自动更新，只有在 PD leader 发生切换或 TiKV 重启加载最新配置后才会更新；为避免 TiKV 缓存的 PD 节点列表过旧的风险，在扩缩容 PD 完成后，PD 集群中应至少包含一个扩缩容操作前就已经存在的 PD 节点成员，如果不满足该条件需要手动执行 PD transfer leader 操作以更新 TiKV 中的 PD 缓存列表。对于 4.0.3 及以后的版本，增加了定时自动更新 PD 节点的机制，可以降低此问题出现的概率，但仍应尽量避免在扩容新 PD 后直接一次性缩容所有扩容前就已经存在的 PD 节点。
