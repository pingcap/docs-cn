...
+ TiKV 对外访问地址。
+ 默认：${addr}
+ 在某些情况下，譬如 docker，或者 NAT 网络环境，客户端并不能通过 `--addr` 的地址来访问到 TiKV，这时候，你就可以设置 `--advertise-addr` 来让客户端访问
...
+ TiKV 对外访问服务状态地址
+ 默认：使用 `--status-addr`
+ 在某些情况下，譬如 docker，或者 NAT 网络环境，客户端并不能通过 `--status-addr` 的地址来访问到 TiKV，这时候，你就可以设置 `--advertise-status-addr` 来让客户端访问
