...
* Raftstore 线程池：
    ...
    * 处理 Raft 日志。如果 `store-io-pool-size` 的值为 0，将日志写入到磁盘，如果该值不为 0，将日志发给 StoreWriter 线程。
    * 当日志在多数副本中达成一致后，它就会把该日志发送给 Apply 线程。
* StoreWriter 线程池：负责将所有 Raft 日志写入到磁盘后，再把结果发回到 Raftstore 线程。
