...
* 从 v8.5.4 起，gRPC 线程池的默认大小（由 `server.grpc-concurrency` 控制）由固定值 `5` 调整为根据 CPU 核数进行自适应计算，具体计算公式请参考 [`server.grpc-concurrency`](/tikv-configuration-file.md#grpc-concurrency)。由于 gRPC 线程池几乎不会有多少计算开销，它主要负责网络 IO、反序列化请求，因此该配置通常不需要调整。
