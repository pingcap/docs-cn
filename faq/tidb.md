...
#### 1.1.23 TiDB 是否支持修改版本号？我们的安全漏洞扫描工具对 mysql version 有要求。

TiDB 目前是支持修改版本号的，可通过修改配置文件中的 `server-version` 配置项进行调整。在使用 tidb-ansible 部署集群时，同样可以通过 `conf/tidb.yml` 配置文件中的 `server-version` 来设置合适的版本号，以避免安全漏洞扫描不通过的问题。
