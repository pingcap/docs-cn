...
TiDB 在执行 SQL 语句时，会根据隔离级别确定一个对象的 `schema` 版本来处理该 SQL 语句，而且 TiDB 支持在线异步变更 DDL。执行 DML 时，可能同时有 DDL 语句在执行，而你需要确保每个 SQL 语句在同一个 `schema` 上执行。因此，当执行 DML 时，如果遇到正在执行中的 DDL 操作，TiDB 可能会报 `Information schema is changed` 的错误。此外，在部分并发 DDL 场景中，某条 DDL 语句自身执行失败后，TiDB 在返回原始错误前也会检查 `schema` 是否已经变化；如果该 DDL 使用的 `schema` version 已落后于最新 `schema` version，也可能返回此错误。
...
- 原因 5：在部分并发 DDL 场景中，某条 DDL 语句自身执行失败，同时执行期间有其他 DDL 推进了全局 `schema` version。TiDB 在返回原始 DDL 错误前会检查 `schema` 是否已变化；这条检查路径可能不携带具体相关表信息，因此不会按“是否同一张表”做精确判断。只要该 DDL 使用的 `schema` version 落后于最新 `schema` version，原始 DDL 错误就可能被转换为此错误。典型场景包括：对同一张表并发执行 `TRUNCATE TABLE` 与 `ALTER TABLE`，或者并发执行 `RENAME TABLE` 与 `ALTER TABLE` 等存在对象状态竞争的 DDL。

以上原因中，原因 1 与 DML 所涉及的表是否和正在执行的 DDL 涉及的表相同直接相关。原因 5 出现在部分并发 DDL 场景中，但其检查路径不按“是否同一张表”做精确判断。原因 1 和原因 2 都不会导致业务问题，相应的 DML 会在失败后重试。对于原因 3，需要检查 TiDB 实例和 PD 及 TiKV 的网络情况。
...
从 TiDB v7.4 开始，如果 JDBC URL 中未配置 `connectionCollation`，且 `characterEncoding` 未配置或配置为 `UTF-8`，[`collation_connection`](/system-variables.md#collation_connection) 变量值取决于 JDBC 驱动版本。详情请参考[JDBC 连接的排序规则](#当-jdbc-url-中未配置-connectioncollation-时jdbc-连接使用什么排序规则)。
