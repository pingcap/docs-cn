...
TiDB 采用了多版本并发控制 (MVCC)，为了使并发事务能查看到早期版本的数据，删除数据不会立即回收空间，而是延迟一段时间后再进行垃圾回收 (GC)。 你可以通过修改系统变量 [`tidb_gc_life_time`](/system-variables.md#tidb_gc_life_time)的值(默认值为 `10m0s`)配置多久触发一次 GC, 即数据删除后会被保留的时限。

在进行数据备份时，`tidb_gc_life_time` 的值会自动延长以顺利完成备份。
