...
当前通过[索引加速功能](/system-variables.md#tidb_ddl_enable_fast_reorg-从-v630-版本开始引入)产生的索引数据无法被 PITR 备份任务备份。

因此，在 PITR 恢复完成后，br 会将通过索引加速功能创建的索引数据删除再重新创建。如果在日志备份期间通过索引加速功能创建的索引很多或索引数据很大，建议在创建索引后进行一次全量备份。
