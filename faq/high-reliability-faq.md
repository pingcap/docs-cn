...
TiDB 在 v3.0.8 后支持修改 server 版本号，可以通过配置文件中的 [`server-version`](/tidb-configuration-file.md#server-version) 配置项进行修改。在使用 TiDB Ansible 部署集群时，同样可以通过 `tiup cluster edit-config clusterName` 配置文件中的 `server_configs - tidb - server-version` 来设置合适的版本号，并通过 reload -R tidb 生效，以避免出现安全漏洞扫描不通过的问题。
