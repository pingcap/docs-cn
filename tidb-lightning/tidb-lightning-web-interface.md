...
- 基本操作流程，请参见 [使用 TiDB Lightning 导入数据](/get-started-with-tidb-lightning.md)。
- 命令行选项，请参见 [TiDB Lightning 命令行参数](/tidb-lightning/tidb-lightning-command-line-full.md)。

要检查导入进度，请在 TiDB Lightning 日志中搜索 `progress` 关键字，或使用 [TiDB Lightning 监控面板](/tidb-lightning/monitor-tidb-lightning.md)。
