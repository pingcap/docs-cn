...
在 SST 导入模式下，唯一键/主键的冲突是单独处理的。相关内容将在接下来的章节进行介绍。

## SST 导入模式下解决重复问题

SST 导入模式下，TiDB Lightning 导入数据时先将数据转换成 KV 对数组（KV pairs），然后批量添加到 TiKV 中。与 TiDB-backend 模式不同，TiDB Lightning 在 SST 导入模式下直到任务结束才会检测重复行。因此，SST 导入模式下的重复错误不是通过 `max-error` 进行控制，而是通过 `duplicate-resolution` 配置项进行控制的。你可以通过配置该参数的行为，来决定如何处理有冲突的数据。
...
**conflict_error_v1** 记录所有[后端中的唯一键/主键冲突](#SST 导入模式下解决重复问题)。每对冲突有两行。
