...
| 检查集群是否开启了 PITR 或有 Changefeed 任务 | >= 6.5.0 | Lightning 物理导入模式不兼容 PITR 和 Changefeed，如果集群开启了 PITR，或者有运行中的 Changefeed，则 TiDB Lightning 会自动报错退出；如果明确 Lightning 导入的表不需要 PITR 或者 Changefeed 进行同步，则可跳过此检查项来使用 Lightning 物理导入模式导入数据。 |
