...
配置为 `error` 时，冲突数据导致的报错会直接导致导入任务终止。配置为 `replace` 或 `ignore` 时，可以通过进一步配置 [`conflict.threshold`](/tidb-lightning/tidb-lightning-configuration.md#tidb-lightning-任务配置) 控制冲突数据的上限，默认值为 9223372036854775807。
