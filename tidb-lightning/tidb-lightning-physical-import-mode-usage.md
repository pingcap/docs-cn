...
# Physical Import Mode 为目标表添加索引的方式。默认根据 TiDB 版本自动选择。如果 TiDB 版本早于 v7.0.0，则默认为 `false` 表示仍然会用 Lightning 将所需导入的数据以及索引编码成 kv pairs 后再一同导入到 TiKV。从 v7.0.0 版本开始，默认为 `true`，即 TiDB Lightning 会导完数据后，再使用 add index 的 SQL 来添加索引。
