...
在技术实现上，TiDB Lightning 不再会暂停全局的调度，而是只暂停目标表数据范围所在 region 的调度，大大降低了对在线业务带来的影响。

> **注意：**
>
> TiDB Lightning 仍然不支持导入数据到已有业务写入的数据表。
>
> TiDB 集群版本须大于等于 v6.1.0，否则 TiDB Lightning 仍然会暂停全局调度，数据导入会给在线业务带来严重影响。

TiDB Lightning 默认情况下即会采用新方式最小范围暂停集群调度，无需额外配置。但默认配置下，TiDB 集群仍然会因为数据导入太快，使在线业务的性能受到影响，所以需要额外配置几个选项来控制导入速度和其他可能影响集群性能的因素：
...
我们在测试中用 TPCC 测试模拟在线业务，同时用 TiDB Lightning 向 TiDB 集群导入数据，测试导入数据对 TPCC 测试结果的影响。测试结果如下：
...
因此，我们*强烈建议*，如果是 TiDB 生产集群上有延迟敏感型业务，并且并发较小，请勿使用 TiDB Lightning 导入数据到该集群，这会给在线业务带来较大影响。
