...
自 TiDB Lightning v7.1.0 版本起，TiDB Lightning 提供参数 [pause-pd-scheduler-scope](/tidb-lightning/tidb-lightning-configuration.md) 来控制暂停调度的范围，默认为 `table`，即只暂停目标表数据范围所在 region 的调度。当集群没有业务流量时，建议设置为 `global` 来减少来自调度器对导入的干扰。
