...
```toml
[mydumper]
# 本地源数据目录或 S3 等外部存储 URI。关于外部存储 URI 详情可参考 https://docs.pingcap.com/zh/tidb/v8.1/backup-and-restore-storages#uri-格式。
...
TiDB Lightning 目前仅支持由 Amazon Aurora、Hive 或者 Snowflake 导出快照生成的 Parquet 文件。要识别其在 S3 的文件组织形式，需要使用如下配置匹配到所有的数据文件：
