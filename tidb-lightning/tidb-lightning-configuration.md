...
# 设置 TiDB 会话变量，提升 Checksum 和 Analyze 的速度。注意，如果设置了参数 checksum-via-sql = true，会通过 `ADMIN CHECKSUM TABLE <table>` 这个 SQL 来执行 Checksum 操作，因此下面的 distsql-scan-concurrency = 15 以及 checksum-table-concurrency = 2 参数不会生效。
