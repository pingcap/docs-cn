...
# 在逻辑导入模式下，设置每个在下游 TiDB 服务器上执行的 SQL 语句的最大值。
# 该参数自 v8.0.0 版本开始引入。
# 这是在单个事务中执行的每个 INSERT 或 REPLACE 语句中 VALUES 部分的期望大小。
# 该参数不是一个强制限制。实际执行的 SQL 可能更长或更短，具体取决于导入的实际内容。
# 默认值为 `96KiB`，在 TiDB Lightning 是集群中唯一的客户端时，这是导入速度的最佳值。
# 由于 TiDB Lightning 的实现细节，该参数最大值为 96 KiB。设置更大的值会被忽略。
# 你可以减小该值以减少大事务对集群的压力。
# logical-import-batch-size = '96KiB'

# 在逻辑导入模式下，限制每个事务中插入的最大行数。
# 该参数自 v8.0.0 版本开始引入。默认值为 65536 行。
# 当 `logical-import-batch-size` 和 `logical-import-batch-rows` 都被指定时，首先达到阈值的参数将生效。
# 你可以减小该值以减少大事务对集群的压力。
