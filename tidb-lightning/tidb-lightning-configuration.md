...
# - ""：不进行冲突数据检测和处理。如果源文件存在主键或唯一键冲突的记录，后续步骤会报错。如果导入模式为逻辑导入，策略将直接被转换为 "error"。
...
# 控制 strategy 为 "replace" 时，能处理的冲突错误数的上限。仅在 strategy 为 "replace" 或 "ignore" 时可配置。默认为 9223372036854775807，表示几乎可以容忍所有错误。物理导入模式下仅当开启前置检查时该配置项有效。
# threshold = 9223372036854775807
# 控制冲突数据记录表 (conflict_records) 中记录冲突数据的条数上限。默认为 100。如果 strategy 为 "ignore"，则会记录被忽略写入的冲突记录。如果 strategy 为 "replace"，则会记录被覆盖的冲突记录。但在逻辑导入模式下，replace 策略无法记录冲突记录。物理导入模式下仅当开启前置检查时该配置项有效。
# max-record-rows = 100
