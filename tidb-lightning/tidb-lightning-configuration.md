...
- 引擎文件的最大并行数。每张表被切分成一个用于存储索引的“索引引擎”和若干存储行数据的“数据引擎”。这两项设置控制两种引擎文件的最大并发数。通常情况下，你可以使用默认值。
- 示例值：`2`
...
- I/O 最大并发数。I/O 并发量太高时，会因硬盘内部缓存频繁被刷新而增加 I/O 等待时间，导致缓存未命中和读取速度降低。
...
- 当 driver = "file" 时：
  - 为断点信息存放的文件路径
  - 默认存储路径为"/tmp/CHECKPOINT_SCHEMA.pb"
- 当 driver = "mysql" 时：
  - 为"用户:密码@tcp(地址:端口)/"格式的 URL
...
- 从 v7.3.0 开始引入的新版冲突数据处理策略
- 默认值：`""`
- 可选值：`""`、`"error"`、`"replace"`、`"ignore"`
- ""：
  - 在物理导入模式下，不进行冲突数据检测和处理。如果源文件存在主键或唯一键冲突的记录，后续步骤会报错
  - 在逻辑导入模式下，"" 策略将被转换为 "error" 策略处理
- "error"：检测到导入的数据存在主键或唯一键冲突的数据时，终止导入并报错
- "replace"：遇到主键或唯一键冲突的数据时，保留最新的数据，覆盖旧的数据。使用物理导入模式时，冲突数据将被记录到目标 TiDB 集群中的 `lightning_task_info.conflict_view` 视图中。在 `lightning_task_info.conflict_view` 视图中，如果某行的 `is_precheck_conflict` 字段为 `0`，表示该行记录的冲突数据是通过后置冲突检测发现的；如果某行的 `is_precheck_conflict` 字段为 `1`，表示该行记录的冲突数据是通过前置冲突检测发现的。你可以根据业务需求选择正确的记录重新手动写入到目标表中。注意，该方法要求目标 TiKV 的版本为 v5.2.0 或更新版本
...
- 默认值：`false`
- 仅当导入模式为物理导入模式时可以使用该参数
- 冲突记录数量高于 1,000,000 的场景建议配置 `precheck-conflict-before-import = true`，可以提升冲突检测的性能，反之建议关闭

##### `threshold`

- 控制 strategy 为 "replace" 或 "ignore" 时，能处理的冲突错误数的上限
- 默认值：`10000`
- 仅在 strategy 为 "replace" 或 "ignore" 时可配置
...
- 默认值：`10000`
- 从 v8.1.0 开始，TiDB Lightning 会自动将 `max-record-rows` 的值设置为 `threshold` 的值，并忽略用户输入，因此无需再单独配置 `max-record-rows`
- `max-record-rows` 将在未来版本中废弃
- 在物理导入模式下，当 strategy 为 "replace" 时会记录被覆盖的冲突记录
- 在逻辑导入模式下：
  - 当 strategy 为 "ignore" 时会记录被忽略写入的冲突记录
  - 当 strategy 为 "replace" 时，不会记录冲突记录
...
- 物理导入模式本地进行 KV 排序的路径
- 默认值：`""`
- 如果磁盘性能较低（如使用机械盘），建议设置成与 `data-source-dir` 不同的磁盘，这样可有效提升导入性能
...
- 默认值：`false`
- 设置为 `true` 时，TiDB Lightning 会在导入数据完成后，使用 add index 的 SQL 来添加索引
...
- 可选值：`"table"`、`"global"`
- 该参数自 v7.1.0 版本开始引入
- "table"：仅暂停目标表数据所在 Region 的调度。仅适用于 TiDB v6.1.0 及以上版本的目标集群
...
- 物理导入模式下，用于控制本地文件排序的 I/O 区块大小
- 默认值：`"16KiB"`
- 该参数自 v7.6.0 版本开始引入
- 取值必须大于或等于 `1B`
- 当 IOPS 成为瓶颈时，可以调大该参数的值以缓解磁盘 IOPS，从而提升数据导入性能
- 注意，如果仅指定数字（如 `16`），则单位为 Byte 而不是 KiB
...
- 引擎文件需按顺序导入时的区块大小比例系数
- 默认值：`0.75`
- 取值范围：`0 <= batch-import-ratio < 1`
- 该参数决定了比例系数，即在完全并发下"导入"和"写入"过程的持续时间比
- 可以通过计算 1 GiB 大小的单张表的（导入时长/写入时长）得到
...
- 默认值为 UTF-8 编码中的 "error" Rune 或 Unicode replacement character
...
- 如果输入数据为非严格格式，可能会将一条完整的数据分割成两部分，导致结果出错
...
- 只导入与该通配符规则相匹配的表
- 默认值：`['*.*', '!mysql.*', '!sys.*', '!INFORMATION_SCHEMA.*', '!PERFORMANCE_SCHEMA.*', '!METRICS_SCHEMA.*', '!INSPECTION_SCHEMA.*']`
...
- 默认值：`true`
- 当设置为 `true` 时：
 - 将把首行的内容作为表头处理，不作为数据导入
- 当设置为 `false` 时：
 - 首行也作为 CSV 数据导入
 - 请确保 CSV 文件的列顺序与目标表的列顺序一致，否则可能会导致数据差异
...
- 仅在 `header = true` 时生效
- 当设置为 `true` 时：
 - 导入数据时会根据 CSV 表头的字段名去匹配目标表对应的列名
 - 即使 CSV 文件和目标表列的顺序不一致也能按照对应的列名进行导入
- 当设置为 `false` 时：
 - 适用于 CSV 表头中的字段名和目标表的列名不匹配但列的顺序一致的情况
 - 导入时会直接忽略 CSV 表头的内容，以避免导入错误
 - 直接把 CSV 数据按照目标表列的顺序导入

##### `not-null`

- CSV 文件是否包含 NULL
- 默认值：`false`
- 如果设置为 `true`，CSV 所有列都不能解析为 NULL

##### `null`

- 如果 not-null = false（即 CSV 可以包含 NULL），为以下值的字段将会被解析为 NULL
- 默认值：`'\N'`
...
- 示例值：`15`
- 如果将 checksum-via-sql 设置为 "true"，则此参数设置不会生效
...
- CA 的公钥证书
- 示例值：`"/path/to/ca.pem"`
- 设置为空字符串可禁用 SQL 的 TLS

##### `cert-path`

- 该服务的公钥证书
- 示例值：`"/path/to/lightning.pem"`
- 默认为 `security.cert-path` 的副本

##### `key-path`

- 此服务的私钥
- 示例值：`"/path/to/lightning.key"`
- 默认为 `security.key-path` 的副本
...
- 从 v4.0.8 开始，checksum 的默认值由此前的 "true" 改为 "required"
- 注意：
 - Checksum 对比失败通常表示导入异常（数据丢失或数据不一致），因此建议总是开启 Checksum
 - 考虑到与旧版本的兼容性，依然可以设置 `true` 和 `false` 两个布尔值，其效果与 `required` 和 `off` 相同

##### `checksum-via-sql`

- 设置是否通过 TiDB 执行 ADMIN CHECKSUM TABLE <table> 操作
- 默认值：`"false"`
- 当设置为 `"false"` 时：通过 TiDB Lightning 下发 ADMIN CHECKSUM TABLE <table> 命令给 TiKV 执行
- 建议设置为 `"true"`：以便在 checksum 失败时更容易定位问题
- 当设置为 `"true"` 时：如果需要调整并发，请在 TiDB 中设置 `tidb_checksum_table_concurrency` 变量

##### `analyze`

- 配置是否在 CHECKSUM 结束后对所有表逐个执行 `ANALYZE TABLE <table>` 操作
- 默认值：`"optional"`
- 可选值：与 `checksum` 相同（`"required"`、`"optional"`、`"off"`）
