...
# 当后端模式为 "tidb"，该值可超过逻辑 CPU 的数量，用于增加导入并发，当下游写入没有瓶颈时，一定程度上可加速导入速度。
# region-concurrency = 
...
# 当 driver = "file"，则 dsn 为断点信息存放的文件路径，默认存储路径为 "/tmp/tidb_lightning_checkpoint.pb"；
# 当 driver = "mysql"，则 dsn 为 "用户:密码@tcp(地址:端口)/" 格式的 URL；若不设置该 URL，则默认会使用 [tidb] 部分指定的 TiDB 服务器来存储断点；
# 为减少目标集群的压力，建议指定另一台兼容 MySQL 的数据库服务器来存储断点。
# dsn = "/tmp/tidb_lightning_checkpoint.pb"

# 存储断点的数据库名称，默认值为 "tidb_lightning_checkpoint"。
...
# 当后端模式是 "local" 时，本地进行 KV 排序的路径。如果磁盘性能较低（如使用机械盘），建议设置成与 "data-source-dir" 不同的磁盘，可有效提升导入性能。
sorted-kv-dir = "/data/sort-dir"
...
# 当后端模式是 “local” 时，一次请求中发送的 KV 数量。
# send-kv-pairs = 32768
# 当后端模式是 "local" 时，TiKV 写入 KV 数据的并发度。当 TiDB Lightning 和 TiKV 直接网络传输速度超过万兆的时候，可以适当增加这个值。
# range-concurrency = 16

# 当后端模式是 "tidb" 时，插入重复数据时执行的操作；
# - replace：默认值，新数据替代已有数据
# - ignore：保留已有数据，忽略新数据
# - error：中止导入并报错
# on-duplicate = "replace"

# 当后端模式是 "importer" 时，tikv-importer 的监听地址（需改为实际地址）。
...
# 配置 CSV 文件的解析方式。
[mydumper.csv]
# CSV 文件是否包含表头；如果 header = true，将跳过首行。
header = true
...
# 仅当需要导入的数据文件名不满足 `dbname.tablename[.idx].parquet` 格式时需要配置该参数进行正确匹配。
...
# 后端模式为 "local" 时，需填写集群中某一台 tidb-server 的地址即可；
# 后端模式为 "tidb" 时，推荐使用 LoadBalance 地址。
host = "172.16.31.1"
port = 4000
user = "root"
...
# 表结构信息需从 TiDB 的 `status-port` 获取；
# 当后端模式为 "local"，该参数为必填项；
# 当后端模式为 "tidb"，该参数为非必填项。
status-port = 10080

# 填写集群中某一台 pd-server 的地址；
# 当后端模式为 "local"，该参数为必填项；
# 当后端模式为 "tidb"，该参数为非必填项。
...
# 默认值为 "optional"，此配置的可选配置项与 `checksum` 相同。
analyze = "optional"

# 默认值为 false；如果设置为 true，会在导入每张表后执行一次 level-1 Compact；
