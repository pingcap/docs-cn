...
strategy = ""
# 控制是否开启前置检查，即导入数据到 TiDB 前，先检查所需导入的数据是否存在冲突，冲突记录比例 >= 1% 的场景建议开启前置检查，可以提升冲突检测的性能，反之建议关闭。默认值为 false，表示仅开启后置冲突检测。取值为 true 时，表示同时开启前置冲突检测和后置冲突检测。仅当导入模式为物理导入模式时可以使用该配置项。
