...
# **注意：**
# 1. 对于参与同一批并行导入的每个 TiDB Lightning 实例，必须将此配置项设置为相同的值，否则将无法确保导入数据的正确性。
# 2. 如果开启并行导入模式，需要确保导入使用的用户（对于 tidb.user 配置项）有权限创建和访问此配置对应的库。
# 3. TiDB Lightning 在导入完成后会删除此 schema，因此不要使用已存在的库名配置该参数。
...
# 当使用并行导入模式时，由于多个 Lightning 实例同时导入一张表，因此要求此开关必须为 true。 
