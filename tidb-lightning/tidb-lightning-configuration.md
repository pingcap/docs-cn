...
| --check-only | 指定该参数后，仅进行前置检查和数据采样检查（从 v6.0 版本开始引入）。数据采样检查是 lightning 对待导入数据文件中一定比例的数据文件的前 N 行进行的数据检查，主要用于检查数据行数是否与列数匹配、字符类型列是否存在非法字符等等。该参数的可选值为 `default` 和 `rate,rows`。当设置为 `--check-only=default` 时，待进行数据采样检查的文件比例为 `1%`，待检查行数为 `1000` 行（即 lightning 会对待导入数据文件中 1% 的文件进行数据采样检查，并检查每个文件的前 1000 行）。在可选值 `rate,rows` 中，`rate` 为 (0, 1] 范围内的浮点数，代表文件采样的比例；`rows` 为正整数或 `-1`，代表每个采样的文件需检查的行数。当 `rows` 为 `-1` 时，表示对文件的所有行进行检查。比如，当 【`--check-only=0.01,-1`】时，lightning 会对待导入数据文件中 【1%】 的文件进行数据采样检查，并检查每个文件的所有行。 | 无对应配置项（仅在命令行中生效） |
