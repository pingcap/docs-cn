...
#  - remove：如果写入的数据 A 和目标表的数据 B 存在 Primary Key 或 Unique Key 冲突，则会将 A 和 B 这两条冲突数据从目标表移除，同时记录到目标 TiDB 中的 `lightning_task_info.conflict_error_v1` 表中。你可以根据业务需求选择正确的记录重新手动写入到目标表中。注意，该方法要求目标 TiKV 的版本为 v5.2.0 或更新版本。如果版本过低，则会启用 'none' 模式。
