...
TiDB v6.4.0 及之后版本的 TiDB Lightning 仅支持 `gzip`、`snappy`、`zstd` 格式的压缩文件。其他类型的压缩文件会报错。你可以修改文件名为 `.bak`，或将该类文件移出导入数据目录来避免此类错误。更多详情请参考[压缩导出的数据文件](/tidb-lightning/tidb-lightning-data-source.md#压缩文件)。
