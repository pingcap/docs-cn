...
输入文件大小统一约为 256 MB 时，使用 TiDB Lightning 效果最佳。如果输入单个大型 CSV 文件，TiDB Lightning 就只能使用一个线程来处理，这会极大降低导入速度。

解决此问题，建议先将 CSV 文件分割为多个文件。对于通用 CSV 格式文件，在没有读取整个文件的情况下无法快速确定行的开始和结束位置。因此，默认情况下 TiDB Lightning 不会自动分割 CSV 文件。但如果确定输入的 CSV 文件符合限制要求，则可以启用 `strict-format` 设置，TiDB Lightning 则会将大文件分割为多个 256 MB 的文件块进行并行处理。
...
当前，严格格式的 CSV 文件意味着每个字段仅占一行，也就是必须满足以下条件之一：

* 分隔符为空。
* 每个字段不包含 CR (`\r`）或 LF（`\n`）。

如果是非严格格式的 CSV 文件， `strict-format` 却被错误设置为 `true`，分布多行的完整字段会被拆成两块，导致解析失败，更严重的后果是难以发现导入了损坏数据。
