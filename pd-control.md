...
- `rank-formula-version` 适用于热点调度，其用来确定调度策略的算法版本，支持的值有 `["v1", "v2"]`。目前该配置的默认值为 `v1`。
    ...
    - 在特定场景下，`strict-picking-store` 为 `true` 的 `v1` 版本算法更为保守，只有当两个维度都变好的时候才会进行调度，有可能出现因为维度冲突无法继续均衡的情况，需要将 `strict-picking-store` 改为 `false` 才能取得更好的均衡效果。`v2` 版本算法则可以获得更好的均衡效果，并减少无效调度。
