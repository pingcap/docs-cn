...
目前执行计划缓存功能默认打开，可以通过变量 `tidb_enable_prepared_plan_cache` 启用或关闭这项功能。
...
- TiDB Server 实例内存阈值 <= 64 GB; tidb_prepared_plan_cache_size = 50
- TiDB Server 实例内存阈值 > 64 GB; tidb_prepared_plan_cache_size = 100
