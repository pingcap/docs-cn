...
TiDB 会尽最大努力兼容 MySQL 的协议，但是毕竟整个构架和 MySQL 是完全不一样的，所以不可能所有特性全部兼容。为了降低 MySQL 已有业务的迁移代码修改，我们采用了一些兼容做法，比如常见的 partition、innodb，在 TiDB 碰到此类语法不会报错都会跳过，但是实际什么都不做，更多的细节差异，请参考具体语法文档。
