...
这个变量用来设置当前 session 期待读取的历史数据所处时刻。
比如当设置为 "2017-11-11 20:20:20" 时，当前回话将能读取到该时刻的数据。
...
这个变量用来设置 ANALYZE 语句执行时读取索引的并发度。
当这个变量被设置得更大时，会对系统性能产生更多的影响。
...
这个变量用来设置 ANDMIN CHECKSUM TABLE 语句执行时扫描索引的并发度。
当这个变量被设置得更大时，会对系统性能产生更多的影响。
