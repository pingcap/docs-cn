...
Coalesce Partition 一种减少 Hash 分区表或 Key 分区表中分区数量的方法。详情参见 [管理 Hash 分区和 Key 分区](/partitioned-table.md#manage-hash-and-key-partitions).
...
[Partitioning](/partitioned-table.md)（分区）是指在物理上将一张表划分为更小的表分区，可以通过 RANGE、LIST、HASH 和 KEY 等分区方法来进行分区。
