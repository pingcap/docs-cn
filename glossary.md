...
Witness 副本只会存储最近的 Raft 日志来进行多数派确认，但不会存储数据。详情参见 [使用 Witness 节约成本](/use-witness-to-save-costs.md) 和 [使用临时 Witness 副本来加速副本恢复](/use-witness-to-speed-up-failover.md)。