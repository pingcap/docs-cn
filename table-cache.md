---
title: 缓存表
summary: 了解 TiDB 中的缓存表功能，用于很少有修改操作的热点小表的场景，提升性能。
---

# 缓存表

TiDB 在 v6.0.0 版本中引入了缓存表功能。该功能针对频繁被访问，且几乎很少修改的热点小表，将表的数据直接全部加载到 TiDB 服务器的内存中，避免访问 TiKV 获取数据，从而提升性能。

本文介绍了 TiDB 缓存表的使用场景、使用示例、与其他 TiDB 功能的兼容性限制。

## 使用场景

TiDB 缓存表的典型使用场景，需要满足以下条件：

- 表的数据量不大
- 只读，或者几乎很少修改
- 表的访问很频繁，期望有更好的性能

比如配置表，业务通过这张表读取配置信息；比如金融场景中的存储汇率的表，每天只更新一次，不会实时更新；或者银行的分行或者网点信息，新增加一条记录项并不频繁。

尤其是当表的数据量不大，访问又特别频繁的情况下，数据会集中在 TiKV 一个 region 上面，会形成热点，影响性能。
...
读取缓存表会用到 `UnionScan` 算子，所以通过 `explain` 看到 `UnionScan` 时，不要惊讶：
