...
2. 配置 TiDB 实例。
    ...
    - 将 TiDB 实例的 [`security.session-token-signing-cert`](/tidb-configuration-file.md#session-token-signing-cert) 和 [`security.session-token-signing-key`](/tidb-configuration-file.md#session-token-signing-key) 配置为上述证书的路径，否则连接不能迁移。
    - 配置 TiDB 实例的 [`graceful-wait-before-shutdown`](/tidb-configuration-file.md#graceful-wait-before-shutdown-从-v50-版本开始引入)，它的值要大于应用程序最长的事务的持续时间，否则 TiDB server 下线时客户端可能断连。请参阅 [TiProxy 使用限制](#TiProxy-使用限制)。 
