...
从 TiProxy v1.3.0 开始，TiProxy 支持捕获生产集群的流量，并将流量回放到测试集群。
...
- TiDB 扩缩容前性能验证：在新规模的测试集群上按对应速率回放流量，验证新规模集群的性能是否满足要求。例如，为了节省成本要将集群规模缩容到 1/2 时，按 1/2 速率回放流量，验证缩容后 SQL 延迟是否满足要求。
