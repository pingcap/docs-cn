...
* `RESTRICTED_STATUS_ADMIN` 允许在 SEM 打开的情况下查看敏感内容。
* `RESTRICTED_VARIABLES_ADMIN` 允许在 SEM 打开的情况下查看所有变量。
* `RESTRICTED_USER_ADMIN` 不允许使用 `SUPER` 用户撤销访问权限。
* `RESTRICTED_CONNECTION_ADMIN` 不允许通过 `PROCESS/CONNECTION_ADMIN` 权限来 kill 连接。
