---
title: 定位消耗系统资源多的查询
aliases: ['/docs-cn/v3.0/identify-expensive-queries/','/docs-cn/v3.0/how-to/maintain/identify-abnormal-queries/identify-expensive-queries/','/docs-cn/how-to/maintain/identify-abnormal-queries/identify-aborted-queries/','/docs-cn/v3.0/how-to/maintain/identify-abnormal-queries/identify-aborted-queries/']
---

# 定位消耗系统资源多的查询

TiDB 会将执行时间超过 [tidb_expensive_query_time_threshold](/tidb-specific-system-variables.md#tidb_expensive_query_time_threshold)（默认值为 60s），或使用内存超过 [mem-quota-query](/tidb-configuration-file.md#mem-quota-query)（默认值为 32 GB）的语句输出到 [tidb-server 日志文件](/tidb-configuration-file.md#logfile)（默认文件为：“tidb.log”）中，用于在语句执行结束前定位消耗系统资源多的查询语句（以下简称为 expensive query），帮助用户分析和解决语句执行的性能问题。
