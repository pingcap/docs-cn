...
Request Unit (RU) 是 TiDB 对 CPU、IO 等系统资源的统一抽象的单位，是一种计量单位，用于表示对数据库的单个请求消耗的资源量。请求消耗的 RU 数量取决于多种因素，例如操作类型或正在检索或修改的数据量。目前，RU 包含以下资源的统计信息：
...
* 每个写操作最终都被会复制到所有副本(TiKV 默认 3 数据副本)，并且每次复制都被认为是一个不同的写操作。
