...
使用 TiDB Ansible 或 TiUP 部署 TiDB 集群时，一键部署监控系统 (Prometheus/Grafana)，监控架构请看 [TiDB 监控框架概述](/tidb-monitoring-framework.md)。

目前 Grafana Dashboard 整体分为 PD、TiDB、TiKV、Node\_exporter、Overview 等，TiDB 分为 TiDB 和 TiDB Summary 面板。
TiDB 面板的主要作用是，提供尽可能全面的信息供排查集群异常。
TiDB Summary 面板是将 TiDB 面板中用户最为关心的部分抽取出来，并做些许修改组成。它的主要作用是，提供数据库的日常运行中用户关心的数据，如，QPS、TPS、响应延迟等，方便作为外部展示、汇报用的监控信息。
...
- Query Summary
    ...
    - QPS：所有 TiDB 上的每秒执行 SQL 数量。按照执行成功或失败（OK/Error）进行了区分
...
- Query Detail
    ...
    - Internal SQL OPS：整个 TiDB 集群内部 SQL 语句执行的 QPS。内部 SQL 是 TiDB 内部自动执行的 SQL，一般由用户 SQL 来触发或者内部定时任务触发
...
- Server
    ...
    - Go GC Duration：每个 TiDB 实例的 Golang GC 耗时统计
...
- GC
    ...
    - Delete Range Failure OPM：Delete range 失败的次数统计
...
- Batch Client
    ...
    - No Available Connection Counter：Batch 客户端找不到可用链接的次数统计
