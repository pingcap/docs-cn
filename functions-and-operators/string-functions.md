...
`FORMAT()` 函数用于将数字格式化为指定的小数位数，返回格式化为指定小数位数的数字。

函数根据指定的小数位数进行格式化。如果指定的小数位数大于实际数字的小数位数，结果会用零填充到相应的长度。

- 此函数接受两个参数：要格式化的数字和小数位数。
- 如果输入的第一个参数为字符串，且该字符串中只包含数字，该函数将按照该数字返回结果。例如，`FORMAT('12.34', 1)` 与 `FORMAT(12.34, 1)` 的返回结果相同。
- 如果输入的第一个参数为字符串，且该字符串第一个字符不是数字：例如，`FORMAT('q12.34', 5)`），则该函数返回 `0.00000`。
- 如果输入的第一个参数为字符串，且该字符串由数字和非数字组成，该函数将按照该参数中最前面连续的数字返回结果。例如，`“FORMAT('12.34q56.78', 1)` 与 `FORMAT('12.34', 1)` 的返回结果相同。
- 如果输入的第二个参数（小数位数）为负数，函数将舍去小数部分返回整数。
- 如果输入的任意参数为 `NULL`，函数将返回 `NULL`。

示例：

格式化数字 12.34 到不同的小数位数：
...
`FROM_BASE64()` 函数用于解码 Base-64 表示的字符串，返回其十六进制形式的字符串。

- 此函数接受一个参数：需要被解码的 Base64 编码的字符串。
- 如果输入参数不是有效的 Base64 编码，`FROM_BASE64()` 函数将返回 `NULL`。

> **注意：**
>
> `FROM_BASE64()` 函数仅处理有效的 Base64 编码字符串。

示例：

解码 Base64 编码的字符串 `'SGVsbG8gVGlEQg=='`（这是`'Hello TiDB'`经过 Base64 编码的结果）
