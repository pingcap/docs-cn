...
1. **`X`：数字或数字字符串：** 要格式化的数字。可以是直接的数字值、数字字符串、或科学记数法格式的数字。
2. **`D`：小数位数：** 指定返回值的小数位数。
3. **`[locale]`：区域设置（可选参数）：** 可选的第三个参数允许指定一个区域设置，用于结果数字的小数点、千位分隔符和分隔符之间的分组。合法的区域设置值与 `lc_time_names` 系统变量的合法值相同。如果未指定或者区域设置为 `NULL`，则默认使用 `'en_US'` 区域设置。
