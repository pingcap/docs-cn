...
summary: TiDB支持MySQL 5.7中的所有日期和时间函数。但是TiDB可能无法完全匹配MySQL的行为，建议使用正确的日期格式。TiDB支持str_to_date()函数，但无法解析所有日期和时间值。此外，TiDB不支持特定的日期和时间格式化选项。系统变量default_week_format影响WEEK()函数。更多信息，请参见GitHub Issue #30082。
---
