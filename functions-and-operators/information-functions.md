...
`LAST_INSERT_ID(expr)` 函数可以接受一个表达式作为参数，并将该值存储以供下一次调用 `LAST_INSERT_ID()` 时使用。你可以使用该函数生成序列，与 MySQL 保持兼容。注意 TiDB 也支持正常的[序列函数](/functions-and-operators/sequence-functions.md)。
