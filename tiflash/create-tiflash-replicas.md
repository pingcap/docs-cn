...
2. 使用 [PD Control](/pd-control.md) 逐步放开新增副本速度限制：
   ...
   ```shell
   tiup ctl:v6.3.0 pd -u http://<PD_ADDRESS>:2379 store limit all engine tiflash 60 add-peer
   ```
   ...
   ```shell
   tiup ctl:v6.3.0 pd -u http://<PD_ADDRESS>:2379 store limit all engine tiflash 90 add-peer
   ```
...
3. 在副本同步完毕后，恢复到默认配置，减少在线业务受到的影响。
   ...
   ```shell
   tiup ctl:v6.3.0 pd -u http://<PD_ADDRESS>:2379 store limit all engine tiflash 30 add-peer
   ```
