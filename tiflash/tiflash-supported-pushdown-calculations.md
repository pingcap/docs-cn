...
* [Window](/functions-and-operators/window-functions.md)：当前支持下推的函数包括窗口函数 `ROW_NUMBER()`、`RANK()`、`DENSE_RANK()`、`LEAD()`、`LAG()`、`FIRST_VALUE()`、`LAST_VALUE()` 和聚合函数 `MIN()`、`MAX()`、`AVG()`、`COUNT()`、`SUM()`。
