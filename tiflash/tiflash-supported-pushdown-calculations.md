...
类似 `MAX()` 这样的函数在聚合函数中支持下推，但是在窗口函数中不支持下推。
...
在该查询中，算子 Limit 被下推到 TiFlash 对数据进行过滤，减少了网络传输数据量，进而减少网络传输开销。具体可查看以上示例中 `Limit` 算子的 `task` 列，其值为 `mpp[tiflash]`，表示该算子被下推到 TiFlash。
