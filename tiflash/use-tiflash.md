...
* 字符串函数：`substr, char_length, replace, concat, concat_ws, left, right, ascii, length, trim, ltrim, rtrim, position, format, lower, ucase, upper, substring_index`
* 日期函数：`date_format, timestampdiff, from_unixtime, unix_timestamp(int), unix_timestamp(decimal), str_to_date(date), str_to_date(datetime), datediff, year, month, day, extract(datetime), date, hour, microsecond, minute, second, sysdate`
* JSON 函数：`json_length`
...
另外，所有包含 Bit/Set/Geometry 类型的表达式均不能下推到 TiFlash。
...
- **V3**: 在 V2 的基础上完善了数据校验功能，提供了调整校验帧大小和校验算法的接口，于 v5.4 版本引入，目前需手动开启（见 TiFlash [配置文件](/tiflash/tiflash-configuration.md#配置文件-tiflashtoml)）。
...
> **注意：**
>
> 设置使用 V3 版本后，新生成的 DTFile 将无法被 v5.4 以前 TiFlash 直接正常读取。

- **性能**： V2 时期已经具有数据文件部分的校验功能，V3 进行了格式上的变更并增大了校验功能的覆盖程度。V3 格式在设计时，更加注意了哈希函数的选择和内存对齐的处理以保障良好的性能。此外，增加校验的文件仅需要在初始化（DTFile 的首次加载）时进行读取，因此不会给数据读取增加额外的负担。经测试，升级到 V3 后 TiFlash 在 TPCH-100 等工作负载下未见性能回退。

- **兼容性**： v5.4 后 TiFlash 同时支持 V2，V3 版本，不会主动进行版本的升降级。如果需要迁移到新的版本，或者需要回退到旧的版本，需要手动使用 DTTool 进行[版本切换](/tiflash/tiflash-command-line-flags.md#dttool-migrate)。

- **空间**： V3 检验信息的存储方式如下：
    - 数据文件中每校验帧头部增加长度信息和本帧数据的哈希值。
    - 元信息，统计信息文件的哈希值单独序列化后存在 config 文件中。
    - config 文件中有效信息尾部添加有效信息的哈希值。

    默认情形下，帧大小为 `1M`，帧头大小为 `16B`，文件哈希值为 `8B`。文件储存 `100G` 数据，数据部分额外空间约为 `(100GB / 1MB * 16B) = 1638400B`, 即膨胀率为 `0.0015%`。信息文件部分需 `8B * (1 + 文件数)` 空间来储存哈希值，文件数目固定且小于`10`，对空间影响可忽略不计。
