...
[profiles.default]
    ...
    ## 设置为整数时，单位为 byte，比如 34359738368 表示 32 GB 的内存限制；其中 0 表示无限制
    ## 设置为 [0.0, 1.0) 之间的浮点数时，指节点总内存的比值，比如 0.8 表示总内存的 80%；其中 0.0 表示无限制
    ## 默认值为 0，表示不限制
    ## 当查询试图申请超过限制的内存时，查询终止执行并且报错
    max_memory_usage = 0

    ## 所有查询过程中，节点对中间数据的内存限制 
    ## 设置为整数时，单位为 byte，比如 34359738368 表示 32GB 的内存限制；其中 0 表示无限制
    ## 设置为 [0.0, 1.0) 之间的浮点数时，指节点总内存的比值，比如 0.8 表示总内存的 80%；其中 0.0 表示无限制
    ## 默认值为 0.8，表示总内存的 80%
    ## 当查询试图申请超过限制的内存时，会终止执行并且报错
