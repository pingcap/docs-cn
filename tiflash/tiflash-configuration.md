...
[profiles.default]
    ...
    ## 从 v6.4.0 引入，表示当前 tiflash 节点最多可以同时运行多少个 query。在 v6.5 和 v7.1 版本中，该参数的默认值是 vcpu 数量的四分之一。用户在设置时，该参数的范围在 0 到五十倍 vcpu 之间，如果参数值超过这个范围，那么会被自动调整为 vcpu 数量的四分之一。在 v7.1 版本中，该参数的默认值是 vcpu 数量的四分之一。在 v7.5 和 v8.1 版本中，该参数的默认值是两倍的 vcpu 数量。
