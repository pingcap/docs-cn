...
listen_host = "0.0.0.0"
## TiFlash TCP 服务的端口。TCP 服务为内部测试接口，默认端口为 9000。在 TiFlash 7.1.0 之前的版本中该端口默认开放，可以在未经授权的情况下通过该端口执行高权限的 SQL，查看关键数据、修改操作数据。在 TiFlash 7.1.0 及之后的版本中，如果不配置该项，则该端口不会开启。建议在任何 TiFlash 的部署中都不配置该项。
# tcp_port = 9000
