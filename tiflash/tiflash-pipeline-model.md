...
- v7.4.0 及之后版本：TiFlash 正式支持 Pipeline Model。Pipeline Model 属于 TiFlash 内部特性，且 Pipeline Model 和 TiFlash resource control 功能相互依赖，因此，开启 TiFlash resource control 功能时，Pipeline Model 模型将自动启用。具体 TiFlash resource control 功能的启用方式和查看方式，请参考 [TiFlash Resource Control](/tidb-resource-control.md#TiFlash Resource Control)，同时变量 `tidb_enable_tiflash_pipeline_model` 被废弃。
