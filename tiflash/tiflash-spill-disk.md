...
* 即使配置了查询级别的落盘，且查询中也含有支持落盘的算子，但是因为查询中其他不能落盘的算子占用的内存不能释放，而且可以落盘算子本身也可能会落盘不及时等原因，对于一个查询而言，仍然是有可能因为内存超过阈值而报错的。对于落盘算子落盘不及时的情况，可以尝试调小 [`tiflash_query_spill_ratio`](/system-variables.md#tiflash_query_spill_ratio-从-v740-版本开始引入) 来避免查询内存超阈值报错。
