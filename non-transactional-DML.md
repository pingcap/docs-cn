---
title: 非事务 DML 
summary: 以事务的原子性和隔离性为代价，将 DML 拆成多个语句依次执行，用以提升批量数据处理场景的稳定性和易用性。
---
...
非事务 DML 执行过程中，可以通过 `show processlist` 查看执行进度，其中的 `Time` 显示的是当前 batch 执行的耗时。日志、慢日志等也会记录每个拆分后的语句在整个非事务 DML 中的进度。
...
非事务 DML 尚不能替代所有的 batch-dml 使用场景。它们的主要区别有:
