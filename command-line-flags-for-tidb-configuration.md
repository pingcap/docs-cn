...
+ 默认：false
+ 可以使用 `./tidb-server --redact=xxx collect-log <input> <output>` 将 `<input>` 日志文件进行脱敏或者还原，并输出到 `<output>`。当值为 `true` 时脱敏，为 `false` 时还原。更多详情，请参考系统变量 [tidb_redact_log](/system-variables.md#tidb-redact-log)。
