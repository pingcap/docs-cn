...
在集群上，如果出现了数据丢失大部分副本或数据同步不完整等问题，从而导致出现数据包含不完整事务的情况，你可以使用 `reset-to-version` 命令用于恢复 ACID 不一致的数据。此时你需要提供一个能保证 ACID 一致性的旧版本号。`tikv-ctl` 会清除掉在此版本之后的所有数据。

- `-v` 选项用于指定恢复的版本号。所使用的 `-v` 参数可以使用 `pd-ctl min-resolved-ts` 查得。
...
> **注意：**
>
> - 该命令只支持在线模式。在执行命令前，你需要停止 TiDB 等会向 TiKV 写入数据的进程。在命令执行成功后，会返回 `success!`。
> - 你需要对集群中所有的 TiKV 节点执行相同的命令。
> - 操作之前应该停止 PD 的所有调度。
