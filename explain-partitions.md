...
使用 `EXPLAIN` 语句可以查看 TiDB 为了执行查询而需要访问的分区。由于存在[分区裁剪](/ partition-pruning.md)，显示的分区通常只是整个分区的一个子集。本文档介绍了常见分区表的一些优化，以及如何解读 `EXPLAIN` 的输出结果。
...
由上述查询可知，从最内部的 `—TableFullScan_19` 算子开始，再返回到根算子 `StreamAgg_21` 的过程中：

* TiDB 成功地识别出只需要访问一个分区（`p2017`），并将其在 `access object` 列中注明。
* 分区本身在 `└─TableFullScan_19` 算子中进行扫描，然后执行 `└─Selection_20` 算子对起始日期为 `2017-06-01 00：00：00.000000` 的行进行筛选。
* 与 `└─Selection_20` 算子匹配的行在 Coprocessor 中进行流聚合且 Coprocessor 本身就可以理解 `count` 聚合函数。
* 每个 Coprocessor 请求将一行数据返回给 TiDB 中的 `└─TableReader_22` 算子，然后将数据在`StreamAgg_21` 下进行流聚合，并将一行数据返回给客户端。
...
* 扫描每个分区时，`Selection` 算子将筛选出与 2017 年不匹配的行。
* 在每个分区上执行流聚合以计算匹配的行数。
* `└─PartitionUnion_21` 算子会将访问每个分区的结果合并。
