...
要快速了解 TiUP 的基本功能、使用 TiUP 快速搭建 TiDB 集群的方法与连接 TiDB 集群并执行 SQL 的方法，建议先观看下面的培训视频（时长 15 分钟）。注意本视频只作为学习参考，如需了解 [TiUP](/tiup/tiup-overview.md) 的具体使用方法和 [TiDB 快速上手具体操作步骤](#在-mac-os-上部署本地测试环境)，请以文档内容为准。
...
3. 在当前 session 执行以下命令启动集群。
    ...
    - 也可以指定 TiDB 版本以及各组件实例个数，命令类似于：
        ...
        上述命令会在本地下载并启动某个版本的集群（例如 v5.3.0）。最新版本可以通过执行 `tiup list tidb` 来查看。运行结果将显示集群的访问方式：
