...
3. 在当前 session 执行以下命令启动集群。
    ...
    > **注意：**
    >
    > 如果按以下方式执行 playground，在结束部署测试后，TiUP 会自动清理掉原集群数据，重新执行命令会得到一个全新的集群。若希望持久化数据，需要在启动集群时添加 TiUP 的 `--tag` 参数：`tiup playground --tag ${tag_name}`，详见[启动集群时指定 `tag` 以保留数据](/tiup/tiup-playground.md#启动集群时指定-tag-以保留数据)。
