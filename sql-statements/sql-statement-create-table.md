...
* TiDB 不支持临时表。如果  [`tidb_enable_noop_functions = 0`](/system-variables.md#tidb_enable_noop_functions-new-in-v40) 时，`CREATE TEMPORARY TABLE` 语法会报错。如果`tidb_enable_noop_functions = 1` 时， TiDB 只会忽略 `TEMPORARY` 关键字。
* 支持除空间类型以外的所有数据类型。
