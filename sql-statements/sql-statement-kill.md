...
* 从 TiDB v6.1.0 起，`enable-global-kill` 默认值为 true。当 `enable-global-kill` 值为 true 时，无论`KILL` 语句和 `KILL TIDB` 语句均能跨实例终止查询或连接，当客户端和 TiDB 中间有代理时，`KILL` 语句和 `KILL TIDB` 语句会被转发给对应的 TiDB 实例执行，无需担心错误地终止其他查询或连接。
* 目前暂时不支持用 Ctrl+C 终止查询或会话，后续版本会进一步支持。
