...
`IMPORT INTO` 支持导入存储在 S3、GCS 和 TiDB 本地的数据文件。
...
执行 `IMPORT INTO` 的用户需要有目标表的 `SELECT`、`UPDATE`、`INSERT`、`DELETE`、`ALTER` 权限，如果是导入存储在 TiDB 本地的文件，还需要有目标表的 `FILE` 权限。
...
- S3 或 GCS URI 路径：配置详见[外部存储](/br/backup-and-restore-storages.md)。此时该路径对应的文件必须存储在当前用户连接的 TiDB 节点上。
- TiDB 本地文件路径：必须为绝对路径，数据文件后缀必须为 `.csv`、`.sql`、或 `.parquet`，且确保当前连接用户有目标表的 `FILE` 权限。
