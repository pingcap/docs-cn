...
- 对于存储在 S3、GCS 或 Azure Blob Storage 的数据文件，`IMPORT INTO` 支持通过 [TiDB 分布式框架](/tidb-distributed-execution-framework.md)运行。
