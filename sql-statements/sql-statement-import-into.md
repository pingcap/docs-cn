...
- TiDB 集群升级期间不支持使用该语句。
- 当使用全局排序时，编码后单行数据总长度不能超过 32 MiB，单个索引 key 长度不能超过 16MiB。
...
全局排序对内存资源使用率较高，在导入开始前，建议先设置 [tidb_server_memory_limit_gc_trigger](/system-variables.md#tidb_server_memory_limit_gc_trigger-从-v640-版本开始引入) 和 [tidb_server_memory_limit](/system-variables.md#tidb_server_memory_limit-从-v640-版本开始引入) 两个变量，避免 golang GC 太多影响导入效率:
