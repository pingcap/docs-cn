...
- 目前该语句支持导入 10 TiB 以内的数据，启用[全局排序](/tidb-global-sort.md)后支持导入 40 TiB 以内的数据。
- 只支持导入数据到数据库中已有的空表。
