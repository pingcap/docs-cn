...
- 对于存储在 S3、GCS 或 Azure Blob Storage 的数据文件，`IMPORT INTO` 支持通过 [TiDB 分布式执行框架](/tidb-distributed-execution-framework.md)运行。
...
- 目前该语句支持导入 10 TiB 以内的数据，启用[全局排序](/tidb-global-sort.md)后能支持 100 TiB 数据导入。
- 只支持导入数据到数据库中已有的空表。
