...
`RESTORE` 语句使用的引擎与 [BR](/br/br-overview.md) 相同，但恢复过程是由 TiDB 本身驱动，而非单独的 BR 工具。BR 工具的优势和警告也适用于 `RESTORE` 语句。需要注意的是，**`RESTORE` 语句目前不遵循 ACID 原则**。
...
有关详细的 URL 语法，见[外部存储](/br/misc/external-storage.md)。
