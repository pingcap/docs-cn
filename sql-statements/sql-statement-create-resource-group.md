...
> - TiDB 集群在初始化时会自动创建 `default` 资源组，所有未绑定资源组的请求都将自动绑定至此资源组。默认配置 `RU_PER_SEC` 为 `UNLIMITED` (等同于`INT`类型最大值，即 `2147483647`), 且为 `BURSTABLE` 模式。在新建配置其他资源组时，建议根据实际情况修改 `default` 资源组的配置。
