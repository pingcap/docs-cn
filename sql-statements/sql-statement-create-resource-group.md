...
| `BURSTABLE`   | 允许对应的资源组超出配额后使用空余的系统资源。分为三种模式，`OFF`，即不允许此资源组超额使用剩余的系统资源；`MODERATED`，即有限度地允许此资源组超额使用剩余的系统资源；`UNLIMITED`，即无限度地允许此资源组超额使用剩余的系统资源。如果仅配置 `BURSTABLE`，而未指定 `MODERATED` 或者是 `UNLIMITED`，将默认启用 `MODERATED`。 | 
