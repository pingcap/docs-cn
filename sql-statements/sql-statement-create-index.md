...
- 多值索引统计信息暂时还没有被收集，多值索引的选择率基于固定假设。查询命中多个多值索引的时候，有可能选不到最优的索引。 这种情况建议用优化器提示 [use_index_merge](/choose-index.md#使用多值索引) 固定执行计划。 
- 复杂的查询条件有可能无法选择到多值索引，多值索引支持的条件模式请参考[索引的选择](/choose-index.md#使用多值索引)。
