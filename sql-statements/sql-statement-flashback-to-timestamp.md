...
* 在执行 `FLASHBACK CLUSTER TO TIMESTAMP` 前，TiDB 会主动断开所有相关表上的连接，并禁止对这些表进行读写操作，直到 `FLASHBACK CLUSTER` 完成。在禁止读写期间，若有数据备份需求，只能使用 br 并指定早于 `FLASHBACK CLUSTER` 开始时间的 `BackupTS` 的进行备份，其他备份同步工具如 PiTR、TiCDC 等都不能正常工作。
