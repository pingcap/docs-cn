...
| task          | 算子属于的 task 种类。目前的执行计划可以分成为四种 task：第一种叫 **root** task，在 tidb-server 上执行；第二种叫 **cop** task，在 TiKV 或者 TiFlash 上并行执行；第三种叫 **batchCop** task，在 TiFlash 上并行执行；第三种叫 **mpp** task，在 TiFlash 上并行执行。当前的执行计划在 task 级别的拓扑关系是一个 root task 后面可以跟许多其他种类 task，root task 使用其他种类 task 的输出结果作为输入。其他种类 task 中执行的也即是 TiDB 下推到 TiKV 或者 TiFlash 上的任务，每个 task 分散在 TiKV 或者 TiFlash 集群中，由多个进程共同执行。 |
