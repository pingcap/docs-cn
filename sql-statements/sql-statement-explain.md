...
| `json`  | `EXPLAIN` 语句将输出 json 格式的执行计划，算子信息将存放在一个 json 数组中           |
...
```sql
EXPLAIN FORMAT = "json" SELECT * FROM t1, t2 WHERE t1.id = t2.id;
```
...
| [
    {
        ...
        "taskType": "root",
        "accessObject": "",
        "operatorInfo": "inner join, equal:[eq(test.t1.id, test.t2.id)]",
        "idx": 0,
        "childrenIdx": [
            1,
            4
    ...
    {
        "id": "TableReader_15(Build)",
        "estRows": "9990.00",
        "taskType": "root",
        "accessObject": "",
        "operatorInfo": "data:Selection_14",
        "idx": 1,
        "childrenIdx": [
            2
        ]
    },
    {
        ...
        "taskType": "cop[tikv]",
        "accessObject": "",
        "operatorInfo": "not(isnull(test.t2.id))",
        "idx": 2,
        "childrenIdx": [
            3
        ]
    ...
    },
    {
        ...
        "taskType": "cop[tikv]",
        "accessObject": "",
        "operatorInfo": "not(isnull(test.t1.id))",
        "idx": 5,
        "childrenIdx": [
            6
        ]
...
在输出结果中，`id`，`estRows`，`task`，`access object`，`operator info` 含义与默认格式相同。`idx` 和 `childrenIdx` 用于表示算子之间的父子关系，`idx` 表示当前算子的编号（从 `0` 开始），`childrenIdx` 数组表示子节点的编号，为空则表示没有子节点。
