...
| `json`  | `EXPLAIN` 语句将输出 JSON 格式的执行计划，算子信息存放在一个 JSON 数组中           |
...
在 `EXPLAIN` 指定 `FORMAT = "json"` 时，示例如下：

```sql
CREATE TABLE t(id int primary key, a int, b int, key(a));
EXPLAIN FORMAT = "json" SELECT id FROM t WHERE a = 1;
```
