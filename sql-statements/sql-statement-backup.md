...
使用 `CONCURRENCY` 来指定 BR 可以同时执行的备份表和索引的任务数量。该参数控制 BR 的线程池大小，优化备份操作的性能和效率。根据备份类型不同，一个任务代表一个表范围或一个索引范围。如果有一个表带有一个索引，则会有两个任务来备份这个表。如果您需要备份许多表或索引，应增加此值。（默认值为4）
