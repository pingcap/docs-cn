...
> **注意：**
>
> - TiDB 从 v8.5.6 版本开始支持在 `FOR UPDATE OF` 子句中使用表别名。为了兼容性，你仍然可以使用定义了别名的基础表名，但这会触发一个推荐使用显式别名的警告。当查询涉及多个具有相同名称但不同数据库的表时（例如 `FROM db1.t, db2.t FOR UPDATE OF t`），TiDB 现在会根据 `FROM` 子句中的顺序从左到右匹配目标表，而不是当前数据库的上下文。为避免歧义，建议在 `FOR UPDATE OF` 子句中指定数据库名称或使用别名。
