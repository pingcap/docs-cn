...
根据历史执行计划创建绑定时，你需要指定相应的 Plan Digest，既可以通过字符串字面量来指定，也可以通过字符串类型的用户变量来指定。你可以通过指定多个 Plan Digest 来同时为多个语句创建绑定，此时你可以指定多个字符串，同时每个字符串也可以包含多个 digest，注意字符串之间和 digest 之间均需使用逗号隔开。
