...
- **离线 DDL 语句**：即数据库接收到用户 DDL 语句，对用户元数据结构进行前，会先对要修改的数据库对象进行加锁，阻塞用户业务在元数据变更过程中对于用户数据的修改。即用户业务会在离线 DDL 语句执行过程中发生阻塞。

- **在线 DDL 语句**：即数据库在执行 DDL 语句时，通过一定的方法，使得 DDL 执行不阻塞用户业务，且能够保证用户业务可在 DDL 执行期间提交修改，在执行过程中保证对应对象的数据正确性与一致性。

按照 DDL 语句的执行是否需要操作 DDL 目标对象所包括的数据来划分，DDL 语句可以划分为：
