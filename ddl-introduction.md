...
summary: 讲解 TiDB 中 DDL 语句的实现原理、在线变更过程、最佳实践等。
---
...
TiDB 采用在线异步变更的方式执行 DDL 语句，这样 DDL 语句的执行不会阻塞其他会话中的 DML 语句。简单来说，用户业务执行过程中，在线异步变更 DDL 可以对数据库对象定义进行变更。

### DDL 语句类型简介

按照执行模式来划分，DDL 语句可以划分为：

- **离线 DDL 语句**：即数据库接收到用户 DDL 语句后，对用户元数据结构进行修改时，会先对要修改的数据库对象进行加锁，在元数据变更过程中阻塞用户业务对用户数据的修改。即用户业务会在离线 DDL 语句执行过程中发生阻塞。
...
TiDB DDL 模块引入 DDL Owner（简称 Owner）角色来代理执行所有进入到 TiDB 集群中的 DDL 语句。在当前 TiDB DDL 模块的实现中，整个 TiDB 集群中只有一个节点能当选为 Owner。当选 Owner 后，TiDB 节点中启动的 worker 才能处理集群中的 DDL 任务。

通过 etcd 的选举功能从多个 TiDB 节点中选举出一个节点来担任 Owner 宿主节点。每个 TiDB 节点都可能当选 Owner（可以通过配置 `run-ddl` 控制某个 TiDB 节点是否竞选 Owner）。Owner 是有任期的，Owner 会主动维护自己的任期，即续约。当 Owner 节点宕机后，其他节点可以通过 etcd 感知到并重新选举出新的 Owner，在集群中继续担任 DDL 任务执行者的角色。
...
TiDB DDL 模块从设计之初就选择了在线异步变更的模式，能够为 TiDB 的用户提供不停机变更业务的服务能力。
...
通过多个小版本演进的方式，确保多个 TiDB 服务器能够正确同步元数据，并保证期间执行用户事务更改数据的正确性与一致性。
