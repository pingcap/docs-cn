...
TiDB 支持 在线 DDL（Online DDL），即在执行 DDL 语句时，数据库通过特定的机制确保 DDL 操作不会阻塞用户业务。你可以在 DDL 执行期间继续提交数据修改，且数据库会保证数据的一致性和正确性。

与传统的离线 DDL（Offline DDL） 不同，离线 DDL 在执行期间会对数据库对象加锁，阻塞用户对数据的修改操作，直到 DDL 执行完成。TiDB 不支持离线 DDL。
