...
当遇到以下情况时，建议使用 TiDB HTAP 提升 TiDB 数据库整体表现：

- 分析查询性能

    某些查询需要进行复杂的数据分析，如聚合、关联等操作，而涉及的表无法使用索引或者索引的选择性很差，需要对大量数据（超过1000万行）进行分析查询，而 TiKV 引擎难以满足查询的性能需求。

- 实时报表计算

    对于那些需要多维度实时分析的复杂报表和数据可视化需求，需要确保用户体验的流畅性。

- 混合负载隔离

    在高并发的 OLTP 业务同时，你可能还需要处理一些 OLAP 业务，同时需要避免这些 OLAP 查询干扰 OLTP 业务性能，确保系统的整体稳定性。

- 简化 ETL 技术栈
    当加工数据量规模中等（100 TB 以内）、数据加工调度流程不复杂时，希望简化替代复杂的 OLTP + ETL + OLAP 技术栈，使用一个数据库同时满足交易系统以及分析系统的需求，降低技术门槛和运维人员需求。

- 强一致性分析

    如果你需要对业务数据进行实时、强一致的分析计算，并且要求数据分析结果和业务数据完全一致，避免数据延迟和不一致的问题。
