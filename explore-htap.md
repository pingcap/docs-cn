...
当遇到以下技术挑战时，建议使用 TiDB HTAP 提升 TiDB 数据库整体表现：

- 分析性能不足

    某些查询属于复杂的数据分析，如聚合、关联等操作，而涉及的表无法使用索引或者索引的选择性很差，需要对大量数据（超过1000万行）进行分析查询，而 TiKV 引擎难以满足查询的性能需求。
...
- 简化 ETL 技术栈
    ...
    当加工数据量规模中等（100 TB 以内）、数据加工调度流程不复杂、并发度不高（10 以内）时，希望简化替代 OLTP + ETL + OLAP 技术栈，使用一个数据库同时满足交易系统以及分析系统的需求，降低技术门槛和运维人员需求。
